...
| HostedService {barındırılan-service-adı} içindeki VM {vm-adı}, VMSnapshot/VMSnapshotLinux Uzantısı içeriyor. Bu uzantının şu an için geçişi desteklenmemektedir. Uzantıyı sanal makineden kaldırın ve geçiş tamamlandıktan sonra Azure Resource Manager kullanarak tekrar ekleyin |Bu, sanal makinenin Azure Backup için yapılandırıldığı senaryodur. Bu şu anda desteklenmeyen bir senaryodur olduğuna göre geçici çözüm, lütfen izleyin https://aka.ms/vmbackupmigration |
...
| Ağ geçidi olmayan PaaS dağıtımları içerdiğinden {vnet-adı} sanal ağı için geçiş desteklenmiyor. | Ağ geçidi olmayan PaaS dağıtımları sanal ağa bağlı uygulama ağ geçidi veya API Management Hizmetleri gibi olduğunda bu hata oluşur.|
