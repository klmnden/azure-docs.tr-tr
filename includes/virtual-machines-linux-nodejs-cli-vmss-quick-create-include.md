...
Şimdi `azure vmss quick-create` komutunu kullanarak ölçek kümenizi oluşturabilirsiniz. Aşağıdaki örnek, `myResourceGroup` adlı kaynak grubunun içinde beş VM örneğine sahip `myVMSS` adlı Linux ölçek kümesi oluşturur:
...
Bu komut, ölçek kümesi dağıtımı tamamlandıktan sonra veri döndürür. Bu noktada, ölçek kümesi yük dengeleyicideki bağlantı noktası 50.000+i'yi VM i'deki bağlantı noktası 22'yle eşleyen NAT kurallarına sahip yük dengeleyici olacaktır. Bu nedenle, yük dengeleyicinin FQDN'sini belirledikten sonra VM'lerimize ssh aracılığıyla bağlanabileceğiz:
