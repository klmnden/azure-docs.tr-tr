...
| [Sistem durumu araştırma davranışını aşağı](../articles/load-balancer/load-balancer-custom-probe-overview.md#probedown) | TCP bağlantıları örneğini araştırma hakkında Canlı kalmasını __ve__ tüm araştırmalar üzerinde. | TCP bağlantıları örneğini araştırma üzerinde etkin kalır. TCP bağlantıları olduğunda tüm araştırmaları alt tarafta olan aşağı. |
...
| Varsayılan olarak güvenli | Varsayılanı Kullan gelen açıkça güvenilir listeye eklenecek akışına için genel IP ve yük dengeleyici uç noktaları ve ağ güvenlik grubu için kapalı. | Ağ güvenlik grubu isteğe bağlı varsayılan açın. |
...
| [Giden kuralları](../articles/load-balancer/load-balancer-outbound-rules-overview.md) | Hangi genel IP adresi veya genel IP öneki, yapılandırılabilir giden boşta kalma zaman aşımı, özel SNAT bağlantı noktası ayırma dahil olmak üzere, bildirim temelli giden NAT yapılandırma | Kullanılamaz |
...
| Fiyatlandırma | Ücret işlenen veri kuralları sayısına göre gelen veya giden kaynakla ilişkilendirilmiş.  | Ücretsiz |
|  |  |  |
