...
Otomatik ölçeklendirme ayarları hazır koşullara göre hizmet örneklerini Ekle/Kaldır olanak sağlar. Bu ayarları Portalı aracılığıyla oluşturulabilir. Bu yöntem oluşturma ve otomatik ölçeklendirme ayarı yapılandırmak için bir tarayıcı tabanlı kullanıcı arabirimi sağlar. Bu öğretici aracılığıyla adımlar:

1. Bir uygulama hizmeti oluşturma
2. Otomatik ölçeklendirme ayarı yapılandırma
3. Bir ölçeklendirme eylemi tetikleyen
4. Bir ölçek eylemi tetikleme
...
1. Tıklatın **yeni** sol gezinti bölmesinden seçeneği
2. Aramak ve seçmek *Web uygulaması* öğesi ve tıklayın **oluştur**
3. Bir uygulama adı gibi seçin *MyTestScaleWebApp*. Yeni bir kaynak grubu oluşturma * myResourceGroup' ve seçtiğiniz kaynak grubuna yerleştirin.
4. Birkaç dakika içinde kaynaklarınızı sağlanmalıdır. Biz, Web App ve karşılık gelen App Service, yalnızca bu öğreticinin geri kalanında oluşturulan planı başvuru.
...
3. Örnek sayısı 1 olarak ayarlayın. Bu ayar, başka bir profil etkin olduğunda veya varsayılan profil etkin örnek sayısı için 1 değerini döndürür sağlar.
...
## <a name="create-recurrence-profile"></a>Yineleme profili oluştur
...
4. İçin **örneği sınırları** ayarlamak **Minimum** '1' olarak **maksimum** '2' olarak ve **varsayılan** olarak '1'. Bu, bu profili 1'den daha az örneği veya 2'den fazla örnekleri için hizmet planı olmayan otomatik ölçeklendirme yapar sağlar. Profili bir karar vermek için yeterli veri yok, varsayılan örnek sayısı (Bu durumda 1) kullanır.

5. İçin **zamanlama** Seç 'belirli günleri Repeat'

6. Pazartesi-Cuma 09:00 PST 18:00 PST yinelenecek profili ayarlayın. Bu, bu profili yalnızca etkin ve geçerli 09: 00 için 6 PM Pazartesi-Cuma olmasını sağlar. Diğer tüm saatler sırasında 'Default' profili, otomatik ölçeklendirme ayarı kullanır profilidir.
...
7. Birkaç dakika içinde örnek sayısı 2'den 1 olarak bırakın. İşlemi en az on dakika sürer.  
...
Bu öğreticide, bir basit Web uygulaması ve App Service planı oluşturuldu. Uygulama hizmeti istek sayısına göre Web uygulaması almakta planı ölçeklendirmek bir otomatik ölçeklendirme ayarı daha sonra oluşturduğunuz. Otomatik ölçeklendirme hakkında daha fazla bilgi edinmek için otomatik ölçeklendirme genel bakış açın ayarları devam edin.
