...
Atama, bir kaynaktır **uyumlu** ilke veya girişim kuralların izleyin değil ise. Aşağıdaki tabloda, farklı ilke efektler elde edilen uyumluluk durumu için Koşul değerlendirmesi ile çalışma gösterilmektedir:
...
Ek olarak **uyumlu** ve **uyumlu**, ilkeleri ve kaynakları diğer üç durumu vardır:

- **Çakışan**: çakışan kurallar (farklı değerlerle aynı etiketi ekleyerek örneğin iki ilke) ile ilkelerinden iki veya daha fazlası mevcut.
- **Başlatılmamış**: Değerlendirme döngüsü, ilke veya kaynak için başlatılmadı.
- **Kayıtlı**: Azure İlkesi kaynak sağlayıcı kaydedilmedi veya oturum açmış hesabın uyumluluk verilerini okuma iznine sahip değil.

İlkeyi kullanan **türü** ve **adı** ilkenin alanlarda kural tanımı bir kaynak bir eşleşme olup olmadığını belirlemek için. Bu nedenle, geçerli olarak kabul edilir ve durumunun gerekir **uyumlu** veya **uyumlu**. Ya da **türü** veya **adı** tüm kaynakların uygun olarak kabul edilir ve değerlendirilir yalnızca ilke kuralı tanımı özelliğidir.

Uyumluluk yüzdesi bölünmesiyle belirlenir **uyumlu** kaynaklar tarafından _toplam kaynakları_.
_Toplam kaynakları_ toplamı olarak tanımlanan **uyumlu**, **uyumlu**, ve **çakışan** kaynakları. Farklı kaynaklar toplamını genel uyumluluk sayılardır **uyumlu** toplamı tüm farklı kaynaklar tarafından ayrılmış. Aşağıdaki görüntüde, geçerli olan 20 farklı kaynak vardır ve yalnızca **uyumlu**. Bu nedenle, genel kaynak uyumluluk 19/20 veya % 95 ' dir.
...
Bir ilke veya girişim farklı kapsamlara atanabilir gibi tablodaki her atama ve bu kapsama atanmış tanım türü için kapsam unutmayın. Uyumlu olmayan kaynakları ve her atama için uyumlu olmayan ilkeler de sağlanır. Bir ilke veya girişim tabloda tıklayarak bu atama için Uyumluluk, daha kapsamlı bir bakış sağlar.

![İlke uyumluluk ayrıntıları](../media/getting-compliance-data/compliance-details.png)

Kaynakları listesini **kaynak Uyumluluk** sekme (varsayılan olarak **uyumlu olmayan** ancak filtrelenebilen) olayları (geçerli atamanın mevcut kaynaklar değerlendirme durumunu yansıtır Append, Denetim, reddetme, dağıtma) kaynak oluşturmak için istek tarafından tetiklenen altında gösterilen **olayları** sekmesi.
...
- Bulma nasıl [uyumlu olmayan kaynakları Düzelt](remediate-resources.md)
