...
Bu makalede, program aracılığıyla oluşturma ve ilkeleri yönetme gösterilmektedir. Ayrıca, kaynak uyumluluk durumlarını görüntülemek gösterilir ve ilkeleri. İlke tanımları, kaynaklarınız üzerinden farklı kuralları ve etkileri uygular. Zorlama kaynakları, Kurumsal standartlarınız ve hizmet düzeyi sözleşmeleri ile uyumluluğu sürdürün, emin olur.
...
4. Henüz yapmadıysanız, Azure CLI'yı yükleyin. En son sürümünü edinebilirsiniz [Windows üzerinde Azure CLI 2.0 yükleme](/cli/azure/install-azure-cli-windows).
...
1. Aşağıdaki JSON kod parçacığında AuditStorageAccounts.json ada sahip bir JSON dosyası oluşturmak için kullanın.
  ...
  Bir ilke tanımı yazma hakkında daha fazla bilgi için bkz. [Azure İlkesi tanım yapısı](policy-definition.md).
...
3. İlke tanımınız oluşturduktan sonra aşağıdaki komutları çalıştırarak ilke ataması oluşturabilirsiniz:
...
2. Aşağıdaki çağrıları birini kullanarak ilke tanımı oluşturun:
  ...
  Önceki {Subscriptionıd}, abonelikte ya da {Managementgroupıd} Kimliğine sahip kimliği ile değiştirin, [yönetim grubu](../azure-resource-manager/management-groups-overview.md).
...
2. Çağrısını kullanarak ilke ataması oluşturun:
...
3. Bir ilke ataması oluşturmak için aşağıdaki komutu kullanın. Örnek bilgiler, değiştirin &lt; &gt; kendi değerlerinizle semboller.
