---
title: 'Öğretici: Event Hubs kullanarak Azure Databricks’e veri akışı yapma | Microsoft Docs'
description: Twitter’dan akış verilerini almak ve verileri gerçek zamanlı olarak okumak için Event Hubs ile Azure Databricks kullanmayı öğrenin.
...
Bu öğreticide bir veri alımı sistemini Azure Databricks’e bağlayarak gerçek zamanlı verilerin bir Apache Spark kümesine akışını yapacaksınız. Azure Event Hubs kullanarak gerçek zamanlı veri alımı sistemi oluşturacak ve sonra gelen iletileri işlemek üzere Azure Databricks’e bağlayacaksınız. Gerçek zaman bir veri akışına erişmek için, Twitter API'lerini kullanarak tweetleri Event Hubs’a alacaksınız. Verileri Azure Databricks’e aldıktan sonra daha ayrıntılı analiz etmek için analiz işleri gerçekleştirebilirsiniz. Bu öğreticide, içinde "Azure" terimi geçen tweetleri ayıklayacaksınız.

Aşağıdaki ekran görüntüsünde uygulama akışı gösterilmektedir:
...
Tweetlerin gerçek zamanlı akışını almak için Twitter’da bir uygulama oluşturun. Yönergeleri izleyerek bir Twitter uygulaması oluşturun ve bu öğreticiyi tamamlamak için ihtiyaç duyduğunuz değerleri kaydedin.
...
Not defterlerini çalıştırmak için **SHIFT + ENTER** tuşlarına basın. Aşağıdaki kod parçacığına benzer bir çıktı görürsünüz. Çıktıdaki her olay, Event Hubs’a alınan ve “Azure” terimini içeren gerçek zamanlı bir tweet’tir. 
...
İşte bu kadar! Azure Databricks kullanarak gerçek zamanlı verilerin Azure Event Hubs'a akışını başarıyla yaptınız. Daha sonra Apache Spark için Event Hubs bağlayıcısını kullanarak akış verilerini kullandınız.
...
>[Azure Databricks kullanarak akış verileri üzerinde yaklaşım analizi](databricks-stream-from-eventhubs.md)
