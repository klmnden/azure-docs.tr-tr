...
ms.locfileid: "46962428"
---
# <a name="navigation-and-dashboards-in-the-application-insights-portal"></a>Gezinti ve panolar Application Insights portalında
Sonra [projenize Application ınsights'ı ayarlama](app-insights-overview.md), uygulamanızın performansı ve kullanımıyla ilgili telemetri verilerini projenizin Application Insights kaynağını görünür [Azure portalında](https://portal.azure.com).
...
İçin oturum açtıktan sonra gördüğünüz ilk şey [Microsoft Azure Portal'da](https://portal.azure.com) bir panodur. Burada, alınan telemetri dahil olmak üzere tüm Azure kaynaklarınız genelinde, sizin için en önemli olan grafikleri bir araya getirebilirsiniz [Azure Application Insights](app-insights-overview.md).

![Özelleştirilmiş Pano.](./media/app-insights-dashboards/31.png)

1. **Belirli kaynaklara gidin** uygulamanıza Application Insights gibi: sol çubuğunu kullanın.
2. **Geçerli panoya dönmek**, veya geçiş son diğer görünümlerle: sol üst köşede açılır menüyü kullanın.
3. **Geçiş panolar**: Pano başlığı üzerinde aşağı açılan menüyü kullanın
4. **Oluşturun, düzenleyin ve panoları paylaşma** Pano araç.
5. **Panoyu Düzenle**: bir kutucuğun üzerine gelin ve ardından üst çubuğunu taşımak, özelleştirme ya da kaldırın.
...
* 1 saate kadar zaman aralığı: 5 dakikada bir yenilenecek
...
Ayrıca [Analytics sabitleme](../log-analytics/query-language/get-started-analytics-portal.md) için grafik bir [paylaşılan](#share-dashboards-with-your-team) Pano. Bu standart ölçümlerin yanı sıra herhangi bir rastgele sorgunun grafikler eklemenize olanak sağlar. 
...
* [**Ölçüm Gezgini** ](app-insights-metrics-explorer.md) -performans ve kullanım kendi grafikler oluşturun.
* [**Arama** ](app-insights-diagnostic-search.md) - olayları istekleri, özel durumlar gibi belirli örneklerini inceleyin veya günlük izlemelerini.
* [**Analytics** ](app-insights-analytics.md) -telemetriniz üzerinde güçlü sorgular.
* **Zaman aralığı** -dikey penceresinde tüm grafikler tarafından görüntülenen aralığını ayarlayın.
* **Silme** -bu uygulama için Application Insights kaynağını silin. Ayrıca uygulama kodunuzdan Application Insights paketlerini kaldırın, veya düzenleme [izleme anahtarını](app-insights-create-new-resource.md#copy-the-instrumentation-key) farklı bir Application Insights kaynağına telemetri yönlendirmek için uygulamanızda.

### <a name="essentials-tab"></a>Temel bilgiler sekmesinde
* [İzleme anahtarı](app-insights-create-new-resource.md#copy-the-instrumentation-key) -bu uygulama kaynağı tanımlar.
...
* [**Uygulama Haritası** ](app-insights-app-map.md) -uygulamanızı bileşenlerini gösteren etkin harita türetilmiş bağımlılık bilgileri.
* [**Akıllı algılama** ](app-insights-proactive-diagnostics.md) -son performans uyarıları gözden geçirin.
* [**Stream canlı** ](app-insights-live-stream.md) - neredeyse anında ölçümler, yeni bir yapı dağıtma yararlı bir dizi sabit veya hata ayıklama A.
* [**Kullanılabilirlik / Web testleri** ](app-insights-monitor-web-app-availability.md) -normal istekleri web uygulamanızdan world.* geçici olarak gönder
* [**Hatalar, performans** ](app-insights-web-monitor-performance.md) -özel durumlar ve hata oranları istekleri için uygulamanızı ve uygulamanıza gelen istekleri için yanıt sürelerini [bağımlılıkları](app-insights-asp-net-dependencies.md).
* [**Performans** ](app-insights-web-monitor-performance.md) -yanıt süresi, bağımlılık yanıt süreleri.
* [Sunucuları](app-insights-web-monitor-performance.md) -performans sayaçları. Kullanılabilir, [Durum İzleyicisi yükleme](app-insights-monitor-performance-live-website-now.md).
* **Tarayıcı** -sayfa görünümü ve AJAX performansı. Kullanılabilir, [web sayfalarınızı araçlama](app-insights-javascript.md).
* **Kullanım** -sayfa görünümü, kullanıcı ve oturum sayıları. Kullanılabilir, [web sayfalarınızı araçlama](app-insights-javascript.md).

YAPILANDIR

* **Başlarken** -satır içi öğretici.
* **Özellikleri** -izleme anahtarı, abonelik ve kaynak kimliği.
* [Uyarılar](app-insights-alerts.md) -ölçüm uyarı yapılandırması.
* [Sürekli dışarı aktarma](app-insights-export-telemetry.md) -Azure depolama alanına telemetri dışarı aktarma yapılandırın.
* [Performans testi](app-insights-monitor-web-app-availability.md#performance-tests) -yapay bir yük kullanarak Web sitenizde ayarlayın.
* [Kota ve fiyatlandırma](app-insights-pricing.md) ve [alım örneklemesi](app-insights-sampling.md).
* **API erişimi** -oluşturma [sürüm ek açıklamaları](app-insights-annotations.md) ve veri erişimi API'si için.
* [**İş öğeleri** ](app-insights-diagnostic-search.md#create-work-item) -bir iş izleme sistemini telemetri denetlenirken hatalar oluşturabilmeniz bağlanın.

AYARLAR

* [**Kilitler** ](../azure-resource-manager/resource-group-lock-resources.md) -Azure kaynakları kilitleme
* [**Otomasyon betiği** ](app-insights-powershell.md) -yeni kaynaklar oluşturmak için şablon olarak kullanabilirsiniz, böylece bir Azure kaynak tanımını dışarı aktarın.
...
|  |  |
| --- | --- |
| [Ölçüm Gezgini](app-insights-metrics-explorer.md)<br/>Filtreleyin ve bölümlendirin ölçümleri |![Arama örneği](./media/app-insights-dashboards/64.png) |
| [Tanılama araması](app-insights-diagnostic-search.md)<br/>Bul ve olayları, ilgili olayları inceleyin ve hatalar oluşturun |![Arama örneği](./media/app-insights-dashboards/61.png) |
