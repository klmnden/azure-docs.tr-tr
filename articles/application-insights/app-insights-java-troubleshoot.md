...
* Bağımlılık <version> öğesi bir desen ile joker karakterler kullanarak (örneğin (Maven) `<version>[2.0,)</version>` veya (Gradle) `version:'2.0.+'`), bunun yerine gibi belirli bir sürümü belirtmeyi deneyin `2.0.1`. Bkz: [sürüm notları](https://github.com/Microsoft/ApplicationInsights-Java/releases) en son sürümü.
...
* Güvenlik Duvarı'nda 80 ve 443 giden trafiği dc.services.visualstudio.com için TCP bağlantı noktalarını açmanız gerekebilir. Bkz: [güvenlik duvarı özel durumlarını tam listesi](app-insights-ip-addresses.md)
...
* Ekleyerek günlüğünü IDE konsol penceresi açmak bir `<SDKLogger />` öğesi kök düğümü altına Applicationınsights.xml dosyasını (projenizin kaynaklar klasörüne) ve yapay ZEKA ile başında girdileri denetle: şüpheli tüm günlükler için bilgileri/uyarı/hata.
...
Dosyaların altında bulunabilir `%temp%\javasdklogs` veya `java.io.tmpdir` Tomcat sunucusunun olması durumunda.
