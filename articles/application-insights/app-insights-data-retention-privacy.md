...
* Ya da uygulamanızla tümleştirin SDK [geliştirme](app-insights-asp-net.md) veya [çalışma zamanında](app-insights-monitor-performance-live-website-now.md). Farklı uygulama türleri için farklı Sdk'ler vardır. Ayrıca bir [web sayfaları için SDK'sı](app-insights-javascript.md), sayfanın yanı sıra son kullanıcının tarayıcıya yükler.
  
  * Her bir SDK çok sayıda sahiptir [modülleri](app-insights-configuration-with-applicationinsights-config.md), farklı tür telemetri toplamak için farklı teknikleri kullanın.
...
* Bazı web sunucuları, ayrıca yanı sıra uygulamayı çalıştırın ve CPU, bellek ve ağ doluluk hakkında telemetri gönderen aracıları vardır. Docker ana bilgisayarları, örneğin, Azure Vm'leri ve [J2EE sunucuları](app-insights-java-agent.md) böyle aracıları olabilir.
* [Kullanılabilirlik testleri](app-insights-monitor-web-app-availability.md) işlemleri, istekleri web uygulamanıza düzenli aralıklarla göndermek Microsoft tarafından çalıştırılır. Sonuçlar Application Insights hizmetine gönderilir.

### <a name="what-kinds-of-data-are-collected"></a>Hangi tür verileri toplanır?
Ana kategoriler şunlardır:

* [Web sunucusu telemetri](app-insights-asp-net.md) -HTTP isteği.  URI, istek, yanıt kodu, istemci IP adresi işlemek için geçen süre. Oturum kimliği.
* [Web sayfaları](app-insights-javascript.md) -sayfası, kullanıcı ve oturum sayıları. Sayfa yükleme süreleri. Özel durumlar. AJAX çağrıları.
...
* [Özel durumlar](app-insights-asp-net-exceptions.md) ve kilitlenmeleri - **yığın dökümlerini**, derleme kimliği, CPU türü. 
* [Bağımlılıkları](app-insights-asp-net-dependencies.md) -REST, SQL, AJAX gibi dış hizmetlerle çağırır. URI veya bağlantı dizesi, süre, başarı, komutu.
* [Kullanılabilirlik testleri](app-insights-monitor-web-app-availability.md) -test ve adımları, yanıt süresi.
* [İzleme günlükleri](app-insights-asp-net-trace-logs.md) ve [özel telemetri](app-insights-api-custom-events-metrics.md) - **günlükleri veya telemetri kodu herhangi bir şey**.
...
Bu yazarak yazılabilir bir [telemetri işlemci eklentisi](app-insights-api-filtering-sampling.md).
