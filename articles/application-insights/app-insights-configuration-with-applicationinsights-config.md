...
* [Microsoft.applicationınsights.Web](http://www.nuget.org/packages/Microsoft.ApplicationInsights.Web) NuGet paketi
* `Microsoft.ApplicationInsights.WindowsServer.UnobservedExceptionTelemetryModule` -parçaları [görev özel durumlarını gözlemlenmeyen](http://blogs.msdn.com/b/pfxteam/archive/2011/09/28/task-exception-handling-in-net-4-5.aspx).
...
Yapabilecekleriniz [kendi başlatıcılar yazma](app-insights-api-filtering-sampling.md#add-properties) bağlam özelliklerini ayarlamak için.
...
* `SyntheticTelemetryInitializer` veya `SyntheticUserAgentTelemetryInitializer` güncelleştirmeleri `User`, `Session`, ve `Operation` tüm telemetri öğelerinin bağlamları özelliklerini, bir kullanılabilirlik testi veya arama motoru robot gibi yapay bir kaynaktan bir isteği işlerken izlenir. Varsayılan olarak, [ölçüm Gezgini](app-insights-metrics-explorer.md) yapay telemetri görüntülemez.
...
* `WebTestTelemetryInitializer` Bu geliyor HTTP istekleri için kullanıcı kimliği, oturum kimliği ve yapay kaynak özelliklerini ayarlar [kullanılabilirlik testleri](app-insights-monitor-web-app-availability.md).
  `<Filters>` Tanımlama isteklerinin özelliklerini ayarlayın.
...
Yapabilecekleriniz [kendi telemetri işlemci yazma](app-insights-api-filtering-sampling.md#filtering).
