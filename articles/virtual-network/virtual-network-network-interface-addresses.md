...
1. Metni içeren kutusunda *arama kaynakları* Azure portalının en üstünde yazın *ağ arabirimleri*. Zaman **ağ arabirimleri** arama sonuçlarında görünecek, onu seçin.
2. Ayarlarını görüntülemek veya IP adresi için listeden değiştirmek istediğiniz ağ arabirimi seçin.
3. Altında **ayarları**seçin **IP yapılandırmaları**.
4. Listeden değiştirmek istediğiniz IP yapılandırması seçin.
5. 5. adımda ayarlar hakkında bilgileri kullanarak, istediğiniz ayarları [bir IP Yapılandırması Ekle](#create-ip-config).
6. **Kaydet**’i seçin.

>[!NOTE]
>Birden fazla IP yapılandırması birincil ağ arabirimi varsa ve birincil IP yapılandırmasının özel IP adresini değiştirmek, birincil ve ikincil IP adresleri (Linux için gerekli değil) Windows içinde ağ arabirimi için el ile atamanız gerekir . El ile bir işletim sistemi içinde bir ağ arabirimi için IP adresleri atamak için bkz: [birden çok IP adresi sanal makinelere atamak](virtual-network-multiple-ip-addresses-portal.md#os-config). El ile bir sanal makine işletim sistemine IP adreslerini eklemeden önce özel hususlar için bkz: [özel](#private) IP adresleri. Herhangi bir ortak IP adresi sanal makine işletim sistemine eklemeyin.
...
3. Altında **ayarları**seçin **IP yapılandırmaları**.
4. Sağ seçin bir [ikincil](#secondary) IP yapılandırması (silemezsiniz [birincil](#primary) yapılandırma) silmek için seçmek istediğiniz **silmek**seçeneğini belirleyip  **Evet**, silmeyi onaylamak için. Yapılandırma için ilişkili ortak bir IP adresi kaynağı varsa, kaynak IP yapılandırmasından ilkenin ilişkisi ancak kaynak silinmez.

**Komutları**

|Aracı|Komut|
|---|---|
|CLI|[az ağ NIC IP-config Sil](/cli/azure/network/nic/ip-config#az_network_nic_ip_config_delete)|
...
- **Yalnızca ortak**: Azure her bir Azure bölgesine benzersiz bir aralıktan adresini atar. Hangi aralıkları her bölgeye atanan öğrenmek için bkz: [Microsoft Azure veri merkezi IP aralıkları](https://www.microsoft.com/download/details.aspx?id=41653). Bir sanal makine (serbest bırakıldığında) yeniden başlatıldı durdurulduğunda adres değiştirebilirsiniz. Atama yöntemlerden birini kullanarak bir IP yapılandırması için genel bir IPv6 adresi atanamıyor.
...
- **Yalnızca ortak**: Azure her bir Azure bölgesine benzersiz bir aralıktan adresini atar. Hangi aralıkları her bölgeye atanan öğrenmek için bkz: [Microsoft Azure veri merkezi IP aralıkları](https://www.microsoft.com/download/details.aspx?id=41653). Adres için atanan ortak IP adresi kaynağı silinmiş veya atama yöntemi dinamik değiştirildiğinde kadar değiştirmez. Bir IP yapılandırması için genel bir IP adresi kaynağı ilişkiliyse, kendi atama yöntemi değiştirmeden önce IP yapılandırmasından ilişkisi gerekir.
...
Bir ortak IP adresi ile temel veya standart SKU oluşturulur. SKU farklar hakkında daha fazla bilgi için bkz: [yönetmek genel IP adresleri](virtual-network-public-ip-address.md).
