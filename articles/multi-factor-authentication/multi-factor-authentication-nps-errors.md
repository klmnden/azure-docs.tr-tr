...
| **CLIENT_CERT_INSTALL_ERROR** | İstemci sertifikası yüklü veya Kiracı ile ilişkilendirilen nasıl bir sorun olabilir. ' Ndaki yönergeleri izleyin [MFA NPS uzantı sorunlarını giderme](multi-factor-authentication-nps-extension.md#troubleshooting) istemci sertifika sorunları araştırmak için. |
| **ESTS_TOKEN_ERROR** | ' Ndaki yönergeleri izleyin [MFA NPS uzantı sorunlarını giderme](multi-factor-authentication-nps-extension.md#troubleshooting) araştırmak için istemci sertifikası ve ADAL sorunları belirteci. |
| **HTTPS_COMMUNICATION_ERROR** | Azure MFA yanıtları almak NPS sunucusunu alamıyor. Güvenlik duvarlarında https://adnotifications.windowsazure.com gelen ve giden trafiği için açık çift yönlü olduğunu doğrulayın |
| **HTTP_CONNECT_ERROR** | NPS uzantısı çalıştıran sunucuya, https://adnotifications.windowsazure.com ve https://login.microsoftonline.com/ erişebildiklerinden emin olun. Bu siteleri yükleme, bu sunucu üzerinde bağlantı sorunlarını giderme. |
| **REGISTRY_CONFIG_ERROR** | Kayıt defteri olabilir uygulama için bir anahtarı eksik [PowerShell Betiği](multi-factor-authentication-nps-extension.md#install-the-nps-extension) yüklendikten sonra Çalıştır değildi. Hata iletisi eksik anahtar içermelidir. Anahtarın HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureMfa altında olduğundan emin olun. |
...
| **InvalidParameter** | Şirket adı uzunluğu \{0} \ izin verilen maksimum {1} uzun |
