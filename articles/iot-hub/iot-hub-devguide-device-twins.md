...
Cihaz ikizi yaşam döngüsü karşılık gelen bağlı [cihaz kimliği][lnk-identity]. Cihaz ikizlerini örtük olarak oluşturulur ve bir cihaz kimliği oluşturulduğunda veya IOT Hub'ında silinmiş silinir.
...
* **Cihaz kimlik özelliklerini**. Cihaz ikizi JSON belgesinin kök saklanan karşılık gelen bir cihaz kimliği salt okunur özelliklerini içeren [kimlik kayıt defteri][lnk-identity].
...
Kapsayıcı nesneleri için ve kök nesne cihaz kimlik özelliklerdir `tags` ve her ikisi de `reported` ve `desired` özellikleri. `properties` Kapsayıcı bazı salt okunur öğeleri içerir (`$metadata`, `$etag`, ve `$version`) açıklanan [cihaz ikizi meta verilerini] [ lnk-twin-metadata] ve [ İyimser eşzamanlılık] [ lnk-concurrency] bölümler.
...
> [!NOTE]
> Bildirilen özellikler, çözüm arka ucu bir özelliğin son bilinen değer burada ilgilendiği senaryoları basitleştirin. Kullanım [CİHAZDAN buluta iletileri] [ lnk-d2c] cihaz telemetrisi zaman serisi gibi zaman damgalı olaylar dizisini biçiminde işlemek çözüm arka ucu gerekip gerekmediğini.
...
3. Çözüm arka ucu yapılandırma işlemi sonuçlarını birçok cihaz arasında göre izleyebilirsiniz [sorgulama] [ lnk-query] cihaz ikizlerini.
...
* SQL benzeri kullanarak cihaz çiftlerini sorgulamak [IOT Hub sorgu dili][lnk-query].
* Cihaz ikizlerini kullanarak büyük kümesi işlemleri [işleri][lnk-jobs].
...
Önceki tüm işlemleri gerektiren **DeviceConnect** tanımlandığı gibi izni [güvenlik] [ lnk-security] makalesi.

[Azure IOT cihaz SDK'ları] [ lnk-sdks] birçok dilleri ve platformları önceki işlemlerle kolaylaştırır. IOT hub'ı temel elemanlar için istenen özellikleri eşitleme ayrıntıları hakkında daha fazla bilgi için bkz. [cihaz yeniden akış][lnk-reconnection].
...
IOT Hub, zaman damgası son güncelleştirme her JSON nesnesi, cihaz ikizi için istenen ve bildirilen özellikler oluşturur. Zaman damgaları UTC biçiminde ve kodlanmış [ISO8601] biçimi `YYYY-MM-DDTHH:MM:SS.mmmZ`.
Örneğin:
...
(Örneğin, cihaz, istenen özellikleri gözleme uygulaması) bir gözlemci aracı alma işleminin sonucu bir güncelleştirme bildirimi arasındaki yarışa hazır mutabık kılmanız gerekir sürümleri de yararlıdır. Bölüm [cihaz yeniden akış] [ lnk-reconnection] daha fazla bilgi sağlar.
...
> [!NOTE]
> Bu mantık zaten uygulanan [Azure IOT cihaz SDK'ları][lnk-sdks]. Bu açıklama, yalnızca cihaz uygulamasını herhangi birini Azure IOT cihaz SDK'ları kullanamazsınız ve MQTT arabirimi doğrudan program gerekir yararlı olur.
> 
> 
...
* [IOT Hub uç noktaları] [ lnk-endpoints] makale her IOT hub'ı ortaya koyan çalışma zamanı ve yönetim işlemleri için çeşitli uç noktaları açıklar.
* [Azaltma ve kotalar] [ lnk-quotas] makalede IOT Hub hizmeti ve hizmetin kullandığınızda beklenir azaltma davranışını uygulanan kotalar.
* [Azure IOT cihaz ve hizmet SDK'ları] [ lnk-sdks] makale çeşitli dil IOT hub'ı ile etkileşim kuran hem cihaz hem de hizmet uygulamaları geliştirirken kullanabileceğiniz SDK'ları listeler.
* [Cihaz ikizleri, işler ve ileti yönlendirme için IOT Hub sorgu dili] [ lnk-query] makalede IOT Hub'ından, cihaz ikizleri ve işler hakkında bilgi almak için kullanabileceğiniz IOT Hub sorgu dili.
* [IOT hub'ı MQTT desteği] [ lnk-devguide-mqtt] makale ve MQTT protokolünü IOT hub'ı desteği hakkında daha fazla bilgi sağlar.

## <a name="next-steps"></a>Sonraki adımlar
Artık öğrendiğiniz cihaz ikizleri hakkında aşağıdaki IOT Hub Geliştirici Kılavuzu konuları ilginizi çekebilir:

* [Anlama ve IOT Hub'ında modül ikizlerini kullanma][lnk-module-twins]
* [Bir cihazda doğrudan yöntem çağırma][lnk-methods]
* [Birden fazla cihazda işleri zamanlama][lnk-jobs]
