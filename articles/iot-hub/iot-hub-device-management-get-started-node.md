...
3.  **IoT Hub** dikey penceresinde IoT Hub'ınız için yapılandırmayı seçin.
  ...
  -   **Cihaz Yönetimini Etkinleştirme** için kutuyu işaretleyin. **Cihaz Yönetimini Etkinleştirme** için kutuyu işaretlemezseniz örnekler çalışmaz. **Cihaz Yönetimini Etkinleştir**’i işaretleyerek IoT Hub’ın yalnızca Doğu ABD, Kuzey Avrupa ve Doğu Asya’da desteklenen ve üretim senaryolarına yönelik olmayan bir önizlemesini oluşturursunuz. Cihaz yönetimi etkinleştiren hub’lara veya bu hub’lardan cihaz geçişi yapamazsınız.
...
Artık cihazlar cihaz kayıt defterine eklendiğine göre, sanal yönetilen cihazları başlatabilirsiniz. Azure IoT Hub'da sağlanan her bir cihaz kimliği için bir sanal cihaz başlatmanız gerekir.
...
Bu betik, **devicecreds.txt** dosyasında listelenen her bir cihaza yönelik **iotdm\_simple\_sample**'ı başlatmak üzere çalışmanız için gereken komutları çıkarır. Her bir sanal cihaz için ayrı bir terminal penceresinden komutları tek tek çalıştırın. Sanal cihaz, siz komut penceresini kapatana kadar çalışmaya devam eder.
...
**simulate.sh**'yi çalıştırdığınızda, çıktı penceresinde bir veri akışı görürsünüz. Bu çıktı, uygulamaya özgü geri çağırma işlevlerindeki **printf** deyimlerinin yanı sıra gelen ve giden trafiği gösterir. Bu çıktı, örnek uygulamanın kodu çözülmüş paketleri nasıl işlediğinin yanı sıra gelen ve giden trafiği de görmenize olanak sağlanır. Cihaz IoT Hub'a bağlandığında, hizmet cihazdaki kaynakları gözlemlemeye otomatik olarak başlar. Ardından, IoT Hub DM istemci kitaplığı, cihazdan en son değerleri almak için cihaz geri çağırmalarını çalıştırır.
