---
title: Intel Edison buluta (C) - Azure IOT hub'a bağlanma Intel Edison | Microsoft Docs
...
ms.locfileid: "38676632"
---
# <a name="connect-intel-edison-to-azure-iot-hub-c"></a>Intel Edison için Azure IoT Hub (C) bağlanma

[!INCLUDE [iot-hub-get-started-device-selector](../../includes/iot-hub-get-started-device-selector.md)]

Bu öğreticide, Intel Edison ile çalışmanın temel bilgileri öğrenerek başlayın. Daha sonra kullanarak cihazlarınızı buluta sorunsuz bir şekilde bağlanmak nasıl öğrenin [Azure IOT hub'ı](iot-hub-what-is-iot-hub.md).
...
Artık panonuz derlendiğinden.
   ...
   ![Birleştirilmiş Pano](media/iot-hub-intel-edison-kit-c-get-started/5_assembled_board.jpg)
...
1. Panonuzu açın Grove temel kalkan yerleştirin. Tüm PIN'ler panonuzu sıkı bir şekilde takılı olduğundan emin olun.
   ...
   ![Grove temel koruma](media/iot-hub-intel-edison-kit-c-get-started/6_grove_base_sheild.jpg)

2. Grove sıcaklık algılayıcısı Grove temel kalkan üzerine bağlamak için Grove kablosu kullanın **A0** bağlantı noktası.

   ![Bağlanmak için sıcaklık algılayıcısı](media/iot-hub-intel-edison-kit-c-get-started/7_temperature_sensor.jpg)
   
   ![Edison ve algılayıcı bağlantı](media/iot-hub-intel-edison-kit-c-get-started/16_edion_sensor.png)
...
1. İçinde güç kaynağı takın.
   ...
   ![Takın güç kaynağı](media/iot-hub-intel-edison-kit-c-get-started/8_plug_power.jpg)
...
2. Mikro USB kablosu üst mikro USB bağlantı noktasına takın.
   ...
   ![En iyi mikro USB bağlantı noktası](media/iot-hub-intel-edison-kit-c-get-started/10_top_usbport.jpg)
...
> Edison bilgisayarınızı ile aynı ağa bağlı olduğundan emin olun. Bilgisayarınız, Edison için IP adresini kullanarak bağlanır.
   ...
   ![Bağlanmak için sıcaklık algılayıcısı](media/iot-hub-intel-edison-kit-c-get-started/12_configuration_tool.png)
...
1. İçin Intel Edison bağlanmak için ana bilgisayarınız aşağıdaki SSH istemcilerden birini kullanın. IP adresi yapılandırma aracını ve bu Aracı'nda belirlediğiniz bir paroladır.
    ...
    - Ubuntu veya macOS üzerinde yerleşik SSH istemcisi (çalıştırma `ssh root@"the IP address"`).
...
3. Ardından Azure IOT SDK'yı derlemek için aşağıdaki komutu çalıştırmak için depo klasörüne gidin
...
1. Yapılandırma dosyası, aşağıdaki komutları çalıştırarak açın:
   ...
   İki makro vardır. Bu dosyada configurate olabilir. İlki `INTERVAL`, buluta gönderme iki ileti arasındaki zaman aralığını tanımlar. İkincisi `SIMULATED_DATA`, olduğu için veya sanal sensör verilerini kullanıp kullanmayacağınızı bir Boolean değeri.

   Varsa, **algılayıcı yok**ayarlayın `SIMULATED_DATA` değerini `1` örnek uygulama oluşturun ve sanal sensör verilerini kullanın.

2. Kaydet ve Çık denetimi-O tuşlarına basarak > Enter > X denetimi.
...
1. Örnek uygulama, aşağıdaki komutu çalıştırarak oluşturun:
   ...
   cmake . && make
   ```
   ![Derleme çıkışı](media/iot-hub-intel-edison-kit-c-get-started/14_build_output.png)
...
![Çıkış - Intel Edison IOT hub'ına gönderilen algılayıcı verileri](media/iot-hub-intel-edison-kit-c-get-started/15_message_sent.png)
