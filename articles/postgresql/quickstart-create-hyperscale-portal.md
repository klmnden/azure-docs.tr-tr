...
Şu ana kadar sorgular github dahil\_olayları özel olarak, ancak bu bilgiler github ile birleştirdik\_kullanıcılar. Parçalı Biz bu yana hem kullanıcılar hem de aynı tanımlayıcıyı olaylarına (`user_id`), kullanıcı kimliklerini eşleşen her iki tablonun satırlarını olacaktır [birlikte](http://docs.citusdata.com/en/stable/sharding/data_modeling.html#colocation) aynı veritabanı düğümleri ve kolayca katılabilir.
