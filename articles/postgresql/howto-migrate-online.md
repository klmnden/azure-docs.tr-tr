---
title: "Azure veritabanı çevrimiçi geçiş PostgreSQL için | Microsoft Docs"
description: "Bu makalede Azure veritabanında PostgreSQL ve ilk yükleme ayarı için pg_dump tarafından oluşturulan ve sürekli arşiv dosyasını PostgreSQL veritabanını geri yüklemeden bir PostgreSQL veritabanına dökümü dosyasına çıkartarak çevrimiçi bir geçiş gerçekleştirme kaynak veritabanından veri eşitleme Attunity çoğaltmak için Microsoft Migrations kullanarak hedef veritabanına."
services: postgresql
author: HJToland3
ms.author: jtoland
manager: jhubbard
...
---
# <a name="online-migration-to-azure-database-for-postgresql"></a>Azure veritabanı çevrimiçi geçiş PostgreSQL için
...
1. **PostgreSQL kaynağı şemasını geçirmek** kullanarak PostgreSQL için Azure veritabanı [pg_dump](https://www.postgresql.org/docs/9.3/static/app-pgdump.html) ve [pg_restore](https://www.postgresql.org/docs/9.3/static/app-pgrestore.html) - n parametresine sahip.

2. **İlk yükleme ve sürekli veri eşitleme kaynak veritabanından hedef veritabanına ayarlama** Attunity çoğaltmak için Microsoft Migrations kullanarak. Bunu yaptığınızda bu nedenle kaynak veritabanı üzerinde Azure uygulamalarınızı hedef PostgreSQL veritabanına geçiş hazırlarken salt okunur olarak ayarlanması gerekir süresini en aza indirir.
