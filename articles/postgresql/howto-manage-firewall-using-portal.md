...
1. PostgreSQL server dikey penceresinde, ayarlar altında başlığını tıklatın **bağlantı güvenliği** PostgreSQL için Azure veritabanı için bağlantı güvenlik dikey penceresini açmak için.

  ![Azure portal - bağlantı güvenliği](./media/howto-manage-firewall-using-portal/1-connection-security.png)

2. Tıklatın **eklemek IP** araç çubuğunda. Azure sistem tarafından algılanan gibi bu otomatik olarak bilgisayarınızı ortak IP adresiyle bir güvenlik duvarı kuralı oluşturur.
...
4. Ek adres aralıklarını ekleyin. Azure veritabanı PostgreSQL için güvenlik duvarı kurallarında tek bir IP adresi veya adres aralığı belirtebilirsiniz. Kural tek bir IP adresi için sınırlamak istiyorsanız, IP başlangıç ve bitiş IP için aynı adres alanına yazın. Güvenlik Duvarı'nı açarak, Yöneticiler, kullanıcılar ve uygulamalar için PostgreSQL sunucu üzerindeki herhangi bir veritabanı için geçerli kimlik bilgilerine sahip oldukları oturum açma sağlar.
