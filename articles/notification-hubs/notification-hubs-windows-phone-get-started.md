<properties
    pageTitle="Windows Phone'da Azure Notification Hubs ile anında iletme bildirimleri gönderme | Microsoft Azure"
...
Bu öğretici, bir Windows Phone 8 veya Windows Phone 8.1 Silverlight uygulamasına anında iletme bildirimleri göndermek için Azure Notification Hubs'ın nasıl kullanılacağını size gösterir. Windows Phone 8.1'i (Silverlight olmayan) hedefliyorsanız [Windows Evrensel](notification-hubs-windows-store-dotnet-get-started.md) sürümüne bakın.
...
> [AZURE.NOTE] Notification Hubs Windows Phone SDK'sı, Windows Anında Bildirim Hizmeti'ni (WNS) Windows Phone 8.1 Silverlight uygulamaları ile kullanmayı desteklemez. WNS'yi (MPNS yerine) Windows Phone 8.1 Silverlight uygulamaları ile kullanmak için REST API'ler kullanan [Notification Hubs - Windows Phone Silverlight öğreticisi]'ni izleyin.
...
5. App.xaml.cs dosyasında **Application_Launching** yönteminin üzerine aşağıdaki kodu ekleyin:
    ...
    >[AZURE.NOTE] **MyPushChannel** değeri, [HttpNotificationChannel](https://msdn.microsoft.com/library/windows/apps/microsoft.phone.notification.httpnotificationchannel.aspx) koleksiyonunda var olan bir kanalı aramak için kullanılan bir dizindir. Bu koleksiyonda bir tane bulunmuyorsa bu adla yeni bir giriş oluşturun.
    ...
    >[AZURE.NOTE]Bu öğretici cihaza bir bildirim gönderir. Bir kutucuk bildirimi gönderdiğinizde, kanalda **BindToShellTile** yöntemini çağırmayı tercih etmeniz gerekir. Bildirim ve kutucuk bildirimlerinin her ikisini de desteklemek için **BindToShellTile** ve **BindToShellToast** yöntemlerini çağırın.
...
8. Uygulamayı kapatın.  
   ...
   >[AZURE.NOTE] Bildirim biçiminde bir anında iletme bildirimi almak için uygulamanın ön planda çalışmıyor olması gerekir.
...
Notification Hubs ile tümleştirilmiş bir ASP.NET WebAPI arka ucundan nasıl anında iletme bildirimleri gönderildiğinin bir örneği için bkz: [Azure Notification Hubs .NET arka ucu ile Kullanıcılara Bildirimde Bulunma](./notification-hubs-aspnet-backend-windows-dotnet-notify-users.md).  

[REST API'ler](https://msdn.microsoft.com/library/azure/dn223264.aspx) kullanarak nasıl anında iletme bildirimleri gönderildiğinin bir örneği için [Java'dan Notification Hubs'ı kullanma](./notification-hubs-java-backend-how-to.md) ve [PHP'den Notification Hubs'ı kullanma](./notification-hubs-php-backend-how-to.md)'ya göz atın.
...
4. **Araçlar**'a, **Kitaplık Paket Yöneticisi**'ne ve ardından **Paket Yöneticisi Konsolu**'na tıklayın.

    Bu, Paket Yöneticisi Konsolu'nu görüntüler.

5.  **Paket Yöneticisi Konsolu** penceresinde, **Varsayılan projeyi** yeni konsol uygulaması projeniz olarak ayarlayın ve ardından konsol penceresinde aşağıdaki komutu yürütün:
...
6. `Program.cs` dosyasını açın ve aşağıdaki `using` deyimini ekleyin:

        using Microsoft.Azure.NotificationHubs;

6. `Program` sınıfında aşağıdaki yöntemi ekleyin:
    ...
    >[AZURE.NOTE]Bağlantı dizesini **Dinleme** erişimi ile değil, **Tam** erişim ile kullandığınızdan emin olun. Dinleme erişimi dizesinin anında iletme bildirimleri gönderme izinleri yoktur.

4. Aşağıdaki satırı `Main` yönteminize ekleyin:

         SendNotificationAsync();
         Console.ReadLine();

5. Windows Phone öykünücünüz çalışırken ve uygulamanız kapalıyken, konsol uygulama projesini varsayılan başlangıç projesi olarak ayarlayın ve ardından uygulamayı çalıştırmak için `F5` tuşuna basın.
