<properties
    pageTitle="Xamarin uygulamaları için Notification Hubs ile iOS Anında İletme Bildirimleri | Microsoft Azure"
...
+ [Xamarin Studio]
   ...
   > [AZURE.NOTE] iOS anında iletme bildirimlerinin yapılandırma gereksinimleri nedeniyle, örnek uygulamayı benzetici yerine fiziksel bir iOS cihazında (iPhone veya iPad) dağıtmanız ve test etmeniz gerekir.
...
* **Java/PHP**: REST API'ler kullanarak nasıl anında iletme bildirimleri gönderildiğinin bir örneği için bkz. "Java/PHP'den Notification Hubs'ı kullanma" ([Java](notification-hubs-java-backend-how-to.md) | [PHP](notification-hubs-php-backend-how-to.md)).
...
3. `Program` sınıfınıza aşağıdaki yöntemi ekleyin (**bağlantı dizesini** ve **hub adını** değiştirmeyi unutmayın):
...
4. Aşağıdaki satırları `Main` yönteminize ekleyin:

         SendNotificationAsync();
         Console.ReadLine();

5. Uygulamayı çalıştırmak için F5 tuşuna basın. Saniyeler içinde, cihazınızda bir anında iletme bildirimi görüntülendiğini görmeniz gerekir. Wi-Fi veya hücresel veri ağı kullanmanızdan bağımsız olarak, cihazda etkin bir İnternet bağlantınızın olduğundan emin olun.
