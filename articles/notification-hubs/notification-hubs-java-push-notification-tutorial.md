...
Benzer şekilde, Android (GCM), Windows Phone (MPNS) ve Kindle Fire (ADM) için kayıtları oluşturabilirsiniz.
...
**Kayıtları oluşturma kullanarak oluşturduğunuz kayıt kimliği + upsert düzeni**
...
CreateOrUpdate, Patch ve Delete Get ile son tutarlılık sağlar. İstenen işlemi yalnızca çağrı sırasında sistem kuyruğa gider ve arka planda çalıştırılır. Get ana çalışma zamanı senaryosu için ancak yalnızca hata ayıklama için tasarlanmamıştır ve sorun giderme amacıyla, sıkı bir şekilde hizmet tarafından kısıtlanır.
...
Bazen kayıtları toplu işlemi gerçekleştirmek için gereklidir. Genellikle başka bir sistem ile tümleştirme için veya yalnızca büyük düzeltme söylemek etiketleri güncelleştirin. Binlerce kaydı söz konusuysa alma/güncelleştirme akışı kullanmak için önerilmez. İçeri/dışarı aktarma özelliği, senaryo karşılamak üzere tasarlanmıştır. Temel olarak, bir erişim bazı blob kapsayıcısı altında depolama hesabınızı gelen verileri ve çıkış için konum kaynağı olarak sağlar.
