---
title: Jenkins ve Azure
description: "Azure ile Jenkins dağıtma ve tümleştirme"
services: multiple
author: czeumault
manager: carolz
layout: LandingPage
ms.service: multiple
ms.tgt_pltfrm: na
ms.devlang: devops
ms.topic: landing-page
ms.date: 06/20/2017
ms.author: carolz
ms.custom: Jenkins
ms.translationtype: HT
ms.sourcegitcommit: 9569f94d736049f8a0bb61beef0734050ecf2738
ms.openlocfilehash: e2f5caf8c4d3eab600bb015db5a4403314a4f6e3
ms.contentlocale: tr-tr
ms.lasthandoff: 08/31/2017

---
<div class="content">
    <h1>Azure üzerinde Jenkins®</h1>
    <div class="introHolder" style="justify-content: space-between;">
       <div class="intro" style="min-width: 200px">
            <img src="media/Jenkins.png" align="right" alt="Jenkins Logo"> Popüler bir açık kaynak otomasyon sunucusu olan Jenkins, sürekli tümleştirme (CI) ve sürekli teslimi (CD) etkinleştirmek için Azure ile sorunsuz bir şekilde tümleştirilir.  Hızlı başlangıç kılavuzlarımız, öğreticilerimiz ve örneklerimizden yararlanarak Azure araçları ve hizmetleri ile Jenkins’i yüklemeyi, yapılandırmayı ve kullanmayı öğrenin.
       </div>
    </div>
<h2>5 Dakikalık Hızlı Başlangıçlar</h2>
<p>Azure üzerinde Jenkins dağıtmayı öğrenin:</p> 
<a href="/azure/jenkins/install-jenkins-solution-template">Bir Azure Sanal Makinesi üzerinde Jenkins yöneticisi oluşturma</a>
<h2 style="margin-top: 36px">Adım Adım Öğreticiler</h2>
<p>Jenkins’i Azure araçları ve hizmetleriyle tümleştirmeyi öğrenin:</p> 
<ol>
    <li><a href="https://www.visualstudio.com/docs/build/apps/jenkins/build-deploy-jenkins">Jenkins ve Visual Studio Team Services ile CI/CD</a></li>
    <li><a href="/azure/container-service/container-service-kubernetes-jenkins">Jenkins’i GitHub, Docker ve Kubernetes ile tümleştirme</a></li>
    <li><a href="/azure/jenkins/execute-cli-jenkins-pipeline">Azure CLI’yı Jenkins ile çalıştırma</a></li>
    <li><a href="/azure/virtual-machines/linux/tutorial-jenkins-github-docker-cicd">Docker, GitHub ve Jenkins’i tümleştirme</a></li>
    <li><a href="/azure/jenkins/deploy-Jenkins-app-service-plugin">Jenkins Eklentisi ile App Service’e dağıtma</a></li>
</ol>
<h2 style="margin-top: 36px">Kaynaklar ve Araçlar</h2>
<p>Azure üzerinde Jenkins ile çalışmaya yönelik diğer başvuru malzemeleri ve araçlar için aşağıdaki bağlantıları ziyaret edin.</p>
<a href="https://plugins.jenkins.io/azure-vm-agents">Azure VM Aracıları eklentisi</a><br/>
<a href="https://plugins.jenkins.io/windows-azure-storage">Azure Depolama eklentisi</a><br/>
<a href="https://jenkins.io">Jenkins hakkında bilgi edinin</a><br/>
<h4>      
Jenkins®, Software in the Public Interest, Inc.’in tescilli ticari markasıdır. [Jenkins® logosu](https://wiki.jenkins.io/display/JENKINS/Logo), [Jenkins projesinden](https://creativecommons.org/licenses/by-sa/3.0/) [CC BY SA 3.0](https://jenkins.io) altında lisanslanmıştır.</h4>
</div>

