...
6. Uygulamanızın portal dikey penceresinde **URL** bağlantısına (veya menü çubuğundaki **Gözat**’a) tıklayın. Bağlantı bir HTTP adresidir.  
    ...
    Azure Active Directory ile daha önce hiçbir işlem yapmadıysanız varsayılan dizininizde herhangi bir Azure AD kullanıcısı olmayabilir. Bu durumda burada olması muhtemel tek hesap, Azure aboneliğinizle birlikte Microsoft hesabıdır. Bu nedenle daha önce aynı tarayıcıdan uygulamada otomatik olarak oturum açtınız.
