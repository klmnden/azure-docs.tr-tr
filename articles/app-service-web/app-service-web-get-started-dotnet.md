<properties 
    pageTitle="İlk .NET web uygulamanızı Azure’a beş dakikada dağıtın | Microsoft Azure" 
...
# İlk .NET web uygulamanızı Azure’a beş dakikada dağıtın

Bu öğretici, [Azure App Service](../app-service/app-service-value-prop-what-is.md)’e basit bir .NET web uygulaması dağıtmanıza yardımcı olur.
...
1. Komut satırı terminalinizde, bir çalışma dizinine (`CD`) geçip örnek uygulamayı şu şekilde kopyalayın:
    ...
    ![Azure’daki ilk web uygulamanız için uygulama örnek kodunu kopyalama](./media/app-service-web-get-started-languages/dotnet-git-clone.png)
...
4. Git ile herhangi bir kodu gönderdiğiniz şekilde, örnek kodunuzu Azure uygulamanıza dağıtın:
    ...
    ![Azure’daki ilk web uygulamanıza kod gönderme](./media/app-service-web-get-started-languages/dotnet-git-push.png)    

    `git push` kodu Azure’a yerleştirir, gerekli paketleri geri yükler ve ASP.NET ikili değerlerini derler. 
...
Visual Studio’da .NET web uygulamaları oluşturma, geliştirme ve doğrudan Azure’a dağıtma hakkında daha fazla bilgi edinmek için bkz. [Visual Studio kullanarak Azure App Service’e ASP.NET web uygulaması dağıtma](web-sites-dotnet-get-started.md).
