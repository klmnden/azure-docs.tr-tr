...
|Öznitelik|Değer|
|---|---|
|content|9 eleman, 0 öznitelikleri|
|Tanımlı|Genel|
...
Kapsayıcı etkin durum denetimi ve bu kapsayıcı için docker raporları health_status olay varsa, Service Fabric bu sisteminin parçası olarak dahil eder sistem durumu bildirdi. Health_status sağlıksız olduğunda Service Fabric sistem durumu uyarısı rapor eder. Varsayılan olarak ayarlanan true.
|Öznitelik|Değer|
|---|---|
...
Kapsayıcı etkin durum denetimi ve bu kapsayıcı için docker raporları health_status olay varsa, docker tarafından bildirilen health_status sağlıksız olduğunda Service Fabric kapsayıcı yeniden başlatılır.
...
|Öznitelik|Değer|
|---|---|
|content|9 eleman, 5 öznitelikleri|
|Tanımlı|Genel|
...
Kapsayıcı için yalıtım modu. Geçerli değerler varsayılan, işlem veya (yalnızca Windows kapsayıcıları için desteklenir) Hyper-v üzerinde Windows 10 konaklarının, kendi yalıtım modunu ayarından bağımsız olarak Hyper-V yalıtım modunda kapsayıcı çalıştığında yalnızca Hyper-V yalıtım modunda desteklenir.
|Öznitelik|Değer|
|---|---|
|ad|Yalıtım|
|Kullanın|isteğe bağlı|
...
Kapsayıcı ya da kalıcı bir uzak depo barındıran VM bir klasörde olabilir kaynak klasörü.
|Öznitelik|Değer|
|---|---|
|ad|Kaynak|
|Kullanın|gerekli|
...
Kaynak çalışan kapsayıcıda eşlenmiş konumu. Bu nedenle, hedefinizi kapsayıcı içinde zaten bir konumu olamaz.
|Öznitelik|Değer|
|---|---|
|ad|Hedef|
|Kullanın|gerekli|
...
#### <a name="driver"></a>Sürücü
Azure dosyaları birim eklentisi için sürücü "sfazurefile" adıdır.
|Öznitelik|Değer|
|---|---|
|ad|Sürücü|
|Kullanın|isteğe bağlı|
...
|Öznitelik|Değer|
|---|---|
|content|3 öğe, 10 öznitelikleri|
|Tanımlı|Genel|
...
Sürücüsüne geçirilecek sürücü Seçenekleri'ni kullanın. Azure dosyaları birim eklentisi aşağıdaki sürücü seçeneklerini destekler: PaylaşımAdı (birim kapsayıcısı sağlayan Azure dosyaları dosya paylaşımı), storageAccountName (Azure dosyaları dosya paylaşımı içeren Azure depolama hesabı), storageAccountKey () Azure dosyaları dosya paylaşımı içeren Azure depolama hesabı için erişim anahtarı).
Bu üç sürücü seçenek gereklidir.
...
Hizmet bildirimi kaynakları bölümünde bildirilmelidir uç nokta adı.  HTTPS kullanırken, aynı bağlantı noktası ve aynı düğümde dağıtılan sertifika farklı hizmet örnekleri (uygulamasının bağımsız olarak) için kullanmayın. Farklı uygulama durumlarda aynı bağlantı noktasını kullanan iki farklı hizmet yükseltme bir yükseltme hatasına neden olur.
|Öznitelik|Değer|
|---|---|
|ad|EndpointRef|
...
|Öznitelik|Değer|
|---|---|
|content|18 eleman, 0 öznitelikleri|
|Tanımlı|Genel|
...
İşletim sistemi sürümünü oluşturun. Örneğin, yapı, Windows Server sürüm 1709 16299 sürümüdür.
|Öznitelik|Değer|
|---|---|
|ad|işletim sistemi|
|type|xs: String|
...
Ana bilgisayarda kullanılabilir kaynakları kısıtlar ve hizmet kod paketi için kaynak sınırları bildirir.

|Öznitelik|Değer|
|---|---|
|content|0 öğe, 9 öznitelikleri|
|Tanımlı|Genel|
...
Yalnızca bellek Çekişme düğümde algılandığında zorlanır bellek idare için yazılım sınırı (MB cinsinden). Pozitif bir tamsayı olmalıdır.
|Öznitelik|Değer|
|---|---|
...
Kullanılabilir CPU (yalnızca Windows) kullanılabilir yüzdesi. Pozitif bir tamsayı olmalıdır. CPU sınırları hizmet paketini belirtilirse, bu parametre etkili bir şekilde yoksayılır.
|Öznitelik|Değer|
|---|---|
|ad|CpuPercent|
|type|xs: String|
...
Yüksek g/ç hızı (okuma ve yazma) ıops'ye kullanılabilir. Pozitif bir tamsayı olmalıdır.
|Öznitelik|Değer|
|---|---|
|ad|Maximumıops|
|type|xs: String|
...
(Okuma ve yazma) kullanılabilecek maksimum g/ç (Saniyedeki bayt cinsinden). Pozitif bir tamsayı olmalıdır.
|Öznitelik|Değer|
|---|---|
...
Diğer kod paketleri göre blok GÇ ağırlığı. 10 ile 1000 arasında bir pozitif tamsayı olmalıdır.
|Öznitelik|Değer|
|---|---|
|ad|BlockIOWeight|
|type|xs: String|
...
Örneğin, "ActorQueueSGType" bir hizmet grubu için kullanıcı tanımlı tür tanımlayıcısı. Bu değer ApplicationManifest.xml dosyasında hizmeti grubu tanımlamak için kullanılır.
|Öznitelik|Değer|
|---|---|
...
#### <a name="servicetypename"></a>ServiceTypeName
bir hizmet için kullanıcı tanımlı tür tanımlayıcı. Örneğin, "QueueType" veya "CalculatorType". Bu değer ApplicationManifest.xml dosyasında Hizmeti'ni tanımlamak için kullanılır.
|Öznitelik|Değer|
|---|---|
|ad|ServiceTypeName|
|Kullanın|gerekli|
...
Kullanıcı tanımlı bir ServiceComponent ya da bir uygulama için ayarları açıklanır. Bir veya daha fazla anahtar-değer çiftleri bölümlerini oluşur.
...
|Öznitelik|Değer|
|---|---|
|content|0 öğe, 11 öznitelikleri|
|Tanımlı|Genel|
