...
Upshift/downshift gerçekleştirmek için sona eren tarafından bölüm sayısı.
|Öznitelik|Değer|
|---|---|
|ad|ScaleIncrement|
|type|xs: String|
...
Statik veri dosyalarının bulunduğu Name özniteliği tarafından adlı bir klasör, bildirir. Service Fabric, tüm exe ve herhangi bir hizmet bildiriminde listelenen veri paketleri yükseltildiğinde konak ve Destek paketlerinde belirtilen DLLHOSTs dönüşüm.
...
|content|0 öğeyi/öğeleri, 0 öznitelikleri|
|Tanımlanan|yerel olarak [EnvironmentOverridesType complexType](service-fabric-service-model-schema-complex-types.md#environmentoverridestype-complextype), [EnvironmentVariablesType complexType](service-fabric-service-model-schema-complex-types.md#environmentvariablestype-complextype)|
|ad|Değişkeninin|
|minOccurs|0|
|maxOccurs|sınırsız|
...
Grupları kümesi güvenlik ilkelerini başvurulabilir sorumlularını bildirir.
...
#### <a name="scaleincrement"></a>ScaleIncrement
Örnekleri tarafından upshift/downshift gerçekleştirmek için sona eren sayısı.
|Öznitelik|Değer|
|---|---|
|ad|ScaleIncrement|
|type|xs: String|
...
Kod, yapılandırma veya veri paketi paylaşılan gösterir.
...
Bu parametrenin değeri true ise şifrelenir. Uygulama geliştiricisi, bir sertifika oluşturma ve hassas bilgileri şifrelemek için Invoke-ServiceFabricEncryptSecret cmdlet'ini kullanarak sorumludur. Değer şifrelemek için kullanılan sertifika bilgilerini sertifikaları bölümünde belirtilendir.
|Öznitelik|Değer|
|---|---|
|ad|Isencrypted|
|type|xs:Boolean|
...
Bir hizmet üzerinde çalışabilen kümedeki hangi düğümlerin denetlemek için kullanılır. Düğüm özelliği adı ve değeri için hizmet s gereksinimlerini tanımlayan bir anahtar/değer çifti. Ayrı deyimler gerekli kısıtlaması oluşturmak için basit Boole mantığı ile birlikte gruplandırılabilir. Örneğin, "(FirmwareVersion > 12 & & InDMZ == True)".
...
Kod paketi için kaynak sınırları belirtir.
...
Hizmet bildirimi kaynakları'nda uç noktaları için kaynak geçersiz kılmaları açıklar.
...
Service Fabric (genellikle NetworkService hesabı) aynı kimlik bilgilerini başka bir giriş noktası önce çalışır, varsayılan olarak ayrıcalıklı giriş noktası. Ayrı bir Kurulum giriş noktası varlığını, hizmet ana bilgisayarı uzun sürelerle yüksek ayrıcalıklarla çalıştır gereğini ortadan kaldırır. Uygulama bildiriminde yerel sistem hesabı veya bir yönetici hesabı altında başlangıç betiği çalıştırmak için güvenlik izinleri değiştirebilirsiniz.
