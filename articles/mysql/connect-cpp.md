...
- Visual Studio 2019 Community'yi yükleyin. Visual Studio 2019 Community bir tam özellikli, Genişletilebilir, ücretsiz bir ıde'dir. Bu IDE ile Android, iOS, Windows, web ve veritabanı uygulamaları için modern uygulamalar oluşturun ve bulut Hizmetleri. Tam .NET Framework’ü veya yalnızca .NET Core’u yükleyebilirsiniz: hızlı başlangıçtaki kod parçacıkları her ikisiyle de çalışır. Bilgisayarınızda Visual Studio zaten yüklüyse, sonraki iki adımı atlayın.
   1. İndirme [Visual Studio 2019 yükleyici](https://www.visualstudio.com/thank-you-downloading-visual-studio/?sku=Community&rel=15). 
...
1. Visual Studio'dan Proje -> Özellikler -> bağlayıcı -> Genel > ek kitaplık Dizinleri'ne "\lib\opt" dizinini ekleyin (örneğin: C:\Program dosyaları (x86) \MySQL\MySQL Connector C++ 1.1.9\lib\opt) C++ bağlayıcısının.
2. Visual Studio'dan Proje -> Özellikler -> C/C++ -> Genel -> Ek Ekleme Dizinleri:
   - C ++ bağlayıcısının "\include" dizinini ekleyin (örneğin: C:\Program dosyaları (x86) \MySQL\MySQL Connector C++ 1.1.9\include\).
   - Boost kitaplığının kök dizinini ekleyin (örneğin: C:\boost_1_64_0\).
...
Host, DBName, kullanıcı ve parola parametrelerini değiştirin. Parametrelerini, sunucuyu ve veritabanını oluştururken belirttiğiniz değerlerle değiştirebilirsiniz. 
