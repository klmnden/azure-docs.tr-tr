...
| İşleç | Otomatik sağlamayı Yapılandır | Bu işlem otomatik sağlama ilk aşaması ile karşılık gelir.<br><br>**Hızlı Başlangıçlar**: IOT Hub, Azure aboneliğinizde örnekleri ve cihaz sağlama hizmeti yapılandırma operatörü rolü gerçekleştirme. | Kurulum otomatik sağlama |
|  | Cihaz kimliği kaydetme | Bu işlem otomatik sağlama ikinci aşaması ile karşılık gelir.<br><br>**Hızlı Başlangıçlar**: cihaz sağlama hizmeti örneğinizi içinde sanal Cihazınızı kaydetme operatörü rolü gerçekleştirin. Cihaz kimliği (TPM veya X.509) hızlı başlangıçta sanal kanıtlama yöntemi tarafından belirlenir. Geliştirici rol kanıtlama ayrıntıları için bkz. | 
