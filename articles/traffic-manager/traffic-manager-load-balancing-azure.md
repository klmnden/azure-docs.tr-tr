...
1. Azure portalında tıklatın **kaynak oluşturma** > **ağ** > **trafik Yöneticisi profili**  >   **Oluşturma**.
...
1. Sol bölmede, Azure portal'ı tıklatın **kaynak oluşturma** > **ağ** > **uygulama ağ geçidi**.
...
5. Dinleyici aşağıdaki seçeneklerden birini yapılandırın:
    * HTTP kullanırsanız yapılandırmak için hiçbir şey yoktur. **Tamam**’a tıklayın.
...
3. Arka uç havuzunun adı ve Havuzda bulunan makinelerin tüm IP adreslerini girin. Bu senaryoda, biz sanal makinelerin iki arka uç sunucu havuzu bağlanırsınız.
...
5. Kural, aşağıdaki bilgileri sağlayarak yapılandırın.
...
3. Bir uç noktası aşağıdaki bilgileri girerek oluşturabilirsiniz:
  ...
  * **Hedef kaynak türünün**: seçin **genel IP adresi** ve ardından, **hedef kaynak**, daha önce yapılandırılan uygulama ağ geçidinin genel IP adresi seçin.
   ...
   ![Trafik Yöneticisi "Ekle endpoint"](./media/traffic-manager-load-balancing-azure/s3-tm-add-endpoint-blade.png)
...
1. Sol bölmede, Azure portal'ı tıklatın **kaynak oluşturma** > **ağ** > **yük dengeleyici**.
2. Yük Dengeleyici için bir ad seçin.
...
1. İçinde yük dengeleyici altındaki **ayarları**seçin **yoklamaları**ve ardından **Ekle** bir araştırma eklemek için.
 ...
 ![Yük Dengeleyici "Ekle araştırma"](./media/traffic-manager-load-balancing-azure/s4-ilb-add-probe.png)

2. Yoklaması için ad girin.
