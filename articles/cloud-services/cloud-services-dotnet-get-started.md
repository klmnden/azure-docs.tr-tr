<properties
    pageTitle="Azure Cloud Services ve ASP.NET kullanmaya başlama | Microsoft Azure"
...
Öğreticide uygulamayı yerel olarak oluşturma, Azure’a dağıtma ve bulutta çalıştırmanın yanı sıra sıfırdan oluşturma işlemleri gösterilmektedir. Tercih ederseniz sıfırdan oluşturmaya başlayabilir ve ardından test ve dağıtım adımlarını gerçekleştirebilirsiniz.

## Contoso Ads uygulaması

Uygulama bir reklam bülteni panosudur. Kullanıcılar metin girerek ve görüntüyü karşıya yükleyerek bir reklam oluşturur. Küçük resim görüntüleriyle birlikte bir reklam listesi görebilirler ve ayrıntılarını görmek üzere bir reklam seçtiklerinde tam boyutlu görüntüyü görebilirler.
...
Uygulamayı bir Azure aboneliği olmadan yerel olarak çalıştırabilirsiniz, ancak uygulamayı buluta dağıtmak için bir abonelik gerekecektir. Bir hesabınız yoksa, [MSDN abone avantajlarınızı etkinleştirebilir](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A55E3C668) veya [ücretsiz deneme için kaydolabilirsiniz.](/pricing/free-trial/?WT.mc_id=A55E3C668)
...
3. Çözümü derlemek için CTRL+SHIFT+B'ye basın.
...
3. **Çözüm Gezgini**’nde başlangıç projesi olarak **ContosoAdsCloudService** öğesinin seçildiğinden emin olun.

2. Visual Studio 2015 kullanıyorsanız ContosoAdsWeb projesinin uygulama *Web.config* dosyasında SQL Server bağlantı dizesini ve ContosoAdsCloudService projesinin *ServiceConfiguration.Local.cscfg* dosyasını değiştirin. Her iki örnekte de "(localdb)\v11.0" seçeneğini "(localdb)\MSSQLLocalDB" olarak değiştirin.
...
2. **Reklam Oluştur**'a tıklayın.

2. Bazı test verilerini girin ve karşıya yüklenecek bir *.jpg* görüntüsü seçip **Oluştur**’a tıklayın.
...
3. Biraz bekleyin ve ardından küçük resmi görmek için Dizin sayfasını yenileyin.

    ![Dizin sayfası](./media/cloud-services-dotnet-get-started/list.png)

4. Tam boyutlu görüntüyü görmek için reklamınıza ilişkin **Ayrıntılar**’a tıklayın.
...
Aşağıdaki bölümde çözümü bulutta çalışan kuyruklar, blob’lar ve uygulama veritabanı için Azure bulut kaynakları kullanacak şekilde yapılandıracaksınız. Yerel olarak çalıştırmaya devam etmek istemenize rağmen bulut depolama ve veritabanı kaynaklarını kullanabilirsiniz; bunun için bağlantı dizelerini ayarlamanız yeterlidir ve bunun nasıl yapılacağını daha sonra göreceksiniz.
...
1. Tarayıcınızda [Klasik Azure portalı](http://manage.windowsazure.com)’nı açın.

2. **Yeni > Hesapla > Bulut Hizmeti > Hızlı Oluştur**’a tıklayın.

4. URL giriş kutusuna bir URL ön eki girin.

    Bu URL benzersiz olmalıdır.  Seçtiğiniz ön ek başka birisi tarafından zaten kullanılıyorsa bir hata iletisi alırsınız.
...
6. **Bulut Hizmeti Oluştur**’a tıklayın.

    Aşağıdaki görüntüde bulut hizmeti contosoads.cloudapp.net URL’si ile oluşturulur.
...
1. [Klasik Azure portalı](http://manage.windowsazure.com)’nda **Yeni > Veri Hizmetleri > SQL Database > Hızlı Oluştur**’a tıklayın.

1. **Veritabanı Adı** kutusuna *contosoads* yazın.

1. **Sunucu** açılır listesinden **Yeni SQL Database sunucusu** öğesini seçin.

    Alternatif olarak, aboneliğiniz zaten bir sunucuya sahipse aşağı açılan listeden bu sunucuyu seçebilirsiniz.

1. Bulut hizmeti için seçtiğiniz aynı **Bölge**’yi seçin.
...
1. Bir yönetici için **Kullanıcı Adı** ve **Parola** girin.

    **Yeni SQL Veritabanı sunucusu** öğesini seçtiyseniz buraya mevcut bir ad ve parola girmezsiniz, daha sonra veritabanına eriştiğinizde kullanmak üzere şimdi tanımlayacağınız yeni adı ve parolayı girersiniz. Daha önce oluşturduğunuz bir sunucuyu seçtiyseniz önceden oluşturduğunuz yönetici kullanıcı hesabı için parola istenir.

1. **SQL Veritabanı Oluştur**’a tıklayın.

    ![Yeni SQL Veritabanı](./media/cloud-services-dotnet-get-started/newdb.png)

1. Azure veritabanını oluşturmayı tamamladıktan sonra portalın sol bölmesindeki **SQL Veritabanları** sekmesine ve ardından yeni veritabanının adına tıklayın.

2. **Pano** sekmesine tıklayın.

3. **İzin verilen IP adreslerini yönet** öğesine tıklayın.

4. **İzin Verilen Hizmetler** altında **Azure Hizmetleri** seçeneğini **Evet** olarak değiştirin.
...
Gerçek bir uygulamada genellikle uygulama verilerine karşı günlük verileri için ve test verilerine karşı üretim verileri için ayrı hesaplar oluşturursunuz. Bu öğretici için yalnızca tek bir hesap kullanacaksınız.

1. [Klasik Azure portalı](http://manage.windowsazure.com)’nda **Yeni > Veri Hizmetleri > Depolama > Hızlı Oluştur**’a tıklayın.

4. **URL** kutusuna bir URL ön eki girin.
...
5. **Bölge** açılır listesini bulut hizmeti için seçtiğiniz bölgeye ayarlayın.
...
>[AZURE.NOTE] Bu bölümde ve sonraki bölümde kimlik bilgilerini proje dosyalarına depolarsınız. [Hassas verileri ortak kaynak kodu depolarına kaydetmeyin](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#secrets).
...
4. *Web.Release.config* dönüşüm dosyasında `{connectionstring}` öğesini silin ve yerine klasik Azure portalındaki ADO.NET bağlantı dizesini yapıştırın.

5. *Web.Release.config* dönüşüm dosyasının içine yapıştırdığınız bağlantı dizesinde `{your_password_here}` öğesini yeni SQL veritabanı için oluşturduğunuz parola ile değiştirin.
...
5. **Çözüm Gezgini**’nde bulut hizmeti projesindeki **Roller** altında **ContosoAdsWorker**’a ve ardından **Özellikler**’e tıklayın.

    ![Rol özellikleri](./media/cloud-services-dotnet-get-started/rolepropertiesworker.png)

6. **Ayarlar** sekmesine tıklayın.

7. **Hizmet Yapılandırması**’nı **Bulut** olarak değiştirin.

7. `ContosoAdsDbConnectionString` ayarı için **Değer** alanını seçin ve ardından öğreticinin önceki bölümünde kopyaladığınız bağlantı dizesini yapıştırın.
...
7. **Kimlik Doğrulamayı Değiştir** iletişim kutusunda **Kimlik Doğrulaması Yok**’u seçin ve ardından **Tamam**’a tıklayın.

    ![Kimlik Doğrulaması Yok](./media/cloud-services-dotnet-get-started/noauth.png)

8. **Yeni ASP.NET Projesi** iletişim kutusunda **Tamam**’a tıklayın.

9. **Çözüm Gezgini**’nde çözüme (projelerden birine değil) sağ tıklayın ve **Ekle - Yeni Proje**’yi seçin.

11. **Yeni Proje Ekle** iletişim kutusunda sol bölmedeki **Visual C#** altında bulunan **Windows**’u seçin ve ardından **Sınıf Kitaplığı** şablonuna tıklayın.  

10. Projeyi *ContosoAdsCommon* olarak adlandırın ve ardından **Tamam**’a tıklayın.

    Entity Framework bağlamına ve hem web hem de çalışan rolü projelerindeki veri modeline başvurmanız gerekir. Alternatif olarak, web rolü projesinde EF ile ilgili sınıflar tanımlayabilir ve çalışan rolü projesinde bu projeye başvurabilirsiniz. Ancak, alternatif yaklaşımda çalışan rolü projeniz gerekli olmayan web bütünleştirme kodlarına başvuruda bulunur.
...
5. *StorageConnectionString* adlı yeni bir ayar ekleyin. **Tür** değerini *ConnectionString* olarak, **Değer** seçeneğini *UseDevelopmentStorage=true* olarak ayarlayın.
...
3. ContosoAdsCommon projesinde *Class1.cs* dosyasını silin ve indirilen projedeki *Ad.cs* ve *ContosoAdscontext.cs* dosyalarını onun yerine ekleyin.

3. ContosoAdsWeb projesinde indirilen projeden aşağıdaki dosyaları ekleyin.
    - *Global.asax.cs*.  
...
Sınıfın iki oluşturucusu vardır. Birincisi web projesi tarafından kullanılır ve Web.config dosyasına depolanan bir bağlantı dizesinin adını belirtir. İkinci oluşturucu, gerçek bağlantı dizesine geçmenizi sağlar. Bu bir Web.config dosyasına sahip olmadığından çalışan rolü projesi için gereklidir. Bu bağlantı dizesinin nereye depolandığını daha önce gördünüz ve kodun DbContext sınıfını başlattığında bağlantı dizesini nasıl aldığını daha sonra göreceksiniz.
...
Benzer bir kod, *görüntüler* kuyruğuna başvuru edinir ve yeni bir kuyruk oluşturur. Bu durumda hiçbir izin değişikliği gerekli değildir.
