...
|enableNativeCodeExecution|Boole değeri|İsteğe bağlı. Varsayılan değer `true`; yerel kod yürütme ve tam güven varsayılan olarak etkinleştirilir. Bu öznitelik ayarlanırsa `false` web rolü için yerel kodu yürütme devre dışı bırakabilir ve Azure kısmi güven kullanın.|  
...
|Protokolü|Dize|Gereklidir. Dış uç noktası için Aktarım Protokolü. Web rolü için olası değerler şunlardır: `HTTP`, `HTTPS`, `UDP`, veya `TCP`.|  
