...
Azure DevOps Projesi, Microsoft’un Azure için DevOps çözümü olan Visual Studio Team Services’te (VSTS) PHP uygulamanız için Azure kaynakları oluşturan ve sürekli tümleştirme (CI) ve sürekli teslim (CD) işlem hattı ayarlayan basitleştirilmiş bir deneyim sunar.  
...
1. **Yeni** bir VSTS hesabı oluşturun veya **mevcut** bir hesabı kullanın.  VSTS projeniz için bir **ad** seçin.  **Azure aboneliğinizi**, **konumunuzu** ve uygulamanız için bir **ad** seçin.  İşiniz bittiğinde **Bitti**’yi seçin.
    ...
    ![VSTS bilgilerini girme](_img/azure-devops-project-php/vstsazureinfo.png)
...
Azure DevOps projesi otomatik olarak bir CI derlemesi ve yayın tetikleyicisi yapılandırır.  Artık en son çalışmanızı otomatik olarak web sitenize dağıtan bir CI/CD işlemiyle PHP uygulaması üzerinde bir ekiple birlikte çalışmaya hazırsınız.
