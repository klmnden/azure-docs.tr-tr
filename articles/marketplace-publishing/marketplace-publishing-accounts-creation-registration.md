---
title: Oluşturma ve yayımcı hesabı kaydederken | Microsoft Docs
...
### <a name="guidelines-for-company-accounts"></a>Şirket hesapları için yönergeler
...
3. DL'ye gönderilen e-posta adresini doğrulayın.
...
> [!NOTE]
> Kendi lisansını (KLG) getirin olarak kabul edilir ve yalnızca sanal makineler için desteklenen bir **ücretsiz** teklifidir.
>
>
...
2. Git [ http://dev.windows.com/registration?accountprogram=azure ](http://dev.windows.com/registration?accountprogram=azure) kendiniz bir satıcı geliştirme Merkezi'ndeki olarak kaydedilecek. Şunlara dikkat edin:
   ...
   >
    ...
    ![Çizim][img-signin]
3. Telefon numarası veya e-posta adresi aracılığıyla kimliğinizi doğrulamamız "hesabınızı korumamıza yardımcı olun" sihirbazını tamamlayın.

    ![Çizim][img-verify]
4. "Kayıt hesap bilgileri" bölümünde, **hesap ülke/bölge** açılan menüden.
...
5. "Hesap türünüz" seçin **şirket** ve ardından **sonraki** düğmesi.
   ...
   > [!IMPORTANT]
   > Hesap türleri ve seçmek size en uygun olan daha iyi anlamak için lütfen sayfayı görüntülemek [hesap türlerini, konumları ve ücretleri](https://msdn.microsoft.com/library/windows/apps/jj863494.aspx)
   >
   >

    ![Çizim](media/marketplace-publishing-accounts-creation-registration/imgRegisterCo_05.png)
6. Girin **yayımcı görünen adı**, genellikle şirket adı.

   > [!TIP]
   > Teklifinizi listelenen gider sonra geliştirme Merkezi'nde girilen yayımcı görünen adı Azure Market'te görüntülenmez. Ancak bu kayıt işlemini tamamlamak için doldurulması gerekir.
   >
   >
7. Girin **kişi bilgisi** Hesap doğrulama.
   ...
   >
   >
8. Kişi bilgilerini girin **şirket onaylayanı**. Şirket onaylayanı, kuruluşunuz adına geliştirme Merkezi'nde hesap oluşturmak için yetkilendirilmiş olduğunuzu doğrulayın kişidir. Tıklayarak **sonraki** taşımak için **"Ödeme bölümü"** işiniz bittiğinde.
...
Vergi ve bank bilgilerinizi daha sonra güncelleştirmek isterseniz, 3, bölümüne gidebilirsiniz [Yayımlama Portalı'nda hesabınızı kaydedin](#3-register-your-account-in-the-publishing-portal)gelen bağlantıları kullanarak Azure Yayımlama Portalı'nda daha sonra tekrar.

### <a name="add-tax-and-banking-information"></a>Vergi ve bankaya ilişkin bilgileri ekleme
 Ticari teklifleri satın alma için yayımlamak istiyorsanız, ödeme ve vergi bilgilerini ve eklemek Geliştirici Merkezi'nde doğrulama için gönderme gerekir. Yalnızca ücretsiz teklifleri yayımlayacak (veya KLG sunmuyorsa), bu bilgiye eklemeniz gerekmez. Daha sonra ekleyebilirsiniz, ancak vergi bilgileri doğrulamak için biraz zaman alabilir. Ticari teklifleri satın alma için sunacaktır biliyorsanız, olabildiğince çabuk eklemenizi öneririz.

**Banka bilgileri**

1. Oturum [Microsoft Developer Center](http://dev.windows.com/registration?accountprogram=azure) Microsoft hesabınızla.
2. Tıklayın **ödeme hesap** soldaki menüde altında **ödeme yöntemi seçin** tıklayın **banka hesabı** veya **PayPal**.

   > [!IMPORTANT]
   > Müşterilerin Market'te satın alması ticari teklifler varsa, bu bu satın alma işlemleri için ödeme almanıza hesabıdır.
   >
   >
3. Ödeme bilgilerini girin ve tıklayın **Kaydet** memnun olduğunuzda.

   > [!IMPORTANT]
   > Güncelleştirme veya ödeme hesabınızı değiştirmeniz gerekiyorsa yukarıdaki geçerli bilgilerinizi yeni bilgiler ile değiştirerek aynı adımları izleyin. Ödeme hesabınızı değiştirme, ödemeler tarafından en çok bir ödeme dönemi gecikmeye yol açabilir. Bu gecikme, ödeme hesabınızı ayarlarken yaptığımız gibi hesap değişikliğini doğrulamak ihtiyacımız oluşur. Hesabınızı doğrulandıktan sonra hala tam miktar için ücretli; son ödeme için geçerli ödeme döngüsü sonraki teste eklenir.
   >
   >
4. **İleri**’ye tıklayın.
...
2. Tıklayın **vergi profili** soldaki menüde.
3. Üzerinde **, vergi formunu'kurmak** sayfasında ülke veya kalıcı yerleşimi bulunduğu bölgeyi seçin ve ardından ülke veya bölge birincil Vatandaşlık tutun yeri seçin. **İleri**’ye tıklayın.
4. Vergi ayrıntılarınızı girin ve ardından **sonraki**.

> [!WARNING]
> Vergi ve banka hesabı bilgilerini, Microsoft Developer Center hesabınızdaki tamamlamadan, ticari sunar üretime göndermeye mümkün olmayacaktır.
>
>
...
## <a name="3-register-your-account-in-the-publishing-portal"></a>3. Yayımlama Portalı'nda hesabınızı kaydedin
[Yayımlama portalı](http://publish.windowsazure.com) tekliflerinizi yönetmek ve yayımlamak için kullanılır.
...
> [!TIP]
> Katılım ilkeleri açıklanan [Azure Web sitesi](https://azure.microsoft.com/support/legal/marketplace/participation-policies/).
>
>

## <a name="4-steps-to-add-a-co-admin-in-the-publishing-portal"></a>4. Yayımlama, ortak yönetici ekleme adımları portalı
**Yönetici olduğu varsayılırsa,** aşağıda verilen olan ortak yönetici ekleme adımları

> [!NOTE]
> **Yeni kullanıcılar için** yayımlama, ortak yönetici eklemeden önce portalı, yayımlama, en az bir uygulaması oluşturduğunuzdan emin olun portalı. Bu olarak gereklidir **YAYIMCILAR** sekmesi görüntülenir, yayımlama yalnızca en az bir uygulama oluşturduktan sonra portal.
>
>
