...
ms.locfileid: "51635049"
---
# <a name="container-support-in-azure-cognitive-services"></a>Azure Bilişsel hizmetler kapsayıcı desteği

Azure Bilişsel hizmetler kapsayıcı desteği geliştiricilerin Azure'da kullanılabilen zengin API'leri kullanmasına olanak sağlar ancak ile birlikte gelen esnekliğiyle [Docker kapsayıcıları](https://www.docker.com/what-container). Kapsayıcı desteği şu anda bir alt kümesini Azure Bilişsel bölümlerini dahil olmak üzere hizmetler için önizleme modunda [görüntü işleme](Computer-vision/Home.md), [yüz](Face/Overview.md), ve [metin analizi](text-analytics/overview.md).
...
Bu izleme [kısa videoyu](https://azure.microsoft.com/resources/videos/containers-support-of-cognitive-services) tanıtım için.

[Görüntü işleme](Computer-vision/Home.md), [yüz](Face/Overview.md), ve [metin analizi](text-analytics/overview.md) Hizmetleri kullanılabilir [Microsoft Azure](https://azure.microsoft.com). Oturum [Azure portalında](https://portal.azure.com/) oluşturma ve bu hizmetler için Azure kaynaklarını keşfedin.

## <a name="features-and-benefits"></a>Özellikler ve avantajlar

- **Veriler üzerinde denetim**: müşterilerine verileri üzerinde tam denetimi ile Bilişsel Hizmetler'i kullanma izni.  Bu, verileri buluta Gönder olamaz, ancak Bilişsel hizmetler teknoloji erişmesi gereken müşteriler için gereklidir. Tutarlılık, veri, yönetim, kimlik ve güvenlik arasında Karma ortamlarda – destekler.
...
- **Taşınabilir mimarisi**: Bulut, şirket içi ve uç cihazlarında dağıtılan bir taşınabilir uygulama mimarisi oluşturulmasını etkinleştir. Kapsayıcılar ayrıca dağıtılabilir doğrudan [Azure Kubernetes hizmeti](/azure/aks/), [Azure Container Instances](/azure/container-instances/), veya bir [Kubernetes](https://kubernetes.io/) kümesi dağıtıldı için [Azure Yığın](/azure/azure-stack/). Daha fazla bilgi için [Azure Stack dağıtma Kubernetes](/azure/azure-stack/user/azure-stack-solution-template-kubernetes-deploy).
- **Yüksek performans / düşük gecikme süresi**: müşteriler yüksek aktarım hızına ve düşük gecikme süresi gereksinimleri için Azure Kubernetes hizmetinde, uygulama mantığı ve verileri yakın fiziksel olarak çalıştırmak, Bilişsel hizmetler etkinleştirerek ölçeklendirme olanağı sağlar.
