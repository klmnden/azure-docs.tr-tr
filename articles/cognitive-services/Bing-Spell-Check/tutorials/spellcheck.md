---
title: Bing yazım denetleme API (Azure'da Microsoft Bilişsel hizmetler) kullanarak yazım denetleme sonuçlarını alma | Microsoft Docs
description: Bing yazım denetimi kullanmayı gösterir.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 06/23/2018
ms.locfileid: "35352366"
---
# <a name="build-a-web-page-spell-check-client"></a>Bir Web sayfası yazım denetimi istemci derleme

Bu öğreticide, biz kullanıcıların Bing yazım denetleme API sorgu olanak sağlayan bir Web sayfası oluşturmak.

Bu öğretici şunların nasıl yapıldığını gösterir:

> [!div class="checklist"]
> - Bing yazım denetleme API için basit bir sorgu olun
> - Sorgu sonuçları görüntüler

## <a name="prerequisites"></a>Önkoşullar

Birlikte öğreticiyi izlemek için Bing yazım denetleme API'si için bir abonelik anahtarı gerekir. Biri, yoksa [ücretsiz deneme için kaydolun](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Yeni bir Web sayfası oluşturma

Bir metin düzenleyicisinde açın. Adlı, yeni bir dosya Örneğin, spellcheck.html oluşturun.

## <a name="add-html-header"></a>HTML üstbilgisi Ekle

HTML üst bilgileri ekleyin ve komut dosyası bölümü şu şekilde başlayın.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey işlevi

Bing yazım denetleme API anahtarını getSubscriptionKey işlevi döndürür. Yerel depolama (diğer bir deyişle, tanımlama bilgisi) alır ya da gerekirse kullanıcıya sorar.

GetSubscriptionKey işlevi başlar ve tanımlama bilgisi adı şu şekilde bildirin.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

FindCookie yardımcı işlevi, belirtilen tanımlama bilgisi değerini döndürür; tanımlama bilgisi bulunmazsa, boş bir dize döndürür.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

GetSubscriptionKeyCookie yardımcı işlevini Bing yazım denetleme API anahtarı değerini kullanıcıdan ister ve anahtar değeri döndürür.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

GetSubscriptionKeyLocalStorage yardımcı işlevini ilk uygun tanımlama bilgisi bakarak Bing yazım denetleme API anahtarını almayı dener. Tanımlama bilgisi bulunamadı, anahtar değeri kullanıcıya sorar. Daha sonra anahtar değeri döndürür.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

GetSubscriptionKey yardımcı işlevini bir parametre alan **geçersiz**. Varsa **geçersiz** olan **doğru**, getSubscriptionKey Bing yazım denetleme API anahtarını içeren tanımlama bilgisi siler. Varsa **geçersiz** olan **yanlış**, getSubscriptionKey Bing yazım denetleme API anahtarı değerini döndürür.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

GetSubscriptionKey yardımcı işlevini dış getSubscriptionKey işlevi sonucu olarak döndürür. Dış getSubscriptionKey işlevinin tanımı kapatın.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Yardımcı işlevleri

İle önceden biçimlendirilmiş belirtilen metin öncesi yardımcı işlevini döndürür [öncesi](https://www.w3schools.com/tags/tag_pre.asp) HTML etiketi.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Bing yazım denetleme JSON kullanarak API, belirtilen sonuçlarından renderSearchResults işlevi görüntüler yazdırma asıl.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

RenderErrorMessage işlevi belirtilen hata iletisi ve hata kodunu görüntüler.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>bingSpellCheck işlevi

BingSpellCheck işlev metin HTML form alanında kullanıcının girdiği her zaman çağrılır.
İki parametre alır: HTML içeriğini alan ve Bing yazım denetleme API anahtarı oluşturur.

```html
function bingSpellCheck(query, key) {
```

Bing yazım denetleme API uç noktası belirtebilir ve uç noktasına istekleri göndermek için kullanacağı bir XMLHttpRequest nesnesi bildirin.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Ayarlama **Apim abonelik anahtar Ocp** için Bing yazım denetleme API anahtarı değerini üstbilgi.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Uç noktasından yanıt işleyin. 200 (Tamam) durumudur, sonuçları görüntüler; Aksi takdirde hata bilgilerini görüntüleyin.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Ayrıca XMLHttpRequest nesneden olası hata olayları idare eder.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

İsteği gönderin. Close bingSpellCheck işlevi **betik** etiketi ve **head** etiketi.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>HTML gövdesi

Web sayfası yüklediğinde, biz gerekirse kullanıcı bunun istenmesi Bing yazım denetleme API anahtarı olduğundan emin olun.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Bing logosunu görüntüler.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Bir HTML formuna bir metin alanı oluşturun. İşleme **onsubmit** olay ve metin alanı ve Bing yazım denetleme API anahtarını içeriğini geçirme bingSpellCheck işlevini çağırın.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

HTML eklemek **div** sonuçları görüntülemek için kullanırız etiketi. Daha önce tanımladığımız JavaScript için başvuruyor **div** etiketi.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Dosyayı kaydedin.

## <a name="display-results"></a>Sonuçları Görüntüle

Web sayfasını tarayıcınızda açın. İsteminde Bing yazım denetleme API abonelik anahtarınızı girin. Bir sorgu (örneğin, "oş, wlrd!") girin **yazım denetimi** metin kutusuna yerleştirip **Enter**. Web sayfası, ardından sorgu sonuçlarını görüntüler.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Sonraki adımlar

> [!div class="nextstepaction"]
> [Kullanım ve görüntü gereksinimleri](../UseAndDisplayRequirements.md)
