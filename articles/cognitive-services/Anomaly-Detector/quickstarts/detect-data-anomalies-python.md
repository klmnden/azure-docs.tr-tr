...
3. İstek gönderirken `requests.post()`. Uç nokta ve anomali algılama URL'NİZİN tam istek URL'si birleştirin ve üst bilgiler ve json istek verileri içerir. 

4. İstek başarılı olursa, yanıtı döndürür.  
...
3. Anomalileri konumları, veri kümesinde bulun. Yanıtın `isAnomaly` alan belirli veri noktasına bir anomali olup ilgili bir Boole değeri içerir. Liste üzerinde yineleme yapmak ve herhangi bir dizin yazdırma `True` değerleri. Karşılanmadığı, bu değerler, anormal veri noktası dizini karşılık gelir.
