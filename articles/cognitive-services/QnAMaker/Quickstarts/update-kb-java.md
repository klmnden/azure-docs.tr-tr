---
title: Bilgi Bankası Güncelleştirme, Java Hızlı Başlangıcı - Azure Bilişsel Hizmetler | Microsoft Docs
...
Henüz bir bilgi bankanız yoksa, bu hızlı başlangıçta kullanmak için bir örneğini oluşturabilirsiniz: [Yeni bilgi bankası oluşturma](create-new-kb-java.md).

1. Sık kullandığınız IDE'de yeni bir Java projesi oluşturun.
1. Aşağıda sağlanan kodu ekleyin.
1. `key` değerini geçerli bir abonelik anahtarı ile değiştirin.
...
1. [apache httpclient](http://hc.apache.org/downloads.cgi) kitaplığına ihtiyacınız olacak. HttpClient 4.x.x.tar.gz gibi bir ikili dosyayı indirerek projenize aktarmanız gereken tüm jar dosyalarını bulabilirsiniz.
1. [GSON](https://github.com/google/gson) kitaplığı da gereklidir. .jar dosyasını el ile [oluşturup](https://stackoverflow.com/questions/5258159/how-to-make-an-executable-jar-file) içeri aktarın veya Maven gibi tercih ettiğiniz bir proje yönetimi aracına bir bağımlılık ekleyin.
...
Bilgi bankanız güncelleştirildikten sonra Soru-Cevap Oluşturma Portalı’nızdaki [Bilgi bankalarım](https://www.qnamaker.ai/Home/MyServices) sayfasından görüntüleyebilirsiniz. Bilgi bankanızın özelleştirilmiş isteğinize göre değiştiğini göreceksiniz.

Bilgi bankanızın diğer öğelerini değiştirmek için Soru-Cevap Oluşturma [JSON şemasına](https://westus.dev.cognitive.microsoft.com/docs/services/5a93fcf85b4ccd136866eb37/operations/5ac266295b4ccd1554da7600) başvurun ve `getRequest()` metodundaki `req` dizesini değiştirin.
