<properties
    pageTitle="Azure portalı ile Azure Site Recovery'yi kullanarak VMM bulutlarındaki Hyper-V sanal makinelerini Azure'a çoğaltma | Microsoft Azure"
...
Azure Site Recovery'ye hoş geldiniz! Azure portalında Azure Site Recovery'yi kullanarak System Center Virtual Machine Manager (VMM) bulutlarında yönetilen şirket içi Hyper-V sanal makinelerini Azure'a çoğaltmak istiyorsanız bu makaleden faydalanın.

> [AZURE.NOTE] Azure, kaynak oluşturmak ve bu kaynaklarla çalışmak için iki [dağıtım modeli](../resource-manager-deployment-model
> ) kullanır: Azure Resource Manager (ARM) ve klasik model. Ayrıca Azure iki portala sahiptir: Klasik dağıtım modelini destekleyen klasik Azure portalı ve her iki dağıtım modeline de destek sağlayan Azure portalı. 


Azure portalında bulunan Azure Site Recovery birçok yeni özellik sunar:

- Azure portalında Azure Backup ve Azure Site Recovery hizmetleri tek bir Kurtarma Hizmetleri kasasında bir araya getirilmiştir. Tek bir yerden iş sürekliliği ve olağanüstü durum kurtarma (BCDR) sürecini ayarlamanıza ve yönetmenize olanak sağlanır. Birleştirilmiş pano sayesinde şirket içi sitelerinizde ve Azure genel bulutunda işlemleri izleyebilir ve yönetebilirsiniz.
...
- Azure portalındaki Site Recovery, makineleri ARM depolama hesaplarına çoğaltabilir. Yük devretme işleminde Site Recovery, Azure'da ARM tabanlı VM'ler oluşturur.
...
Bu makaleyi okuduktan sonra yorumlarınızı aşağıda bulunan Disqus yorumlarında paylaşabilirsiniz. Teknik sorular için [Azure Kurtarma Hizmetleri Forumu](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr)'nu kullanın.
...
- Kolayca şirket içi altyapınızdan Azure'a yük devredebilir ve Azure'dan şirket içi sitenizdeki Hyper-V ana bilgisayar sunucularına yeniden çalışma (kurtarma) işlemini gerçekleştirebilirsiniz. 
...
Bu senaryoyu dağıtmak için Azure'da ihtiyaç duyulanlar aşağıda verilmiştir.
...
**Azure ağı** | Yük devretme gerçekleştiğinde Azure VM'lerinin bağlanabileceği bir Azure sanal ağı gerekir. Ağın, Kurtarma Hizmetleri kasasıyla aynı bölgede olması gerekir.
...
**Önkoşul** | **Ayrıntılar**
--- | ---
**VMM**| System Center 2012 R2 üzerinde çalışan bir veya daha fazla VMM sunucusu. Her VMM sunucusunun bir veya daha fazla yapılandırılmış bulut içermesi gerekir. Bir bulutun şunları içermesi gerekir:<br/><br/> Bir veya birden çok VMM ana bilgisayar grubu.<br/><br/> Her ana bilgisayar grubunda bir veya daha fazla Hyper-V sunucusu ya da kümesi.<br/><br/>VMM bulutlarını ayarlama hakkında [daha fazla bilgi edinin](http://www.server-log.com/blog/2011/8/26/vmm-2012-and-the-clouds.html).
**Hyper-V** | Hyper-V ana bilgisayar sunucularının Hyper-V rolü içeren Windows Server 2012 R2 sürümünde veya sonraki bir sürümde çalıştırılması ve en son güncelleştirmelerinin yüklenmiş olması gerekir.<br/><br/> Bir Hyper-V sunucusunun bir veya daha fazla VM içermesi gerekir.<br/><br/> Çoğaltmak istediğiniz VM'leri içeren Hyper-V ana bilgisayar sunucusunun veya kümesinin bir VMM bulutunda yönetilmesi gerekir.<br/><br/>Hyper-V sunucularının, doğrudan veya bir ara sunucu üzerinden İnternet'e bağlanması gerekir.<br/><br/>Hyper-V sunucularında [2961977](https://support.microsoft.com/kb/2961977) makalesinde belirtilen düzeltmelerin yüklü olması gerekir.<br/><br/>Hyper-V ana bilgisayar sunucularının Azure'a veri çoğaltabilmesi için internet erişimi gerekir. 
**Sağlayıcı ve aracı** | Azure Site Recovery dağıtımı sırasında, Azure Site Recovery Sağlayıcısı'nı VMM sunucusuna ve Kurtarma Hizmetleri aracısını Hyper-V ana bilgisayarlarına yüklersiniz. Sağlayıcı ve aracının, Azure'a doğrudan internet üzerinden veya bir ara sunucu aracılığıyla bağlanması gerekir. HTTPS tabanlı bir ara sunucunun desteklenmediğini unutmayın. Hyper-V ana bilgisayarlarının ve VMM sunucusundaki ara sunucunun şunlara yönelik erişime izin vermesi gerekir: <br/><br/> *.hypervrecoverymanager.windowsazure.com <br/><br/> *.accesscontrol.windows.net <br/><br/> *.backup.windowsazure.com <br/><br/> *.blob.core.windows.net <br/><br/> *.store.core.windows.net<br/><br/>VMM sunucusunda IP adresi tabanlı güvenlik duvarı kurallarına sahipseniz bu kuralların Azure ile iletişim kurmaya izin verip vermediğini denetleyin. [Azure Veri Merkezi IP Aralıkları](https://www.microsoft.com/download/confirmation.aspx?id=41653)'na ve HTTPS (433) protokolüne izin vermeniz gerekir.<br/><br/>Aboneliğinizin Azure bölgesi ve Batı ABD için IP adresi aralıklarına izin verin.<br/><br/>Ayrıca, VMM sunucusundaki ara sunucunun https://www.msftncsi.com/ncsi.txt adresine erişmesi gerekir.


## Korumalı makine önkoşulları
...
1. [Azure ağı ayarlama](#set-up-an-azure-network) 
2. Azure VM'leri, ağa yük devretme işleminden sonra yerleştirilir. 
2. Çoğaltılan veriler için [Azure depolama hesabı ayarlama](#set-up-an-azure-storage-account).
4. Site Recovery dağıtımı için [VMM sunucusunu hazırlama](#prepare-the-vmm-server). 
5. [Ağ eşlemesi için hazırlanma](#prepare-for-network-mapping). Site Recovery dağıtımı sırasında ağ eşlemesini yapılandırabilmeniz için ağları ayarlayın.

### Azure ağı ayarlama

Yük devretme işleminden sonra oluşturulan Azure VM'lerinin bağlanabilmesi için bir Azure ağı gerekir.

- Ağın, Kurtarma Hizmetleri kasasını dağıtacağınız bölge ile aynı bölgede yer alması gerekir.
- Yük devri yapılan Azure VM'lerinde kullanmak istediğiniz kaynak modeline bağlı olarak, Azure ağını [ARM modunda](../virtual-network/virtual-networks-create-vnet-arm-pportal.md) veya [klasik modda](../virtual-network/virtual-networks-create-vnet-classic-pportal.md) ayarlarsınız.
- Başlamadan önce bir ağ ayarlamanızı öneririz. Başlamadan önce yapmazsanız Site Recovery dağıtımı sırasında yapmanız gerekecektir.
...
- Yük aktarımı yapılan Azure VM'lerinde kullanmak istediğiniz kaynak modeline bağlı olarak, hesabı [ARM modunda](../storage/storage-create-storage-account.md) veya [klasik modda](../storage/storage-create-storage-account-classic-portal.md) ayarlarsınız.
- Başlamadan önce bir hesap ayarlamanızı öneririz. Başlamadan önce yapmazsanız Site Recovery dağıtımı sırasında yapmanız gerekecektir.

### VMM sunucusunu hazırlama

- VMM sunucusunun [önkoşullar](#on-premises-prerequisites) ile uyum sağladığından emin olun.
- Site Recovery dağıtımı sırasında, VMM sunucusundaki tüm bulutların Azure portalında kullanılabilir olması gerektiğini belirtebilirsiniz. Portalda yalnızca belirli bulutların görünmesini isterseniz VMM yönetici konsolunda ilgili bulut için bu ayarı etkinleştirebilirsiniz. 
...
## 1. Adım: Koruma hedeflerinizi seçme
...
## 2. Adım: Kaynak ortamını ayarlama
...
5. Kayıt anahtarını indirin. Kurulumu çalıştırırken bu anahtar gerekir. Anahtar, oluşturulduktan sonra 5 gün boyunca geçerlidir. 
...
- **/proxyUsername**Ara sunucu kullanıcı adını belirten isteğe bağlı parametre (ara sunucu kimlik doğrulaması gerekirse).
- **/proxyPassword**: Ara sunucu ile kimlik doğrulamak için parolayı belirten isteğe bağlı parametre (ara sunucu kimlik doğrulaması gerekirse).
...
Hyper-V ana bilgisayarlarında çalışan Kurtarma Hizmetleri aracısının VM çoğaltması için Azure'a yönelik internet erişimi gerekir. İnternet'e bir ara sunucu aracılığıyla erişiyorsanız aşağıdaki ayarlamaları yapın:
...
4.  Bir depolama hesabı oluşturmadıysanız ve ARM kullanarak bir tane oluşturmak isterseniz bu işlemi satır içinde yapmak için **+Depolama hesabı** seçeneğine tıklayın.  **Depolama hesabı oluştur** dikey penceresinde hesap adını, türünü, aboneliği ve konumu belirtin. Hesabın, Kurtarma Hizmetleri kasasıyla aynı konumda olması gerekir.

    ![Depolama](./media/site-recovery-vmm-to-azure/gs-createstorage.png) 

    Şunlara dikkat edin:

    - Klasik modeli kullanarak bir depolama hesabı oluşturmak isterseniz bu işlemi Azure portalında gerçekleştirirsiniz. [Daha fazla bilgi edinin](../storage/storage-create-storage-account-classic-portal.md)
    - Çoğaltılan veriler için bir premium depolama hesabı kullanıyorsanız şirket içi verilerde gerçekleşen değişiklikleri yakalayan çoğaltma günlüklerini depolamak üzere ilave bir standart depolama hesabı ayarlamanız gerekir.

4.  Henüz bir Azure ağı oluşturmadıysanız ve ARM kullanarak bir ağ oluşturmak isterseniz bu işlemi satır içinde yapmak için **+Ağ** seçeneğine tıklayın. **Sanal ağ oluştur** dikey penceresinde ağ adı, adres aralığı, alt ağ ayrıntıları, abonelik ve konum belirtin. Ağın, Kurtarma Hizmetleri kasasıyla aynı konumda olması gerekir.
...
- Ağ eşlemesi işlevinin genel hatlarını [okuyun](#prepare-for-network-mapping). Daha ayrıntılı bir açıklama için [bu sayfayı okuyun](site-recovery-network-mapping.md).
- VMM sunucusu üzerindeki sanal makinelerin VM ağına bağlı olduğunu ve en az bir Azure sanal ağı oluşturduğunuzu doğrulayın. Birden çok VM ağının tek bir Azure ağına eşlenebileceğini unutmayın.
...
5. **Hedef ağ** kısmında ise çalışmaya başladıklarında çoğaltılan Azure VM'lerinin kaydedileceği Azure ağını seçin. Daha sonra, **Tamam**'a tıklayın.

    ![Ağ eşlemesi](./media/site-recovery-vmm-to-azure/network-mapping2.png)

Ağ eşlemesi başladığında gerçekleşecekler şunlardır:

- Eşleme başladığında kaynak VM ağındaki VM'ler hedef ağ ile bağlantı kurar. Kaynak VM ağına bağlanan yeni VM'ler, çoğaltma gerçekleştiğinde eşlenen Azure ağıyla bağlantı kurar. 
...
- **Bant genişliğini kısıtlama**: İkincil siteye çoğaltılan Hyper-V trafiği belirli bir Hyper-V ana bilgisayarına gider. Ana bilgisayar sunucusunda bant genişliğini kısıtlayabilirsiniz.
...
## 6. Adım: Çoğaltmayı etkinleştirme

Şimdi aşağıda belirtilen şekilde çoğaltmayı etkinleştirin:

1. **2. Adım: Uygulama çoğaltma** > **Kaynak** seçeneklerine tıklayın. Çoğaltmayı ilk kez etkinleştirdikten sonra ilave makineler için çoğaltma işlemini etkinleştirmek için kasada **+Çoğalt**'a tıklayın.
...
4. Kullanmak istediğiniz depolama hesabını seçin. Sahip olduğunuz hesaplardan farklı bir depolama hesabı kullanmak isterseniz [yeni bir hesap](#set-up-an-azure-storage-account) oluşturabilirsiniz. ARM modelini kullanarak bir depolama hesabı oluşturmak için **Yeni oluştur**'a tıklayın. Klasik modeli kullanarak bir depolama hesabı oluşturmak isterseniz bu işlemi [Azure portalında](../storage/storage-create-storage-account-classic-portal.md) gerçekleştirirsiniz. Daha sonra, **Tamam**'a tıklayın.
5. Yük devretme sonrasında çalışmaya başlayan Azure VM'lerinin bağlanacağı Azure ağını ve alt ağını seçin. Koruma için seçtiğiniz tüm makinelere ağ ayarını uygulamak için **Seçili makineler için şimdi yapılandırı** seçin. Makineler için Azure ağını ayrı ayrı seçmek için **Daha sonra yapılandır**'ı seçin. Sahip olduğunuz ağlardan farklı bir ağ kullanmak isterseniz [yeni bir ağ](#set-up-an-azure-network) oluşturabilirsiniz. ARM modelini kullanarak ağ oluşturmak için **Yeni oluştur**'a tıklayın. Klasik modeli kullanarak bir ağ oluşturmak istiyorsanız bu işlemi [Azure portalında](../virtual-network/virtual-networks-create-vnet-classic-pportal.md) gerçekleştirebilirsiniz. Bir alt ağ (varsa) seçin. Daha sonra, **Tamam**'a tıklayın.
...
5. **Özellikler** > **Özellikleri yapılandır** seçeneklerinde, seçili VM'ler için işletme sistemini ve OS diskini seçin. Daha sonra, **Tamam**'a tıklayın. Daha sonra ek özellikleri ayarlayabilirsiniz. 

    ![Çoğaltmayı etkinleştirme](./media/site-recovery-vmm-to-azure/enable-replication6.png)

    
12. **Çoğaltma ayarları** > **Çoğaltma ayarlarını yapılandır** seçeneklerinde, korumalı VM'lere uygulamak istediğiniz çoğaltma ilkesini seçin. Daha sonra, **Tamam**'a tıklayın. **Ayarlar** > **Çoğaltma ilkeleri** > ilke adı > **Ayarları Düzenle**'de çoğaltma ilkesini değiştirebilirsiniz. Uyguladığınız değişiklikler, zaten çoğaltılmış olan makinelere ve yeni makinelere uygulanır.
...
3. **İşlem ve Ağ** > **İşlem özellikleri** seçeneklerinden Azure VM adını ve hedef boyutu belirtebilirsiniz. Gerekirse [Azure gereksinimleri](site-recovery-best-practices.md#azure-virtual-machine-requirements) ile uyum sağlamak için adı değiştirin. Ayrıca, Azure VM'sine atanacak IP adresini, alt ağ ve hedef ağ ile ilgili bilgileri de görüntüleyebilir ve değiştirebilirsiniz. Şunlara dikkat edin:
...
5.  **Diskler** kısmında çoğaltılacak VM'deki işletim sistemi ve veri disklerini görebilirsiniz. 
...
3. **Yük Devretme Testi** kısmında, yük devretme gerçekleştikten sonra Azure VM'lerinin bağlanacağı Azure ağını seçin.
...
5. Yük devretme **Testi tamamla** aşamasına ulaştığında şunları yapın:
    ...
    > [AZURE.NOTE] Bir yük devretme testi iki haftadan fazla sürerse zorla tamamlanır.
