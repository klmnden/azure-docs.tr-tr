...
2. Seçin **OpenAPI belirtimi** listeden.
    ...
    |Ayar|Değer|Açıklama|
    |---|---|---|
    |**OpenAPI Specification**|http://conferenceapi.azurewebsites.net?format=json|API uygulama hizmeti başvurur. API management bu adrese isteklerini iletir.|
    ...
    |**Ad**|*demo-conference-api*|API için benzersiz bir ad sağlar. <br/>Hizmet URL'si girdikten sonra SEKME tuşuna basın, APIM ne json'da olduğuna bağlı olarak bu alanı doldurur.|
    ...
    |**API'si URL soneki**|*conference*|API management hizmeti için temel URL soneki eklenir. API Management API'leri kendi soneki ayırır ve bu nedenle soneki her API için belirtilen yayımcı için benzersiz olmalıdır.|
...
2. Tuşuna **Test** sekmesi.  ![Test API'si](./media/api-management-get-started/test-api.png)
3. Tıklayın **GetSpeakers**.
    Alanlar sorgu parametreleri için sayfanın görüntüler, ancak bu durumda biz yok. Sayfa üstbilgileri alanları de görüntüler. Üst bilgilerinden biri "Ocp-Apim-Subscription-Key" Bu API ile ilişkili ürün abonelik anahtarı içindir. Anahtarı otomatik olarak doldurulur.
4. **Gönder**’e basın.
