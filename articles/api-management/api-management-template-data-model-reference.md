...
|body|Dize|Kod parçacığını kod örnek kısmı için bir yer tutucu.|  
...
|MultipleSubscriptionsCount|numarası|Geçerli kullanıcı tarafından bu ürün için abonelik sayısı.|  
...
|Ürün kimliği|Dize|Abone olunan ürün ürün kaynak tanımlayıcısı. Biçiminde geçerli bir göreli URL değerdir `products/{pid}` burada `{pid}` bir ürün tanımlayıcısı.|  
...
|DisplayName|Dize|Abonelik adını görüntüler.|  
