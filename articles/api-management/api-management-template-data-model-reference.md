...
|Yöntemi|string|İşlemi HTTP yöntemi.|  
...
|MultipleSubscriptionsCount|Sayı|Bu ürün için abonelik maksimum sayısı bir kullanıcı aynı anda sahip olabilir.|  
...
|Özellik|Tür|Açıklama|  
|--------------|----------|-----------------|  
|contentType|string|Kayıtlı ya da özel bir içerik türü için bu gösterim örneğin belirtir `application/xml`.|  
...
|ProductId|string|Abone olunan ürün ürün kaynak tanımlayıcısı. Biçiminde geçerli bir göreli URL değerdir `products/{pid}` burada `{pid}` bir ürün tanımlayıcısı.|  
