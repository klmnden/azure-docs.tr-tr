# Genel Bakış
## [Mantıksal Uygulama’ya genel bakış](logic-apps-what-are-logic-apps.md)
## [Bağlayıcılar listesi](../connectors/apis-list.md)
## [Enterprise Integration Pack](logic-apps-enterprise-integration-overview.md)

# Başlarken
## [Mantıksal uygulama oluşturun.](logic-apps-create-a-logic-app.md)
## [Mantıksal uygulama özelliklerini kullanma](logic-apps-use-logic-app-features.md)
## [Şablonlara genel bakış](logic-apps-use-logic-app-templates.md)
## [Şablon kullanarak uygulama oluşturma](logic-apps-arm-provision.md)
## [Örnekler ve senaryolar](logic-apps-examples-and-scenarios.md)
## [Service Bus senaryosu](logic-apps-scenario-function-sb-trigger.md)
## [B2B işleme](logic-apps-enterprise-integration-b2b.md)
## [XML işleme](logic-apps-enterprise-integration-xml.md)

# Nasıl yapılır?
## Oluşturma
### [Bağlayıcı oluşturma](logic-apps-create-api-app.md)
### [Dosya Sistemi bağlayıcısı kullanma](logic-apps-using-file-connector.md)
### [SAP bağlayıcısı kullanma](logic-apps-using-sap-connector.md)
### [Visual Studio araçları](logic-apps-deploy-from-vs.md)
### [Azure işlevlerini kullanma](logic-apps-azure-functions.md)
### [Çağrılabilir uç noktalar olarak Logic Apps](logic-apps-http-endpoint.md)
### [Döngüler, kapsamlar ve ayırma](logic-apps-loops-and-scopes.md)
### [Özel API'nizi kullanma](logic-apps-custom-hosted-api.md)
### [Özel durum işleme](logic-apps-exception-handling.md)
### [Özel durum işleme senaryosu](logic-apps-scenario-error-and-exception-handling.md)
### [İçerik türlerini işleme](logic-apps-content-type.md)
### [İş akışı tanımı](logic-apps-author-definitions.md)
### [Sorun giderme ipuçları](logic-apps-diagnosing-failures.md)
### [Limitler ve yapılandırma](logic-apps-limits-and-config.md)


## Enterprise Integration Pack’i (EIP) kullanma
### [Tümleştirme hesaplarına genel bakış](logic-apps-enterprise-integration-create-integration-account.md)
### [Sözleşmeler](logic-apps-enterprise-integration-agreements.md)
### [B2B işleme](logic-apps-enterprise-integration-b2b.md)
### [XML işleme](logic-apps-enterprise-integration-xml.md)
### [Düz dosya işleme](logic-apps-enterprise-integration-flatfile.md)
### [XSLT eşlemeleri ekleme](logic-apps-enterprise-integration-maps.md)
### [XML’i dönüştürme](logic-apps-enterprise-integration-transform.md)
### [XML doğrulama](logic-apps-enterprise-integration-xml-validation.md)
### [Sertifikaları kullanma](logic-apps-enterprise-integration-certificates.md)
### [İş ortakları ekleme](logic-apps-enterprise-integration-partners.md)
### [Şema ekleme](logic-apps-enterprise-integration-schemas.md)
### [Meta veri depolama](logic-apps-enterprise-integration-metadata.md)
### [AS2 tümleştirme](logic-apps-enterprise-integration-as2.md)
### [AS2 kodlama](logic-apps-enterprise-integration-as2-encode.md)
### [AS2 kodunu çözme](logic-apps-enterprise-integration-as2-decode.md)
### [EDIFACT tümleştirme](logic-apps-enterprise-integration-edifact.md)
### [EDIFACT kodlama](logic-apps-enterprise-integration-edifact-encode.md)
### [EDIFACT kodunu çözme](logic-apps-enterprise-integration-edifact-decode.md)
### [X12 tümleştirme](logic-apps-enterprise-integration-x12.md)
### [X12 kodlama](logic-apps-enterprise-integration-x12-encode.md)
### [X12 kodunu çözme](logic-apps-enterprise-integration-x12-decode.md)


## Ağ geçidi kullanma
### [Şirket içi bağlanma](logic-apps-gateway-connection.md)
### [Yükleme](logic-apps-gateway-install.md)

## Otomatikleştirme
### [Otomasyon şablonu](logic-apps-create-deploy-template.md)
### [VS’den yayımlama](logic-apps-deploy-from-vs.md)

## Yönet
### [Use Visual Studio Cloud Explorer’ı kullanma](logic-apps-manage-from-vs.md)
### [Mantıksal uygulamanızı güvenli hale getirme](logic-apps-securing-a-logic-app.md)
### [Mantıksal Uygulamaları izleme](logic-apps-monitor-your-logic-apps.md)
### [B2B iletilerini izleme](logic-apps-monitor-b2b-message.md)
#### [AS2 izleme şeması](logic-apps-track-integration-account-as2-tracking-schemas.md)
#### [X12 izleme şeması](logic-apps-track-integration-account-x12-tracking-schema.md)
#### [Özel izleme şeması](logic-apps-track-integration-account-custom-tracking-schema.md)
#### [B2B iletilerini izleme](logic-apps-track-b2b-messages-omsportal.md)
#### [OMS portalını sorgulama](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md)

### [Tümleştirme hesaplarına genel bakış](logic-apps-enterprise-integration-accounts.md)
### [Fiyatlandırma](logic-apps-pricing.md)

# Başvuru
## [PowerShell](/powershell/resourcemanager/azurerm.logicapp/v2.3.0/azurerm.logicapp)
## Şema Geçmişi
### [GA](logic-apps-schema-2016-04-01.md)
### [Önizleme](logic-apps-schema-2015-08-01.md)
## [REST](/rest/api/logic/)

# Kaynaklar
## [Fiyatlandırma](https://azure.microsoft.com/pricing/details/logic-apps/)  
## [MSDN forumu](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azurelogicapps)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-logic-apps)
## [Hizmet güncelleştirmeleri](https://azure.microsoft.com/updates/?product=logic-apps)


<!--HONumber=Jan17_HO3-->


