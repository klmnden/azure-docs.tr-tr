...
## Azure Backup hizmetini kullanarak Azure'a yedekleme yapabileceğim desteklenen işletim sistemlerinin listesi ne şekildedir? <br/>
Azure Backup dosya-klasör yedeklemesi, Azure Backup Sunucusu ve SCDPM kullanarak uygulama yedekleme için aşağıdaki listede bulunan işletim sistemlerini destekler. 
...
- **Linux**: Azure Backup, Core OS Linux hariç [Azure tarafından onaylanan bir dağıtım listesini](../virtual-machines/virtual-machines-linux-endorsed-distros.md) destekler.  Sanal makinede VM aracısı kullanılabilir oldukça ve Python desteği var oldukça diğer Kendi Linux’unu Getir dağıtımları da çalışabilir.
- **Windows Server**:  Windows Server 2008 R2’den eski sürümler desteklenmez.

## En son Azure Backup aracısını nereden indirebilirim? <br/>
...
## SCDPM sunucusunun hangi sürümü desteklenir? <br/>
...
## Azure Yedekleme aracısını yapılandırırken kasa kimlik bilgilerini girmem isteniyor. Kasa kimlik bilgilerinin süresi dolar mı?
...
## Her bir Azure aboneliği için oluşturulan kasaların sayısına yönelik herhangi bir sınır var mıdır? <br/>
Evet. Ağustos 2016’dan itibaren bir abonelikte 25 kasa (yedekleme kasası ve Kurtarma Hizmetleri Kasası için ayrı ayrı) oluşturabilirsiniz. Daha fazla kasaya ihtiyacınız varsa yeni bir abonelik oluşturun.

## Her bir kasa için kaydedilebilen sunucu/makine sayısına yönelik herhangi bir sınır var mıdır? <br/>
...
## Sunucumu başka bir veri merkezine nasıl kaydederim?<br/>
Backup verileri, kayıtlı olduğu kasanın veri merkezine gönderilir. Veri merkezini değiştirmenin en kolay yolu, aracıyı kaldırmak ve aracıyı yeniden yükleyip istenilen veri merkezine ait yeni bir kasa kaydetmektir.

## Azure'a veri yedekleyen bir Windows sunucusunu yeniden adlandırırsam ne olur?<br/>
...
## Ne tür sürücülerden dosya ve klasör yedekleyebilirim? <br/>
...
## Sunucumdan hangi dosya ve klasör hangi türlerini yedekleyebilirim?<br/>
...
## Önbellek klasörü için minimum boyut gereksinimini nedir? <br/>
...
## Kuruluşumun bir kasası varsa veri geri yükleme sırasında bir sunucunun verilerini başka bir sunucudan nasıl ayırabilirim?<br/>
...
## Yedekleme verilerimin veya kasamın abonelikler arasında "geçişini" sağlayabilir miyim? <br/>
Hayır. Kasa abonelik düzeyinde oluşturulur ve oluşturulduktan sonra başka bir aboneliğe yeniden atanamaz.

## Azure Backup Aracısı, Windows Server 2012 yinelenenleri kaldırma özelliğini kullanan bir sunucu üzerinde çalışır mı? <br/>
...
## Bir yedekleme işini başlatıldıktan sonra iptal edersem aktarılan yedekleme verileri silinir mi? <br/>
...
## Daha önce düzenli yedeklemeler zamanlamış olmama karşın neden "Azure Yedeklemeleri bu sunucu için yapılandırılmamış" uyarısını görüyorum? <br/>
...
## Azure Backup için hangi güvenlik duvarı kuralları yapılandırılmalıdır? <br/>
...
##Azure Backup aracısını, önceden VM uzantısı kullanılarak Azure Backup hizmeti tarafından yedeklenmiş olan bir Azure VM üzerine yükleyebilir miyim? <br/>
...
## Azure Backup aracısını bir Azure VM'ye yükleyerek mevcut dosya ve klasörleri Azure VM tarafından sağlanan geçici depolama alanına yedekleyebilir miyim? <br/>
...
## Dosya ve klasörlerimi korumak için Azure Backup aracısını yükledim. Artık Azure'a yönelik şirket içi uygulama/VM iş yüklerini korumak için Azure Backup aracısıyla çalışmak üzere SCDPM'yi yükleyebilir miyim? <br/>
...
## Azure Backup aracısını kullanan Azure Yedekleme ilkesinin bir bölümü olarak belirtilebilecek dosya yolunun uzunluğu nedir? <br/>  
...
## Azure Backup aracısını kullanan Azure Yedekleme ilkesinin dosya yolunda hangi karakterlere izin verilir? <br>  
...
## Bir fiziksel sunucu için Tam Kurtarma (BMR) yedeklemesi oluşturmak üzere Azure Backup Sunucusu'nu kullanabilir miyim? <br/>
Evet.

## Bir yedekleme işi başarısız olursa posta göndermek için Backup hizmetini yapılandırabilir miyim? <br/>
...
## Yedeklenmekte olan her veri kaynağının boyutuna yönelik bir sınır var mıdır? <br/>
...
## Bir yedekleme işinin günde kaç kez zamanlanabileceğine yönelik sınırlar var mıdır?<br/>
...
## DPM ve Windows Server'a yönelik zamanlama ilkesi (DPM olmadan Windows Server'da) arasında bir fark var mıdır? <br/>
...
## DPM ve Windows Server'a/istemcisine yönelik bekletme ilkesi (DPM olmadan Windows Server'da) arasında bir fark var mıdır?<br/>
Hayır, hem DPM hem de Windows Server/istemcisi günlük, haftalık, aylık ve yıllık bekletme ilkelerine sahiptir.

## Bekletme ilkelerimi seçerek yapılandırabilir miyim? Başka bir deyişle, haftalık ve günlük yapılandırmaya olanak tanırken yıllık ve aylık yapılandırmayı engelleyebilir miyim?<br/>
...
## Saat 18:00 için "bir yedekleme zamanlayıp" farklı bir saat için de "bekletme ilkeleri" belirtebilir miyim?<br/>
...
## Zamanlanmış bekletme ilkeleri için bir artımlı kopya aktarılır mı? <br/>
...
## Yedekleme uzun süre tutulursa daha eski bir veri noktasının kurtarılması daha uzun mu sürer? <br/>
...
## Her kurtarma noktası bir tam nokta gibiyse bu durum toplam faturalanabilir yedekleme alanını etkiler mi?<br/>
...
## Oluşturulabilecek kurtarma noktalarının sayısına yönelik bir sınır var mıdır?<br/>
Hayır. Kurtarma noktalarına yönelik sınırları kaldırdık. İstediğiniz sayıda kurtarma noktası oluşturabilirsiniz.

## Yedeklemede aktarılan veri miktarı neden yedeklenen veri miktarına eşit değil?<br/>
...
## Backup hizmeti tarafından kullanılan bant genişliği miktarını ayarlamanın bir yolu var mıdır?<br/>
...
## İnternet bant genişliğim, yedeklemem gereken veri miktarı için sınırlı durumda. Verileri büyük bir ağ kanalı ile belirli bir konuma taşıyıp bu verileri Azure'a gönderebilmemin bir yolu var mıdır? <br/>
...
## Azure'a yedeklenen veriler üzerinde kaç kurtarma işlemi yapabilirim?<br/>
Azure Backup ile gerçekleştirilen kurtarma işlemlerinin sayısına yönelik bir sınır yoktur.

## Kurtarma sırasında Azure veri merkezi çıkış trafiği için ödeme yapmam gerekir mi?<br/>
 Hayır. Kurtarma işlemleriniz ücretsizdir ve çıkış trafiği için sizden ücret tahsil edilmez.

## Veriler Azure'a şifreli olarak mı gönderilir? <br/>
...
## Azure üzerindeki yedekleme verileri de şifreli midir?<br/>
 Evet. Azure'a gönderilen veriler (bekleyen) şifreli olarak kalır. Microsoft herhangi bir noktada yedekleme verilerinin şifresini çözmez. Azure VM yedeklemesi için Azure Backup sanal makinenin şifrelemesini kullanır; örneğin, sanal makineniz Azure Disk Şifrelemesi veya başka bir şifreleme teknolojisi kullanılarak şifrelenirse Azure Backup verilerinizin güvenliğini sağlamak için bu şifrelemeyi kullanır. 

## Yedekleme verilerini şifrelemek için kullanılan şifreleme anahtarının minimum uzunluğu nedir? <br/>
 Şifreleme anahtarı en az 16 karakterden oluşmalıdır.

## Şifreleme anahtarını kaybedersem ne olur? Verileri kurtarabilir miyim?/Microsoft, verileri kurtarabilir mi? <br/>
...
## Azure Backup aracısı için belirtilen önbellek konumunu nasıl değiştiririm?<br/>
...
## Azure Backup Aracısı'nın beklendiği şekilde çalışması için önbellek klasörünü nereye koyabilirim?<br/>
...
## Önbellek klasörünün desteklenmeyen herhangi bir özniteliği var mıdır?<br/>
