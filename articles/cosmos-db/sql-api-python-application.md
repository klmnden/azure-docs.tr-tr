...
Projeniz kurulduktan sonra pydocumentdb ve Azure Cosmos DB SQL API için Python paket dahil olmak üzere projenize gerekli Flask paketlerini eklemeniz gerekir.
...
1. Çözüm Gezgini'nde sağ **öğretici** proje, tıklatın **Ekle**, tıklatın **yeni öğe**seçin **boş Python dosyası**ve ardından ad Dosya **config_cosmos.py**. Bu yapılandırma dosyası, Flask'taki formlar için gereklidir. Bunu gizli bir anahtar sağlamak için de kullanabilirsiniz. Ancak bu anahtar bu öğretici için gerekli değildir.
2. Config_cosmos.py için aşağıdaki kodu ekleyin, değerlerini değiştirmeniz gerekecektir **COSMOSDB\_KONAK** ve **COSMOSDB\_anahtar** sonraki adımda.
...
4. Azure portalında yedekleme **anahtarları** sayfasında, değerini kopyalayın **birincil anahtar** veya **ikincil anahtar**ve yapıştırın **config_cosmos.py** değeri olarak dosya **COSMOSDB\_anahtar** özelliği.
5. İçinde  **\_ \_init\_\_.py** dosya, yapılandırma dosyasını okuma ve bazı temel günlük dahil etmek için aşağıdaki satırları ekleyin: 
...
6. İçinde **Yayımla** iletişim kutusunda **Microsoft Azure App Service**seçin **Yeni Oluştur**ve ardından **Yayımla**.
    ...
    ![Microsoft Azure App Service vurgulanmış Web'de Yayımla penceresinin ekran görüntüsü](./media/sql-api-python-application/cosmos-db-python-publish.png)
...
13. Sunucu ortamı tam olarak yapılandırdıktan sonra tarayıcıda sayfayı yenileyin ve web uygulaması görünmelidir.
    ...
    ![Uygulama hizmeti Bottle, Flask ve Django uygulamaları yayımlama sonuçları](./media/sql-api-python-application/python-published-app-services.png)
...
Web uygulamanıza ek işlevsellik eklemek için kullanılabilen API'leri inceleyin [Azure Cosmos DB Python SDK'sı](sql-api-sdk-python.md).
