...
Azure Cosmos DB, birden fazla veri modelini (anahtar-değer, belgeler, grafikler ve sütunlu) yerel olarak destekleyen, yazma bakımından iyileştirilmiş, kaynakları yönetilen, şemadan bağımsız bir veritabanı altyapısı içerir. Verilere erişmek için [MongoDB](../documentdb/documentdb-protocol-mongodb.md), [DocumentDB SQL](../documentdb/documentdb-introduction.md), [Gremlin](graph-introduction.md) (önizleme) ve [Azure Tabloları](table-introduction.md) (önizleme) dahil çok sayıda API’yi genişletilebilir biçimde destekler. 
...
* [**Aktarım hızı ve depolamayı dünyanın dört bir yanında, talep üzerine esnek bir şekilde ölçeklendirin**](#horizontal-scale)
    * Aktarım hızını [saniye](../documentdb/documentdb-request-units.md) ve [dakika](https://aka.ms/acdbrupm) ayrıntı düzeylerinde kolayca ölçeklendirin ve dilediğiniz zaman değiştirin. 
...
* [**"Her zaman açık" özelliği ile kullanılabilirlik endişeniz olmaz**](#high-availability)
    ...
    * Sıfır veri kaybı garantisi ile bir veya daha fazla bölgenin [hata benzetimini gerçekleştirin](../documentdb/documentdb-regional-failovers.md). 

* [**Genel olarak dağıtılan uygulamaları, doğru şekilde yazın**](#consistency)
    * [Beş tutarlılık modeli](../documentdb/documentdb-consistency-levels.md), NoSQL benzeri nihai tutarlılık, güçlü SQL benzeri tutarlılığı ve çok daha fazlasını sunar. 
...
Cosmos DB’de kaynakların genel dağıtımı [kullanıma hazırdır](../documentdb/documentdb-distribute-data-globally.md). Dilediğiniz zaman birkaç düğmeye tıklayarak (veya program aracılığıyla tek bir API çağrısı ile), dilediğiniz sayıda coğrafi bölgeyi veritabanı hesabınızla ilişkilendirebilirsiniz. 
...
<a id="data-models"></a>
## <a name="multi-model-multi-api-support"></a>Çoklu model, çoklu API desteği
...
![Çok modelli ve çok API’li Azure Cosmos DB](./media/introduction/azure-cosmos-db-multimodel.png) 
...
<a id="low-latency"></a>
## <a name="low-latency-guarantees-at-the-99th-percentile"></a>99. yüzdebirlik dilimde düşük gecikme süresi garanti edilir
...
## <a name="transparent-multi-homing-and-9999-high-availability"></a>Şeffaf çoklu giriş ve %99,99 yüksek kullanılabilirlik
...
<a id="consistency"></a>
## <a name="multiple-well-defined-consistency-models"></a>Birden çok, iyi tanımlanmış tutarlılık modeli
...
Cosmos DB, tutarlılık spektrumunda [iyi tanımlanmış beş tutarlılık modeli](../documentdb/documentdb-consistency-levels.md) arasından seçim yapmanıza olanak tanır: güçlü, sınırlanmış eskime durumu, [oturum](http://dl.acm.org/citation.cfm?id=383631), tutarlı ön ek ve nihai. 
...
<a id="sla"></a>
## <a name="guaranteed-service-level-agreements"></a>Garantili hizmet düzeyi sözleşmeleri
...
<a id="tco"></a>
## <a name="low-cost-of-ownership"></a>Düşük sahip olma maliyeti
