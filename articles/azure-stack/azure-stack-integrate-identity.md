...
Azure yığını, Azure Active Directory (Azure AD) veya Active Directory Federasyon Hizmetleri (AD FS) kullanarak kimlik sağlayıcıları olarak dağıtılabilir. Bu seçenek dağıtım öncesinde yapılması gerekir. AD FS kullanarak dağıtımı da bağlantısı kesilmiş modunda Azure yığın dağıtma olarak adlandırılır.
...
**Tetikleyici Otomasyon grafik yapılandırmak için**
...
2. (Yönetici olarak çalıştır) yükseltilmiş bir Windows PowerShell oturumu açın ve ayrıcalıklı uç noktası IP adresine bağlanın. Kimlik doğrulaması için kimlik bilgilerini CloudAdmin kullanın.
...
3. Ayrıcalıklı uç noktasına bağlı değilseniz, aşağıdaki komutları çalıştırın. İstendiğinde, Grafik Hizmeti (örneğin, graphservice) için kullanmak istediğiniz kullanıcı hesabının kimlik bilgilerini belirtin.
...
**Grafik protokoller ve bağlantı noktaları**
...
Bu yordam için Azure yığınında ayrıcalıklı uç noktası ile iletişim kurabilen bir bilgisayar kullanın. Hesap STS AD FS tarafından kullanılan sertifikanın Azure yığını tarafından güvenilir kabul edildiğinden emin beklenir.
...
SPN oluşturma hakkında daha fazla bilgi için bkz: [AD FS için hizmet sorumlusu oluşturma](https://docs.microsoft.com/en-us/azure/azure-stack/azure-stack-create-service-principals#create-service-principal-for-ad-fs).
...
2. Ardından aşağıdaki cmdlet'i çalıştırın:
   ...
   Geri alma eylemi çalıştırdıktan sonra tüm yapılandırma değişikliklerini geri alınır. Yalnızca kimlik doğrulaması yerleşik "CloudAdmin" kullanıcı ile mümkündür.
