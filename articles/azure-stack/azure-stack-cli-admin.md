...
2. Yeni bir depolama hesabı Azure yığınında oluşturun. Bu tamamlandığında, yeni blob kapsayıcı oluşturun. "Genel" için erişim ilkesi ayarlama  
3. JSON dosyasını yeni kapsayıcı karşıya yükleyin. Bu tamamlandığında, blob adı tıklatıp sonra URL blob özelliklerinden seçerek blob URL'si görüntüleyebilirsiniz.
