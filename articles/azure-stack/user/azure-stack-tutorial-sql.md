---
title: Azure Stack'te yüksek oranda kullanılabilir SQL veritabanı oluşturma | Microsoft Docs
...
ms.locfileid: "46369268"
---
# <a name="tutorial-create-highly-available-sql-databases"></a>Öğretici: yüksek oranda kullanılabilir SQL veritabanları oluşturma

Bir Azure Stack Kiracı kullanıcı olarak, SQL Server veritabanlarını barındırmak için server Vm'leri yapılandırabilirsiniz. Sonra bir SQL barındırma sunucusu başarıyla, oluşturulan ve Azure Stack tarafından yönetilen SQL veritabanları SQL hizmetlerine abone olan kullanıcılar kolayca oluşturabilirsiniz.
...
Bu öğreticide adımları uygulamaya başlamadan önce Azure Stack operatörü aşağıdaki öğeler Azure Stack marketini hazırlamıştır emin olun:
...
- [PowerShell Desired State Configuration (DSC)](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.DSC-arm) 2.76.0.0 sürümü veya üzeri. DSC, dağıtma ve yazılım hizmetleri için yapılandırma verilerini yönetme ve bu hizmetlerin çalıştırıldığı ortam yönetimi etkinleştiren bir Windows PowerShell içinde bir yönetim platformudur.
  ...
  > [!TIP]
  > Kullanıcı Portalı'ndan bir VM oluştururken, SQL Server Iaas uzantısı, özel betik uzantısı ya da PowerSHell DSC Market öğesi görmeniz mümkün olmayacaktır. Bu öğreticideki adımları başlamadan önce bu öğeler Azure'dan yüklendi emin olmak için Azure Stack operatörü başvurun.
...
> [!NOTE]
> Bu adımları Azure Stack Kullanıcı Portalı'ndan (işlem, ağ ve depolama hizmetleri) Iaas özellikleri sağlayan bir abonelikle bir kiracı kullanıcı olarak çalıştırın.
...
SQL AlwaysOn Kullanılabilirlik grubunun kaynak grubunun oluşturulduğu olan SQL yük dengeleyicinin genel IP daha önce kaydedilen için genel IP veya tam Azure Stack operatörü sunduğunuzdan emin olun (**SQLPIPsql\<kaynak Grup adı\>**). Ayrıca, işleç SQL Server AlwaysOn Kullanılabilirlik grubunda SQL örneklerine erişmek için kimlik doğrulama kimlik bilgileri kullanılan bilmeniz gerekir.

> [!NOTE]
> Bu adım, Azure Stack Yönetim Portalı'ndan Azure Stack operatör tarafından çalıştırılmalıdır.

SQL AlwaysOn Kullanılabilirlik grubunun yük dengeleyici dinleyicisini Kiracı Kullanıcı tarafından sağlanan genel IP ve SQL kimlik doğrulaması oturum açma bilgileri, bir Azure Stack operatörü artık [SQL AlwaysOn Kullanılabilirlik grubu kullanarak SQL barındırma sunucusu oluşturma ](.\.\azure-stack-sql-resource-provider-hosting-servers.md#provide-high-availability-using-sql-always-on-availability-groups). 

Ayrıca Azure Stack operatörü planları oluşturdu ve SQL AlwaysOn veritabanı oluşturma kullanıcılar için kullanılabilir hale getirmek sunar emin olun. İşleç eklemeniz gerekecektir **Microsoft.SqlAdapter** hizmeti bir plana ve yüksek oranda kullanılabilir veritabanları için özel bir yeni kota oluştur. Planları oluşturma hakkında daha fazla bilgi için bkz. [Plan, teklif, kota ve abonelik genel bakış](.\.\azure-stack-plan-offer-quota-overview.md).
...
3. Seçin **SKU** uygun SQL barındırma sunucusu kullanmak için SKU seçin. Bu örnekte, Azure Stack operatörü oluşturduğu **Kurumsal-HA** SQL AlwaysOn Kullanılabilirlik grupları için yüksek kullanılabilirliği desteklemek için SKU.
   ...
   ![SKU'ları seçin](./media/azure-stack-tutorial-sqlrp/createdb2.png)
