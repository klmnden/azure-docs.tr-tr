---
title: Edilgen Önbellekli düzeni kullanan Redis Cache ile Web Uygulaması oluşturmaya ilişkin öğretici | Microsoft Docs
description: Edilgen Önbellekli düzeni kullanan Redis Cache ile Web Uygulaması oluşturma hakkında bilgi edinin
...
Bu öğreticide, Azure Redis Cache ile [edilgen önbellekli düzeni](https://docs.microsoft.com/azure/architecture/patterns/cache-aside) kullanan bir puan tablosu dahil etmek için [Azure Redis Cache için ASP.NET hızlı başlangıcı](cache-web-app-howto.md) içinde oluşturulan *ContosoTeamStats* ASP.NET web uygulamasını güncelleştireceksiniz. Örnek uygulama, bir veritabanındaki takım istatistiklerinin listesini görüntüler ve performansı artırmak için önbellekten veri depolama ve almaya yönelik Azure Redis Cache’i kullanmak için farklı yollar gösterir. Öğreticiyi tamamladığınızda, Azure Redis Cache ile en iyi hale getirilmiş ve Azure’da barındırılan, bir veritabanını okuyan ve yazan çalışan bir web uygulamasına sahip olursunuz.

Bu öğreticide şunların nasıl yapıldığını öğreneceksiniz:

> [!div class="checklist"]
> * Azure Redis Cache’i kullanarak verileri depolayıp alarak veri aktarım hızını artırma ve veritabanı yükünü azaltma.
...
* Bu öğretici, [Azure Redis Cache için ASP.NET hızlı başlangıcı](cache-web-app-howto.md)’nda kaldığınız yerden devam eder. Henüz yapmadıysanız önce hızlı başlangıcı izleyin.
...
1. Visual Studio’da, [Azure Redis Cache için ASP.NET hızlı başlangıcı](cache-web-app-howto.md)’nda oluşturduğunuz *ContosoTeamStats* Çözümünü açın.
...
1. `Team` sınıfının tanımını, bazı diğer Entity Framework yardımcı sınıflarının yanı sıra güncelleştirilmiş `Team` sınıf tanımını içeren aşağıdaki kod parçacığı ile değiştirin. Bu öğreticide, Entity Framework ile code first yaklaşımı kullanılmaktadır. Bu yaklaşım, Entity Framework’ün kodunuzdan veritabanını oluşturmasını sağlar. Bu öğreticide kullanılan Entity Framework için ilk kod yaklaşımı hakkında daha fazla bilgi için, bkz. [Yeni bir veritabanına ilk kod](https://msdn.microsoft.com/data/jj193542).
...
1. `body` bölümünde, *Azure Redis Cache Testi* bağlantısının hemen altındaki *Contoso Team Stats* için şu yeni `Html.ActionLink` deyimini ekleyin.
...
## <a name="configure-the-app-for-redis-cache"></a>Redis Cache için uygulamayı yapılandırma
...
Bu testte uygulama ve veritabanının her ikisi de yerel olarak çalışmaktadır. Ancak Redis Önbelleği, Azure’da uzaktan barındırılır. Bu nedenle önbellek büyük olasılıkla veritabanı performansının biraz gerisinde kalır. En iyi performans için, istemci uygulaması ve Azure Redis Cache örneği aynı konumda olmalıdır. Sonraki bölümde, önbellek kullanımından elde edilen yüksek performansı görmek için tüm kaynakları Azure’a dağıtacaksınız.
...
> [Azure Redis Cache’i Ölçeklendirme](./cache-how-to-scale.md)