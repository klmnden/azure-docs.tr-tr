...
Log Analytics aracısını Log Analytics ağ geçidi ile iletişim kurmak ve izleme, performans veya olay verilerini analiz etmek için Log Analytics gateway için gereken hizmet uç noktaları tanımlamak sırayla çalıştıran bilgisayarda gereklidir.
...
Doğrudan aracılardan Azure Otomasyonu ve ağ geçidi sunucusu kullanarak Log Analytics'e veri akışı aşağıdaki diyagramda gösterilmektedir.  Aracıları, ağ geçidi Log Analytics hizmetiyle iletişim kurmak için yapılandırılmış aynı bağlantı noktası eşleşmesi kendi proxy yapılandırmasına sahip olmalıdır.  
...
* Windows için log Analytics aracısını 
...
1. Azure portalından indirin.  Sonra Azure portalında oturum açın:  
   ...
   1. İçinde **doğrudan aracı** dikey penceresinde tıklayın **indirme Log Analytics gateway**.<br><br> ![Log Analytics gateway indirin](./media/log-analytics-oms-gateway/download-gateway.png)
...
1. Kurulum tamamlandıktan sonra tıklayın **son**. Hizmeti services.msc ek bileşenini açarak çalışan ve doğrulayın doğrulayabilirsiniz **Log Analytics gateway** durumu ve Hizmetler listesinde görünür olan **çalıştıran**.<br><br> ![Hizmetleri – Log Analytics ağ geçidi](./media/log-analytics-oms-gateway/gateway-service.png)  
...
1. Microsoft izleme aracısının yüklü olduğu bir Log Analytics Ağ Geçidi sunucusuna bağlanmak için kümenin IP adresine sağ tıklayın ve ardından **konak kümesine Ekle**.<br><br> ![Ağ Yükü Dengeleme Yöneticisi – kümeye konak Ekle](./media/log-analytics-oms-gateway/nlb02.png)<br> 
...
Gereksinimler ve Windows bilgisayarları doğrudan Log Analytics'e bağlanması Log Analytics aracısını yüklemek adımları anlamak için bkz [bağlanmak Windows bilgisayarlarını Log Analytics'e](log-analytics-windows-agents.md) veya Linux bilgisayarlar için bkz. [ Linux bilgisayarlarını Log Analytics'e bağlama](log-analytics-quick-collect-linux-computer.md). Aracısı yapılandırılırken bir ara sunucu belirtmek yerine, bu değer, bağlantı noktası numarası ile Log Analytics ağ geçidi sunucusu ve IP adresi ile değiştirin.  Bir ağ yük dengeleyicinin arkasına birden çok ağ geçidi sunucusu dağıttıysanız, Log Analytics Aracısı Ara sunucu yapılandırmasını NLB sanal IP adresi ' dir.  
...
1. Seçin **Operations Management Suite erişimi için bir proxy sunucusunu kullanmak** ve Log Analytics ağ geçidi sunucusu IP adresini veya NLB sanal IP adresini yazın. İle başlatıldığından emin olmak `http://` önek.<br><br> ![Operations Manager – proxy sunucu adresi](./media/log-analytics-oms-gateway/scom02.png)<br> 
...
Ağ Geçidi tarafından günlüğe kaydedilen olayları toplamak için de Log Analytics aracısı yüklü olması gerekir.<br><br> ![Olay Görüntüleyicisi-Log Analytics ağ geçidi günlüğü](./media/log-analytics-oms-gateway/event-viewer.png)
...
![Günlük analizi gateway performans sayaçları](./media/log-analytics-oms-gateway/counters.png)
