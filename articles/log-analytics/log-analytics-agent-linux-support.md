...
* Gönderilen fikirleri ve hataları gözden geçirmek için Log Analytics geri bildirim sayfasını ziyaret edin [ http://aka.ms/opinsightsfeedback ](http://aka.ms/opinsightsfeedback) veya yeni bir dosya.  
...
## <a name="important-configuration-files"></a>Önemli yapılandırma dosyaları
 ...
 >[!NOTE]
 >Performans sayaçları ve Syslog için yapılandırma dosyalarını düzenleyerek yazılır gelen koleksiyon yapılandırıldıysa [veri menüsü Log Analytics Gelişmiş ayarlar](log-analytics-data-sources.md#configuring-data-sources) çalışma alanınız için Azure portalında. Log Analytics koleksiyonundan yapılandırması tüm aracılar için devre dışı bırakmak için devre dışı bırak **Gelişmiş ayarlar** veya tek bir aracı aşağıdaki komutu çalıştırın:  
...
| 4 | Geçersiz paket veya geçersiz proxy ayarları yazın. omsagent -*rpm*.sh paketler, yalnızca RPM tabanlı sistemler ve omsagent - yüklenebilir*deb*.sh paketleri Debian tabanlı sistemlerde yalnızca yüklenebilir. Bu Evrensel Yükleyicisi'nden kullanmanız önerilir [en son sürüm](log-analytics-quick-collect-linux-computer.md#install-the-agent-for-linux). Ayrıca [gözden](#issue:-unable-to-connect-through-proxy-to-log-analytics) proxy ayarlarınızı doğrulayın. |
...
### <a name="resolution"></a>Çözüm
* Syslog yapılandırmasını Log Analytics çalışma alanındaki tüm özellikleri ve doğru günlük düzeyleri olduğunu doğrulayın. Gözden geçirme [Syslog koleksiyonunu, Azure portalında yapılandırma](log-analytics-data-sources-syslog.md#configure-syslog-in-the-azure-portal)
...
2. Ardından aracıyı yüklemek [yönergeleri](log-analytics-quick-collect-linux-computer.md).
