---
title: Toplama ve performans sayaçları Azure Log analytics'te çözümleme | Microsoft Docs
...
Performans sayaçlarını Yapılandır [günlük analizi ayarları veri menüde](log-analytics-data-sources.md#configuring-data-sources).

Yeni bir Log Analytics çalışma alanı için Windows veya Linux performans sayaçlarını ilk kez yapılandırırken, birkaç ortak sayacı hızlı bir şekilde oluşturma seçenekleri sunulur.  Her birinin yanında bir onay kutusu görüntülenir.  İlk olarak oluşturmak istediğiniz herhangi bir sayaç denetlenir ve ardından olun **Seçili performans sayaçlarını Ekle**.
...
![Windows performans sayaçlarını Yapılandır](media/log-analytics-data-sources-performance-counters/configure-windows.png)
...
![Linux performans sayaçlarını Yapılandır](media/log-analytics-data-sources-performance-counters/configure-linux.png)
...
Aşağıdaki tabloda, nesneleri ve yapılandırma dosyasında belirttiğiniz sayaçları listeler.  Kullanılabilir ek sayaçları belirli uygulamalar için açıklandığı [Log analytics'te Linux uygulamaları için performans sayaçları toplamak](log-analytics-data-sources-linux-applications.md).
...
Log Analytics, sayaç yüklü olan tüm aracıları, belirtilen örnekleme aralığında tüm performans sayaçlarını toplar.  Veriler değil toplanır ve ham veriler, aboneliğinizi tarafından belirtilen süre için tüm günlük arama görünümlerde kullanılabilir.
...
## <a name="log-searches-with-performance-records"></a>Performans kayıtları ile günlük aramaları
Aşağıdaki tabloda farklı performans kayıtları almak günlük aramaları örnekleri sağlar.
...
| Perf &#124; nerede ObjectName "İşlemci" ve CounterName == "% işlemci zamanı" ve InstanceName == "_Toplam" == &#124; AVGCPU özetlemek avg(Average) bilgisayar tarafından = |Tüm bilgisayarlardaki ortalama CPU kullanımı |
| Perf &#124; CounterName burada "% işlemci zamanı" == &#124; Summarize aggregatedvalue = max(Max) bilgisayar tarafından |Tüm bilgisayarlardaki en fazla CPU kullanımı |
| Perf &#124; nerede ObjectName "LogicalDisk" ve CounterName == "Geçerli Disk Sırası Uzunluğu" ve bilgisayar == "MyComputerName" == &#124; Summarize aggregatedvalue = avg(Average) InstanceName tarafından |Belirli bir bilgisayarın tüm örneklerdeki ortalama geçerli Disk Sırası Uzunluğu |
| Perf &#124; CounterName burada "DiskTransfers/sn" == &#124; Summarize aggregatedvalue = bilgisayar tarafından yüzdebirlik (ortalama, 95) |95. yüzdebirlik, Disk aktarımı/sn tüm bilgisayarlardaki |
