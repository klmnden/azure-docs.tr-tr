...
ms.locfileid: "31519588"
---
# <a name="working-with-alert-rules-in-log-analytics"></a>Günlük analizi uyarı kurallarında ile çalışma

> [!NOTE]
> Bu makalede OMS portalında yönetilen Klasik günlük analizi uyarılar açıklanmaktadır.  Günlük analizi uyarıların [Azure'da genişletilen](../monitoring-and-diagnostics/monitoring-alerts-extend.md).  Bunu yaptıktan sonra ardından oluşturacak ve Azure portalında uyarı kuralları düzenlemek ve Not, bu makaledeki yordamları kullanın.


Düzenli aralıklarla otomatik olarak günlük aramaları çalıştıran uyarı kuralları tarafından uyarılar oluşturulur.  Sonuçları belirli ölçütlere uyan varsa bunlar bir uyarı kaydı oluşturun.  Daha sonra kural, uyarı hakkında proaktif olarak size bildirim göndermek veya başka bir işlem çağırmak için otomatik olarak bir ya da daha fazla eylem çalıştırabilir.   
...
>[!NOTE]
> En fazla 250 uyarı kuralları şu anda günlük analizi çalışma alanında oluşturulabilir. 

1. OMS genel bakış sayfasında **günlük arama**.
2. Yeni bir günlük arama sorgusu oluşturun ya da kaydedilmiş günlük Ara'yı seçin. 
3. Sayfanın üst kısmındaki **Uyarı**’ya tıklayarak **Uyarı Kuralı Ekle** ekranını açın.
4. Bilgileri kullanarak uyarı kuralı yapılandırmak [uyarı kuralları ayrıntılarını](#details-of-alert-rules) aşağıda.
6. **Kaydet**’e tıklayarak uyarı kuralını tamamlayın.  Çalıştırdıktan hemen başlar.
...
### <a name="generate-alert-based-on"></a>Şuna bağlı olarak uyarı oluştur:
...
| Özellik | Açıklama |
|:--- |:---|
| Toplam değer | Sonuçları toplama her değer bir ihlal olarak kabul edilmesi için aşması gereken Eşiği değeri. |
| Şuna bağlı olarak uyarıyı tetikle: | Oluşturulacak bir uyarı için ihlal sayısı.  Belirtebilirsiniz **toplam ihlal** ihlallerinden sonuçları arasında herhangi bir birleşimini ayarlamak veya **ardışık ihlallerini** ihlallerini ardışık örnekler içinde gerçekleşmelidir gerektirecek şekilde. |
