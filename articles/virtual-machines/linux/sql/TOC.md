# Genel Bakış
## [Linux SQL Server VM’leri hakkında](sql-server-linux-virtual-machines-overview.md)

# Hızlı Başlangıçlar
## [SQL VM Oluşturma - Portal](provision-sql-server-linux-virtual-machine.md)

# Başvuru
## [Azure CLI](/cli/azure/)
## [PowerShell](/powershell/azure/overview)
## [T-SQL](https://msdn.microsoft.com/library/azure/bb510741.aspx)
## [SQL Server Sürücüleri](https://msdn.microsoft.com/library/mt654049.aspx)
## [REST](/rest/api/)

# Kaynaklar
## [SSS](sql-server-linux-faq.md)
## [Linux üzerinde SQL Server Belgeleri](https://docs.microsoft.com/sql/linux/sql-server-linux-overview)
## [Fiyatlandırma](../../windows/sql/virtual-machines-windows-sql-server-pricing-guidance.md?toc=%2fazure%2fvirtual-machines%2flinux%2fsql%2ftoc.json)
## [PaaS mi IaaS mi?](../../../sql-database/sql-database-paas-vs-sql-server-iaas.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fsql%2ftoc.json)
## [SQL Server Veri Araçları (SSDT)](https://docs.microsoft.com/sql/ssdt/download-sql-server-data-tools-ssdt)
## [SQL Server Araçları](https://docs.microsoft.com/sql/tools/overview-sql-tools)
## [Azure Yol Haritası](https://azure.microsoft.com/roadmap/?category=compute)
## [Stack Overflow](https://stackoverflow.com/search?q=%5Bazure-virtual-machine%5D+sql+server)