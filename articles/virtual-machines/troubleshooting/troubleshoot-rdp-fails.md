---
title: RDP, azure'da bir lisans sağlanabilecek Uzak Masaüstü lisans sunucusu yok ise başarısız | Microsoft Docs
...
ms.locfileid: "49989239"
---
# <a name="rdp-fails-if-no-remote-desktop-license-server-is-available-in-azure"></a>Azure'da Uzak Masaüstü lisans sunucusu yok varsa RDP başarısız olur.

Bu makale, bir lisans sağlanabilecek Uzak Masaüstü lisans sunucusu yok olduğundan Azure sanal makinesi (VM) için bağlanamıyor sorunu yardımcı olur.

## <a name="symptoms"></a>Belirtiler

Bir VM'ye bağlanmaya çalıştığınızda, aşağıdaki senaryolarda karşılaşırsınız:

- Sanal makine (VM) ekran görüntüsü, işletim sistemini tam olarak yüklenir ve kimlik bilgileri bekleniyor olduğunu gösterir.
- Microsoft Uzak Masaüstü Protokolü (RDP) bağlantısı kurmaya çalışırken aşağıdaki hata iletileri alırsınız:
  ...
  - **Hiçbir Uzak Masaüstü lisans sunucusu kullanılabilir. Uzak Masaüstü Hizmetleri, bu bilgisayar, yetkisiz kullanım süresi ve geçerli en az bir Windows Server 2008 Lisans sunucusu iletişim kurmadığı çalışmayı durdurur. Lisans Tanılama kullanmak için RD Oturumu Ana Bilgisayar Sunucu Yapılandırması'nı açmak için bu iletiye tıklayın.**
...
Uzak oturumu başlatmak için lisans sağlamak bir Uzak Masaüstü lisans sunucusu kullanılamıyorsa, bu sorun oluşur. Uzak Masaüstü Oturumu Ana Bilgisayarı rol VM üzerinde ayarlanmış olsa bile bu sorunu çeşitli senaryolar tarafından kaynaklanabilir:

- Ortamda hiçbir zaman bir Uzak Masaüstü lisans rol vardı ve yetkisiz kullanım süresi (180 gün).
- Uzak Masaüstü lisans ortamda yüklenmiş, ancak hiçbir zaman etkinleştirilir.
- Bir Uzak Masaüstü Lisansı ortamında, istemci erişim lisansları (CAL) bağlantıyı ayarlamak için eklenmiş yok.
- Bir Uzak Masaüstü Lisansı ortama yüklenmiştir. CAL'ler mevcuttur vardır ancak düzgün yapılandırılmış vermediği.
...
1. Yönetici oturumu kullanarak VM'ye bağlanın:
  ...
  Yönetici oturumu kullanarak sanal Makineye bağlanamıyorsanız, kullanabileceğiniz [seri konsol](serial-console-windows.md) gibi sanal Makineye erişmek için:
...
2. VM'ye bir Uzak Masaüstü oturumu ana bilgisayarı rolü etkin olup olmadığını denetleyin. Rol etkinleştirilirse, düzgün çalıştığından emin olun. Yükseltilmiş bir komut örneği açın ve aşağıdaki adımları izleyin:
  ...
  2. İlkeleri denetlemek ve gerektiği şekilde yeniden yapılandırmak için aşağıdaki komutu kullanın:
    ...
    Varsa **LicensingMode** değeri 4 (kullanıcı başına)'den başka bir değere ayarlayın ve ardından 4'e ayarlayın:

    ```
    reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\Licensing Core" /v LicensingMode /t REG_DWORD /d 4
    ```

    Varsa **SpecifiedLicenseServers** değeri yok ya da bir yanlış lisans sunucusu bilgilerini içerir, aşağıdaki gibi değiştirin:
  ...
  4. CAL yoksa, Uzak Masaüstü oturumu ana bilgisayarı rolü kaldırın. Ardından, RDP geri normal olarak ayarlanır ve yalnızca VM için iki eş zamanlı RDP bağlantılarına izin verir.

    ```
    dism /ONLINE /Disable-feature /FeatureName:Remote-Desktop-Services
    ```

    VM'ye Uzak Masaüstü lisans rol varsa ve değil kullanılır, ayrıca bu rolü kaldırabilirsiniz.
...
3. Ortamında Uzak Masaüstü lisans sunucusu yok ve bir istiyorsanız yapabilecekleriniz [bir Uzak Masaüstü Lisansı rol hizmeti yükleme ](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc731765(v=ws.11))ve ardından [RDS lisansı yapılandırmak](https://blogs.technet.microsoft.com/askperf/2013/09/20/rd-licensing-configuration-on-windows-server-2012/).
