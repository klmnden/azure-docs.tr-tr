...
[Klasik Azure portalını](https://manage.windowsazure.com/) kullanıyorsanız SSH anahtarları için X.509 sertifikası .pem dosyasını kullanmanız gerekir.  Diğer SSH ortak anahtar türlerine izin verilmez, tümünün X.509 sertifikası *olması gerekir*.
...
Klasik dağıtım modelini kullanıyorsanız (CLI’daki `asm` modu) bir pem kapsayıcısındaki SSH-RSA ortak anahtarını veya RFC4716 biçimindeki anahtarı kullanabilirsiniz.  SSH-RSA ortak anahtarı, bu makalenin önceki bölümlerinde `ssh-keygen` kullanılarak oluşturulan anahtardır.
