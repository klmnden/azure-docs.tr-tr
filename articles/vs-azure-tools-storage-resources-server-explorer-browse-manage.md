...
3. Klavyenizi veya Select girin tıklatın veya blob kapsayıcısında kaydetmek için ad alanı dışında dokunun.
...
2. İçinde **yeni dosyasını karşıya yükle** iletişim kutusunda **Gözat** karşıya yüklemek istediğiniz dosyayı belirtmek üzere düğmesine tıklayın ve ardından bir klasör adı girin **(isteğe bağlı) klasör** kutusu.
...
3. Klasörü çift tıklatın veya Enter klasörünün içeriğini görmek için seçin. Kapsayıcının klasöründe olduğunuzda kapsayıcı köküne seçerek dönebilirsiniz **açık üst dizin** (okunu).

### <a name="to-delete-a-container-folder"></a>Bir kapsayıcı klasörü silmek için
* Klasördeki tüm dosyaları silin.
  
> [!NOTE]
> Blob kapsayıcıları klasörlerde sanal klasörlerdir için boş bir klasöre oluşturulamıyor. Ayrıca, dosya içeriğini silmek için bir klasör silinemez. Klasörünü silmek için bir klasörün tüm içeriğini silmeniz gerekir.
> 
> 
...
2. Seçin **Sorgu Oluşturucusu** Tablo görünümünün araç çubuğunda.
...
2. İçinde **depolama hesabı oluştur** iletişim kutusunda, seçin veya aşağıdaki bilgileri girin:
...
Kullanarak bir depolama hesabı oluşturduysanız **okuma erişimli coğrafi olarak yedekli** çoğaltma seçeneği, kendi ikincil uç noktaları görüntüleyebilirsiniz:

* Hesap adı için kısayol menüsünü açın ve ardından **özellikleri**.
