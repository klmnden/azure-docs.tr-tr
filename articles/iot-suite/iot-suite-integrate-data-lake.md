...
1. Azure portalında tıklatın **bulut Kabuk** düğmesi.
    ...
    ![Portal başlatma simgesi](media/iot-suite-integrate-data-lake/portal-launch-icon.png)
...
1. Tıklatın **kaynak oluşturma**, nesnelerin interneti marketten seçin ve tıklatın **Stream Analytics işi**.
    ...
    ![Yeni akış analizi işi](media/iot-suite-integrate-data-lake/new-stream-analytics-job.png)
...
1. **Oluştur**’a tıklayın.
    ...
    ![Akış analizi işi oluşturma](media/iot-suite-integrate-data-lake/create-stream-analytics-job.png)
...
1. Genel bakış sayfasında, tıklatın **girişleri**.
    ...
    ![Genel Bakış sayfası](media/iot-suite-integrate-data-lake/stream-analytics-overview.png)

1. Tıklatın **akış giriş Ekle** seçip **IOT hub'ı** açılan gelen.

    ![Giriş Ekle](media/iot-suite-integrate-data-lake/stream-analytics-add-input.png)
...
1. Tüketici grubu açılan listeden, daha önce oluşturduğunuz tüketici grubu seçin. Burada kullanıyoruz **streamanalyticsjob**.
    ...
    ![Giriş seçin](media/iot-suite-integrate-data-lake/stream-analytics-new-input.png)

1. **Kaydet**’e tıklayın.

1. Genel bakış sayfasında, tıklatın **çıkışları**.

    ![Data Lake Store ekleme](media/iot-suite-integrate-data-lake/stream-analytics-overview-2.png)

1. ' I tıklatın **Ekle** seçip **Data Lake Store** gelen açılır.

    ![Çıktı Ekle](media/iot-suite-integrate-data-lake/stream-analytics-output.png)
...
1. Tarih biçimi alanı girin **/streaming/ {date} / {time}**. Varsayılan tarih biçimi YYYY/AA/GG ve SS saat biçimi bırakın.
    ...
    ![Klasör yapısı sağlayın](media/iot-suite-integrate-data-lake/stream-analytics-new-output.png)
...
1. Tıklatın **yetkilendirmek**.
    ...
    ![Akış analizi Data Lake Store'a yetkilendirmek](media/iot-suite-integrate-data-lake/stream-analytics-out-authorize.png)
...
1. Genel sekmesinde, **düzenleme sorgu**.
    ...
    ![Sorguyu Düzenle](media/iot-suite-integrate-data-lake/stream-analytics-edit-query.png)
...
1. Sorgu Düzenleyicisi'nde [YourOutputAlias] değiştirin ve önceden tanımlanmış değerler [YourInputAlias] yer tutucularını.
    ...
    ![Stream Analytics sorgusu](media/iot-suite-integrate-data-lake/stream-analytics-query.png)
...
1. Genel sekmesinde, **Başlat**.
    ...
    ![Akış analizi işi Başlat](media/iot-suite-integrate-data-lake/stream-analytics-start.png)
...
1. Tıklatın **Başlat**.
    ...
    ![Özel tarih seçin](media/iot-suite-integrate-data-lake/stream-analytics-start-custom.png)
    ...
    ![İşi çalıştırma](media/iot-suite-integrate-data-lake/stream-analytics-running.png)
...
1. Veri Gezgini'ni detayına gitmek **/ akış** klasör. YYYY/AA/GG/HH biçimiyle oluşturulan klasörler görürsünüz.
    ...
    ![Veri akışı keşfedin](media/iot-suite-integrate-data-lake/data-lake-store-data-explorer.png)

    Saat başına bir dosyayla json dosyaları görürsünüz.

    ![Veri akışı keşfedin](media/iot-suite-integrate-data-lake/data-lake-store-file-preview.png)
