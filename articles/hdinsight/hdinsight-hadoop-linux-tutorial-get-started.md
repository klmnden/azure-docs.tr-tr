<properties
    pageTitle="Linux öğreticisi: Hadoop ve Hive kullanmaya başlama | Microsoft Azure"
...
HDInsight’ta Linux tabanlı [Hadoop](http://hadoop.apache.org/) kümeleri oluşturmayı ve HDInsight’ta Hive işleri çalıştırmayı öğrenin. [Apache Hive](https://hive.apache.org/) Hadoop ekosistemindeki en popüler bileşendir. HDInsight şu anda 4 farklı küme türü ile gelir: [Hadoop](hdinsight-hadoop-introduction), [Spark](hdinsight-apache-spark-overview.md), [HBase](hdinsight-hbase-overview.md) ve [Storm](hdinsight-storm-overview.md).  Her küme türü farklı bir bileşen kümesini destekler. 4 küme türü de Hive’ı destekler. HDInsight’ta desteklenen bileşenlerin listesi için bkz. [HDInsight tarafından sağlanan Hadoop küme sürümlerindeki yenilikler nelerdir?](hdinsight-component-versioning.md)  
...
Hadoop işlerinin çoğu toplu işlemdir. Bir küme oluşturur, bazı işleri çalıştırır ve kümeyi silersiniz. Bu bölümde, [Azure ARM şablonu](../resource-group-template-deploy.md) kullanarak HDInsight’ta Linux tabanlı Hadoop kümesi oluşturursunuz. ARM şablonları tamamen özelleştirilebilir.; HDInsight gibi Azure kaynaklarını oluşturmayı kolaylaştırır. Azure ARM şablonu deneyimi bu öğreticiyi izlemek için gerekli değildir. Diğer küme oluşturma yöntemleri ve bu öğreticide kullanılan özellikler hakkında bilgi edinmek için bkz. [HDInsight kümeleri oluşturma](hdinsight-hadoop-provision-linux-clusters.md). Bu öğreticide kullanılan ARM şablonu bir ortak blob kapsayıcısında bulunur: [https://hditutorialdata.blob.core.windows.net/armtemplates/create-linux-based-hadoop-cluster-in-hdinsight.json](https://hditutorialdata.blob.core.windows.net/armtemplates/create-linux-based-hadoop-cluster-in-hdinsight.json) 
...
2. **Parametreler** dikey penceresinde aşağıdakileri girin:
    ...
    ![HDInsight Linux kullanmaya başlama portalda arm şablonu](./media/hdinsight-hadoop-linux-tutorial-get-started/hdinsight-linux-get-started-arm-template-on-portal.png).
...
4. Sayfadaki __Sorgu Düzenleyicisi__ bölümünde, aşağıdaki HiveQL ifadelerini çalışma sayfasına yapıştırın:

        SHOW TABLES;

    >[AZURE.NOTE] Hive noktalı virgül gerektirmektedir.       
        
5. __Yürüt__’e tıklayın. Sorgu Düzenleyicisi’nin altında __Sorgu İşleminin Sonuçları__ bölümü görüntülenmeli ve iş hakkındaki bilgileri görüntülemelidir. 

    Sorgu tamamladığında, __Sorgu İşleminin Sonuçları__ bölümünde işlemin sonuçlarını görüntülenir. **hivesampletable** adlı bir tablo görürsünüz. Bu örnek Hive tablosu tüm HDInsight kümeleri ile birlikte gelir.
...
6. Aşağıdaki sorguyu çalıştırmak için 4. ve 5. adımı yineleyin:
    ...
    > [AZURE.TIP] __Sorgu İşleminin Sonuçları__ bölümünün sol üst kısmındaki __Sonuçları Kaydet__ açılır kutusuna dikkat edin; bunu sonuçları indirmek ya da CSV dosyası olarak HDInsight depolamaya kaydetmek için kullanabilirsiniz.
...
>[AZURE.NOTE] [Azure Data Factory ](hdinsight-hadoop-create-linux-clusters-adf.md)’yi kullanarak, istek üzerine HDInsight kümeleri oluşturabilir ve kümeleri otomatik olarak silmek için TimeToLive ayarını yapılandırabilirsiniz. 
...
Bu öğreticide, ARM şablonu kullanarak Linux tabanlı HDInsight kümesi oluşturmayı ve temel Hive sorguları gerçekleştirmeyi öğrendiniz.
