...
---
# <a name="choose-a-cloud-sql-server-option-azure-sql-paas-database-or-sql-server-on-azure-vms-iaas"></a>Bir bulut SQL Server seçeneği seçin: Azure SQL (PaaS) veritabanı ya da (Iaas) Azure vm'lerinde SQL Server
...
- [Azure SQL veritabanı](https://azure.microsoft.com/services/sql-database/): Enterprise Edition'ın SQL Server'da modern uygulama geliştirme için en iyi duruma getirilmiş, temel bir SQL veritabanı altyapısı. Azure SQL veritabanı dağıtım için çeşitli seçenekler sunar:
  ...
  - Dağıtabileceğiniz bir [Azure SQL veritabanı yönetilen örnekleri](sql-database-managed-instance.md).
...
- **Azure sanal makinelerinde (VM'ler) SQL Server**

Endüstri kategorisine denk gelen *-olarak-hizmet altyapı (Iaas)* ve buluttaki bir sanal makine içinde SQL Server'ı çalıştırmanıza olanak tanır. [SQL Server sanal makineleri](../virtual-machines/windows/sql/virtual-machines-windows-sql-server-iaas-overview.md) de, sahip olduğu, barındırdığı ve Microsoft tarafından yönetilen standartlaştırılmış donanım üzerinde çalıştırın. Bir VM'de SQL Server kullanırken ödeyebilir miyim-gibi bir SQL Server görüntüsüne zaten dahil bir SQL Server lisansı için Git ya da mevcut bir lisansı kolayca kullanabilirsiniz. Ayrıca, durdurabilir veya VM gerektiği gibi devam.
