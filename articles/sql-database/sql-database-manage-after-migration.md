...
Bu makalede, Azure SQL DB temel özelliklerinden bazıları kolayca yararlanabileceğiniz bir platform anlatılmaktadır. Bunlar şunlardır:- 
...
Ayrıca, [uzun süreli saklama (LTR)](sql-database-long-term-retention.md) özelliği, yedekleme dosyalarının çok daha uzun bir süre için 10 yıla kadar özellikle tutun ve belirtilen süre içinde herhangi bir noktada bu yedeklemelerinden veri geri olanak tanır. Ayrıca, veritabanı yedeklerinin coğrafi olarak çoğaltılmış bir depolama alanından bölgesel felaketler esnekliği sağlamak için tutulur. Ayrıca, herhangi bir Azure bölgesinde süre Bekletme dönemi içinde herhangi bir noktada bu yedekleri geri yükleyebilirsiniz. Bkz: [iş sürekliliğine genel bakış](sql-database-business-continuity.md).
...
Özet olarak, geleneksel şirket içi SQL Server Kurulumu, etkin bir şekilde yük devretme kümelemesi, veritabanı yansıtma, işlem çoğaltma, günlük aktarma vb. gibi özelliklerini kullanarak kullanılabilirlik kümenizi yönetmek ve korumak ve emin olmak için yedeklemeleri yönetmek gerekli İş sürekliliği. Geliştirme ve veritabanı uygulamanızı en iyi duruma getirme ve çok olağanüstü durum yönetimi konusunda endişe duymamanızı SQL veritabanı ile bunları, platform yönetir. Yedekleme ve olağanüstü durum kurtarma planları yapılandırılır ve yalnızca birkaç tıklamayla Azure portalında (veya PowerShell API'leri kullanarak bazı komutlar) ile çalışma olabilir. 
...
### <a name="what-user-authentication-methods-are-offered-in-sql-database"></a>Kullanıcı kimlik doğrulama yöntemleri, SQL veritabanı'nda sunulur?
...
### <a name="how-do-i-limit-or-control-connectivity-access-to-my-database"></a>Nasıl miyim sınırlamak denetlemek veya veritabanı bağlantı erişim?
...
**Saydam veri şifrelemesi (TDE)**: iki anahtar hiyerarşi içinde TDE – her bir kullanıcı veritabanındaki veriler sunucu benzersiz bir asimetrik RSA tarafından sırayla şifrelenir bir simetrik AES-256'yı veritabanı benzersiz bir veritabanı şifreleme anahtarı (DEK) tarafından şifrelenir 2048 ana anahtarı. Ana anahtarı olabilir ya da yönetilen:
...
Express Route 2 katına için ek ücret satın aldığınız bant genişliği sınırını veri bloğu sağlar. Expressroute kullanarak, bölge bağlantı yapılandırmak da mümkündür. ER bağlantı sağlayıcılarının bir listesini görmek için bkz: [Express Route iş ortakları ve eşleme konumları](../expressroute/expressroute-locations.md). Aşağıdaki makaleler Express Route daha ayrıntılı olarak açıklanmaktadır:
...
**Performans izleme ve iyileştirme**: ile sorgu performansı öngörüleri elde edebilirsiniz özel olarak uyarlanmış önerileri için veritabanı iş yükünüzü uygulamalarınızın çalışmaya devam en iyi düzeyde - her zaman böylece. Ayrıca, böylece bu önerileri otomatik olarak uygulandığından ve bakım görevlerini gerçekleştirme rahatsız gerekmez ayarlayabilirsiniz. Dizin Danışmanı ile dizin önerileri, iş yüküne göre otomatik olarak uygulayabilir: Bu otomatik ayarlama çağrılır. Uygulama iş yükü değişikliklerinizi en ilgili öneriler sağlamak için öneriler değiştikçe. Ayrıca el ile bu önerileri gözden geçirin ve bunları istediğiniz kadar uygulama seçeneğine sahip olursunuz.  

**Güvenlik en iyi duruma getirme**: SQL veritabanı, verilerinizi ve tehdit algılama tanımlama ve olası bir iş parçacığına oluşturabileceği şüpheli veritabanı etkinlikleri araştırma güvenliğini sağlamanıza yardımcı olması için eyleme dönüştürülebilir güvenlik önerileri sunar Veritabanı. [SQL güvenlik açığı değerlendirmesi](sql-vulnerability-assessment.md) tarama ve veritabanlarınızı uygun ölçekte güvenlik durumunu izleme ve güvenlik risklerini tanımlayın ve tanımladığınız güvenlik çizgisinden kayma olanak tanıyan bir hizmet raporlama bir veritabanıdır. Her bir taramadan sonra özelleştirilmiş bir liste eyleme dönüştürülebilir adımlar ve düzeltme betikleri sunulur yardımcı olmak için Uyumluluk gereklerini karşılamak için kullanılabilecek bir değerlendirme raporu yanı sıra.

Azure Güvenlik Merkezi ile güvenlik önerileri Pano genelinde tanımlamak ve bunları tek bir tıklamayla uygulayabilirsiniz. 

**En iyi duruma getirme maliyeti**: platform Azure SQL veritabanlarında değerlendirmek ve maliyeti en iyi duruma getirme seçenekleri önermek için sunucu kullanım geçmişi çözümler. Bu analiz, genellikle çözümleyin ve eyleme dönüştürülebilir öneriler oluşturmak için bir fortnight alır. Elastik havuz, böyle bir seçenek ' dir. Öneri Portalı'nda bir başlık görünür:
...
- **Azure portalında**: Azure portal, veritabanına seçtiğinizde ve grafik genel bakış bölmesinde tıklatarak tek veritabanlarının kullanımını gösterir. CPU yüzdesi, DTU yüzdesi, veri g/ç yüzdesi, oturumları yüzdesi ve veritabanı boyutunun yüzdesi de dahil olmak üzere birden çok ölçüm göstermek için grafiğin değiştirebilirsiniz.
...
- **Azure SQL Analytics (Önizleme) Log analytics'te**: [Azure Log Analytics](../azure-monitor/insights/azure-sql.md) destekleyen en fazla 150.000 SQL veritabanları ve başına 5.000 SQL elastik havuzları toplamanıza ve anahtar Azure SQL Azure performans ölçümleri görselleştirmenize olanak tanır Çalışma alanı. İzleme ve bildirimler almak için kullanabilirsiniz. Bir uygulama yığınının her katmanında sorunları belirlemek için kullanılabilir ve SQL veritabanı ve elastik havuz ölçümleri birden çok Azure abonelikleri ve elastik havuzlar arasında izleyebilirsiniz.
...
- **İçeri aktarma**: Ayrıca veri bir BACPAC dosyasını Azure portalını kullanarak veritabanına içeri aktarabilirsiniz.
...
- **[İşlem çoğaltma](https://azure.microsoft.com/blog/transactional-replication-to-azure-sql-database-is-now-generally-available/)**  – işlem çoğaltma ile şirket içi verilerinizi Azure SQL DB'ye yayımcısı ve abonesi olan Azure SQL DB ile şirket içi eşitleyebilirsiniz. Şu an için yalnızca bu kurulum desteklenir. Verilerinizin şirket içinden en az kapalı kalma süresi ile Azure SQL'e geçirme hakkında daha fazla bilgi için bkz: [işlem çoğaltma kullanma](sql-database-cloud-migrate.md#method-2-use-transactional-replication)
