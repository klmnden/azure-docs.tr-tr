...
> [!NOTE]
> Uzaktan izleme çözüm Hızlandırıcısını için Azure Time Series Insights şu anda Önizleme aşamasındadır ve yalnızca kullanılabilir [bölgeleri seçin](https://azure.microsoft.com/global-infrastructure/services/). Uzaktan izleme çözüm Hızlandırıcısını Bu bölgeler dışında dağıtıyorsanız, Cosmos DB varsayılan depolama seçenektir.
