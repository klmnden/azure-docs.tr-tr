...
-   Sunucu ve uygulama ana bilgisayar tek bir Active Directory etki alanında bulunur. Etki alanı ve orman senaryoları çapraz ilgili ayrıntılı bilgiler bulunabilir [KCD Teknik İnceleme](https://aka.ms/KCDPaper).
...
En basit senaryoda temsilci test etmeniz gerekir. Daha fazla değişken tanıtmak, daha fazla ile yüklüyorsa gerekebilir. Örneğin, tek bir bağlayıcıyı test sınırlama değerli zamandan tasarruf edebilirsiniz ve sorunları giderildikten sonra ek bağlayıcı eklenebilir.
...
Bağlayıcı ana bilgisayar ve etki alanı KDC arasında alışverişleri yakalama bir ağ izlemesi sonra konuları hakkında daha fazla alt düzey ayrıntı alma sonraki en iyi adım olacaktır. Daha fazla bilgi için bkz: [derinlemesine sorun giderme kağıt](https://aka.ms/proxytshootpaper).
...
Bu aşamada, bağlayıcı bir Kerberos Hizmet anahtarının arka uç için ilk uygulama isteği içinde bir başlık olarak gönderdiğiniz beklenir.
...
Bu değer değiştirdikten sonra **doğru**, önbelleğe alınan tüm Kerberos biletleri arka uç sunucusundan kaldırılması gerekir. Aşağıdaki komutu çalıştırarak bunu yapabilirsiniz:
