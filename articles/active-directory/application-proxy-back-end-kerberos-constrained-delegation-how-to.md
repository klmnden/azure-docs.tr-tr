---
title: "Kerberos kısıtlanmış temsili kullanmak üzere bir uygulama proxy'si uygulamasının nasıl yapılandırılacağını | Microsoft Docs"
description: "Bir Azure AD uygulama proxy'si uygulaması için Kerberos Kısıtlı temsilci yapılandırma yapma."
...
---
# <a name="how-to-configure-an-application-proxy-application-to-use-kerberos-constrained-delegation"></a>Kerberos kısıtlanmış temsili kullanmak üzere bir uygulama proxy'si uygulama yapılandırma

SSO yayımlanan uygulamalar için uygulama başka bir uygulama biraz değişebilir elde ve Azure uygulama ara kutusunu dışında sağ sunar seçeneklerden birini yöntemleri Kerberos Kısıtlı temsilci (KCD) olur. Bir bağlayıcı konak kullanıcılar adına arka uç uygulamaları için kısıtlı kerberos kimlik doğrulaması gerçekleştirmek üzere yapılandırıldığı budur.
...
Bu nedenle, bu makalede tek bir sorun giderme ve bazı biz bkz en yaygın sorunları otomatik olarak düzeltmek yardımcı olması başvuru noktası sağlamaya çalışır. Aynı anda daha karmaşık ve uygulama troubled tanılamaya yönelik ek yönergeler sunar.

Bu makalede aşağıdaki varsayımlar kullandığına dikkat edin:

-   Azure uygulama proxy'si dağıtılan göre bizim [belgelerine](https://docs.microsoft.com/azure/active-directory/active-directory-application-proxy-enable) ve genel olmayan KCD uygulamalara erişim beklendiği gibi çalıştığını.

-   Yayımlanan hedef uygulama IIS ve Microsoft'un kerberos üzerinde temel alır.

-   Sunucu ve uygulama ana bilgisayar tek bir Active Directory etki alanında bulunur. Etki alanı ve orman senaryoları çapraz ilgili ayrıntılı bilgiler bulunabilir bizim [KCD Teknik İnceleme](http://aka.ms/KCDPaper).

-   Konu olan uygulamanın yayımlanan bir Azure Kiracı ile ön kimlik doğrulama etkinleştirildi ve kullanıcılar için Azure kimlik doğrulaması form tabanlı kimlik doğrulama beklenir. Zengin istemci kimlik doğrulama senaryoları bu makalenin kapsamında değildir, ancak belirli bir noktada gelecekte eklenmesi.

## <a name="prerequisites"></a>Ön koşullar

Azure uygulama proxy'si neredeyse her tür altyapı veya ortam dağıtılabilir ve mimarileri Şüphesiz kuruluştan kuruluşa göre değişir. En yaygın nedenlerinden biri KCD sorunları ortamları kendileri ancak bunun yerine basit yanlış yapılandırmaları veya genel gözetim değil ilişkilidir.

Bu nedenle, bizim öneriler her zaman bizim ana düzenlendiği tüm ön koşul karşılanır sağlayarak başlamaktır [KCD SSO'su kullanarak uygulama proxy'si makaleyi](https://docs.microsoft.com/azure/active-directory/active-directory-application-proxy-sso-using-kcd) önce başlatma sorunlarını giderme.
...
-   Belirli bir etki alanı denetleyicisi ile bir güvenli kanal iletişim kutusunu açmak bir etki alanı üye sunucusu seyrek değil. Bağlayıcı konakları genellikle yalnızca belirli yerel site ile DC'leri iletişim geliştirebilmek için sınırlandırılmalıdır olmayan şekilde başka bir iletişim kutusu için belirli bir zamanda taşıyın.

-   Yerel ağ çevre dışında yer alabileceği DC'leri bağlayıcı konağa doğrudan başvuruları senaryoları Bel etki alanları arası yukarıdaki noktasına benzer. Eşit olduğu Bu senaryoda emin olmak önemli, ayrıca başlayarak ilgili diğer etki alanlarındaki temsil DC'leri trafiğine izin veya başka temsilci başarısız.
...
Sorunların hızla çözülmesine isteriz ve etkili bir şekilde zaman alabilir kadar bu nedenle, mümkün olduğunda deneyin ve gerekir temsilci en basit senaryoyu test. Daha fazla değişken tanıtmak, daha fazla ile yüklüyorsa gerekebilir. Örneğin, tek bir bağlayıcıyı test sınırlama değerli zamandan tasarruf edebilirsiniz ve sorunları giderildikten sonra ek bağlayıcı eklenebilir.

Olası çalışırsanız ve test etmek için tam minimum mimarisi en aza indirmek bazı çevresel etmenler ayrıca bir sorun için bu nedenle yeniden katkıda bulunan. Örneğin, yanlış yapılandırılmış iç güvenlik duvarı ACL'ler seyrek, böylece mümkünse düz aracılığıyla DC'leri ve arka uç uygulaması izin verilen bir bağlayıcı gelen tüm trafiği vardır. 
...
Bu nedenle, nelerin KCD sorun yine de oluşturduğunu? İyi var. bir sorunun ilk açar ve KCD başarısız SSO birkaç Klasik göstergeleri genellikle kendilerini tarayıcıda bildirimi.
...
Ardından sorun giderme nasıl sorun ve gözlemlenen Belirtiler bağlıdır. Henüz gelmiş olabilir üzerinden değil yararlı bilgiler içerdiğinden herhangi geçmeden önce daha fazla aşağıdaki bağlantılardan önerdiğimiz:
...
Ana sorunu kesinlikle şimdiye kadar bu ardından ekranınız varsa bulunmaktadır. İhtiyacınız olan derinliklerine, giderebileceğinizi üç ayrı aşamaları akışına ayırarak başlatın.

**İstemcisi ön kimlik doğrulaması** - bir tarayıcı üzerinden Azure için kimlik doğrulama dış kullanıcı.

Azure için ön kimlik doğrulaması için olan işleve KCD SSO için zorunludur. Bu test edilip herhangi bir sorun varsa, ilk olarak ele. Ön kimlik doğrulama aşama KCD veya yayımlanan uygulama hiçbir ilişki olduğuna dikkat edin. Oldukça kolay sağlamlık ile uyumsuzlukları düzeltmek konu hesap denetimi Azure'da var ve onu devre dışı bırakılmış/engellenmez emin olmalıdır. Tarayıcıda hata yanıtı nedenini anlamak için genellikle açıklayıcı yeterli. Ayrıca, diğer bizim sorunlarını giderme belgeleri emin değilseniz, doğrulamak için kontrol edebilirsiniz.

**Temsilci hizmet** - kerberos KDC (Kerberos Dağıtım Merkezi), hizmet biletindeki kullanıcılar adına alma Azure Ara sunucusu Bağlayıcısı.

Dış iletişimleri Azure ön uç ile istemci arasında şifrelemeyle KCD üzerinde doğabilecek, diğerinden sağlama çalıştığını olması gerekir. Azure Proxy Hizmeti geçerli bir kullanıcı kimliği ile sağlanabilir böylelikle bir kerberos anahtarı edinmek için kullanılır. Bu KCD mümkün değildir ve başarısız olacağı olmadan.

Daha önce belirtildiği gibi tarayıcı hata iletileri şey neden başarısız iyi bazı ipuçları genellikle sağlar. Bunun gibi davranış Azure Proxy olay günlüğünde gerçek olayları ilişkilendirmek için aşağı etkinlik kimliği ve zaman damgası yanıt unutmayın emin olun.
...
-   Bağlayıcı ana hedef hesabının SPN ve belirlenen temsilci hakları verilmiş yeniden onaylayın **herhangi bir kimlik doğrulama protokolünü kullan** seçilir. Bu bizim ana kapsamdaki [SSO yapılandırma makale](https://docs.microsoft.com/azure/active-directory/active-directory-application-proxy-sso-using-kcd)
...
-   Sınırlamak için bir etki alanı ilkesi zorlanmasını olmadığını denetleyin [en büyük boyut verilen kerberos belirteçlerin](https://blogs.technet.microsoft.com/askds/2012/09/12/maxtokensize-and-windows-8-and-windows-server-2012/)Bunu önlemek gibi bağlayıcı belirteci IF alma aşırı olmasını bulundu

Bağlayıcı ana bilgisayar ve etki alanı KDC arasında alışverişleri yakalama bir ağ izlemesi sonra daha düşük düzey ayrıntılı sorunları alma sonraki en iyi adım olacaktır. Bu konuda bulabilirsiniz [derinlemesine sorun giderme kağıt](https://aka.ms/proxytshootpaper).
...
Bu aşamada bağlayıcı bir kerberos hizmet bileti arka ucuna olarak üstbilgisi ilk uygulama isteği içinde gönderilen beklenir.

-   Uygulamanın İç URL Portalı'nda tanımlanan kullanarak, uygulama bağlayıcı ana bilgisayarda doğrudan tarayıcıdan erişilebilir olduğunu doğrulayın. Ardından, başarılı bir şekilde oturum açabilirsiniz. Bunun yapılması ile ilgili ayrıntılar connector sorunlarını giderme sayfasında bulunabilir.
...
Kerberos kullanılabilir durumda değilse, uygulamanın kimlik doğrulaması emin olmak için IIS ayarlarında anlaşma onay üstteki hemen altındaki NTLM ile listelenir. (Değil Anlaş: kerberos veya anlaşma: pku2u ile). Yalnızca Kerberos işlevsel ise devam edin.
...
-   Şimdi ön kimlik doğrulama Portalı'nda yeniden etkinleştirin ve onun dış URL aracılığıyla uygulamaya bağlanmak deneyerek Azure üzerinden kimlik doğrulaması. SSO başarısız olursa, tarayıcı yanı sıra 13022 olay günlüğünde olay Yasak hata iletisini görmeniz gerekir:
...
-   SPN karşı AD içinde aşağıda gösterildiği gibi IIS'de giderek yapılandırıldı aynı hesabı kullanmak üzere yapılandırılmış uygulama havuzunda sağlamak için IIS uygulama yapılandırılıp denetleyin
...
-   Hedef AD hesabı uygulamanın uygulama havuzunun kullandığı karşı yapılandırılmış aynı SPN portalındaki uygulamanın ayarları karşı SPN tanımlanan denetleyin.

   ![Azure Portalı'nda SPN yapılandırma](./media/application-proxy-back-end-kerberos-constrained-delegation-how-to/graphic11.png)
   
-   IIS ve select INTO gidin **yapılandırma Düzenleyicisi** seçeneği için uygulama ve gidin **system.webServer/security/authentication/windowsAuthentication** emin olmak için **UseAppPoolCredentials** ayarlanmış true
...
Hala sorununuzu ilerleme yapamıyorsanız, destek ve buradan devam yardımcı olmak üzere birden çok memnun olacaktır. Doğrudan portal içinde bir destek bileti oluşturun ve bizim mühendisleri size ulaşmak.

## <a name="other-scenarios"></a>Diğer senaryolar

-   Azure uygulama proxy'si, bir uygulamaya isteğini göndermeden önce bir Kerberos anahtarı ister. Tableau Server gibi bazı 3 taraf uygulamalar kimlik doğrulama yöntemi beğendiniz mi ve yerine gerçekleşmesi daha geleneksel anlaşmaları bekliyor. İlk istek anonim kimlik doğrulama türü ile bir 401 destekler yanıtlamak uygulama izin verme.
