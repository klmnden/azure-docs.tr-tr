...
Azure AD ile tümleştirilmiş uygulamalar oluşturmak, kuruluş çoklu oturum açmayı Office 365 ile kullanıcılara sağlar. Uygulamaya sahip olunması, üzerinde uygulama için kimlik doğrulama İlkesi denetim Azure AD'ye sağlar. Koşullu erişim ve çok faktörlü kimlik doğrulaması (MFA) bakın ile uygulamaları koruma hakkında daha fazla bilgi için [yapılandırma erişim kuralları](active-directory-conditional-access-azure-portal-get-started.md).
...
SaaS uygulamalarınız için uygulama başına erişim kurallarını yapılandırın. Örneğin, mfa'yı gerekli veya yalnızca güvenilen ağlarda kullanıcılar erişime izin vermek. Ayrıntılar için bu belgede kullanılabilir [yapılandırma erişim kuralları](active-directory-conditional-access-azure-portal-get-started.md).
