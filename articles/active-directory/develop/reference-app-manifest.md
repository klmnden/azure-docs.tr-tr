...
| `availableToOtherTenants` | Boolean | uygulamanın diğer kiracılarla paylaşılıyorsa true; Aksi takdirde false. <br><br> _Not: Bu değiştirilir `signInAudience` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | |
...
| `displayName` | String | Uygulama görünen adı. <br><br> _Not: Bu değiştirilir `name` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | `"MyRegisteredApp"` |
...
| `homepage` | String | Uygulamanın giriş sayfası URL'si. <br><br> _Not: Bu değiştirilir `signInUrl` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | `"https://MyRegisteredApp"` |
| `objectId` | String | Uygulama dizinindeki benzersiz tanımlayıcısı. <br><br> _Not: Bu değiştirilir `id` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | Bu uygulamanın genel bir istemci (örneğin, bir mobil cihazda çalışan yüklü bir uygulama) olup olmadığını belirtir. <br><br> _Not: Bu değiştirilir `allowPublicClient` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | |
| `publisherDomain` | String | Uygulama için yayımcı etki alanı. Salt okunur. | https://www.contoso.com |
| `replyUrls` | Dize dizisi | Bu çok değerli özellik belirteçleri döndürülürken hedefleri olarak Azure AD'ye kabul kayıtlı redirect_uri değerlerin listesini tutar. <br><br> _Not: Bu değiştirilir `replyUrlsWithType` içinde [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) karşılaşırsınız._ | |
...
Uygulama bildirimini Azure AD'de temel uygulama modeli şemasını temsil eder. Arka plandaki şema geliştikçe bildirim düzenleyicisini yeni şemayı zaman zaman yansıtacak şekilde güncelleştirilir. Sonuç olarak, uygulama bildiriminde gösteren yeni öznitelikler fark edebilirsiniz. Nadir durumlarda, varolan öznitelikleri bir söz dizimi veya anlam değişiklik fark edebilirsiniz veya önceden var olan bir öznitelik artık desteklenmiyor bulabilirsiniz. Örneğin, yeni öznitelikler göreceğiniz [uygulama kayıtları (Önizleme)](https://developer.microsoft.com/graph/blogs/new-app-registration/) olduğu bilinir uygulama kayıtları (GA) deneyimi içinde farklı bir ada sahip.


| Uygulama kayıtları (GA)    | Uygulama kayıtları (Önizleme) |
|---------------------------|-----------------------------|
...
1. İş akışınızı bildirimleri daha sonra kullanmak için kaynak deponuza kaydetmeye gerektiriyorsa, kaydedilmiş bildirimleri içinde gördüğünüz deponuzda bir yeniden Temellendirme öneririz **uygulama kayıtları (Önizleme)** karşılaşırsınız.
