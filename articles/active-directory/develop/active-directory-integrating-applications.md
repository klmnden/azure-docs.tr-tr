...
3. Sol gezinti bölmesinden **Azure Active Directory** hizmeti ye **uygulama kayıtları**, tıklatıp **yeni uygulama kaydı**.
   ...
   ![Yeni bir uygulamayı kaydetme](./media/active-directory-integrating-applications/add-app-registration.png)
...
4. Zaman **Oluştur** sayfası görüntülenirse, uygulamanızın kayıt bilgilerini girin: 
  ...
  - **Yeniden yönlendirme URI'si:** "Yerel" uygulamaları için Azure AD'nin belirteç yanıtlarını döndürmek için kullanılan URI girin. Uygulamanıza özgü bir değer girin, örneğin `http://MyFirstAADApp`
   ...
   ![Yeni bir uygulamayı kaydetme - oluşturma](./media/active-directory-integrating-applications/add-app-registration-create.png)
...
Onay çerçevesine OAuth2.0 yetkilendirme vermeleri ile kullanma hakkında daha fazla bilgi için bkz. [OAuth 2.0 ve Azure AD kullanarak web uygulamalarına erişim yetkisi verme](active-directory-protocols-oauth-code.md) ve [AzureADiçinkimlikdoğrulamasenaryoları](active-directory-authentication-scenarios.md). Microsoft Graph aracılığıyla Office 365 için yetkili erişim sağlama hakkında daha fazla bilgi için bkz: [Microsoft Graph ile uygulama kimlik doğrulamasını](https://graph.microsoft.io/docs/authorization/auth_overview).
...
6. Bir yönetici olarak, aynı zamanda tüm kullanıcılar adına uygulamanın temsilci izinleri için kiracınızda onay verebilir. Yönetici onayı onay iletişim kutusunu kiracıdaki her kullanıcı için görüntülenmesini engeller ve yapılabilir [Azure portalında](https://portal.azure.com) Yönetici rolüne sahip kullanıcılar tarafından. Gelen **ayarları** sayfasında uygulamanız için **gerekli izinler** tıklayın **izinler** düğmesi. 
  ...
  ![Açık yönetici onayı için izin ver](./media/active-directory-integrating-applications/grantpermissions.png)
...
3. Sol gezinti bölmesinden **Azure Active Directory** hizmeti ye **uygulama kayıtları**, sonra bulma/tıklatın yapılandırmak istediğiniz uygulama.
   ...
   ![Bir uygulamanın kaydı güncelleştir](./media/active-directory-integrating-applications/update-app-registration.png)
...
4. Açılan uygulamanın ana kayıt sayfasına yönlendirilirsiniz **ayarları** uygulama sayfası. Web uygulamanızın kimlik bilgileri için gizli bir anahtar eklemek için:
  ...
  ![Uygulamanın kayıt - anahtarları güncelleştirme](./media/active-directory-integrating-applications/update-app-registration-settings-keys.png)
...
5. Kaynak API'leri istemcinizden erişmek için izinler eklemek için
  ...
  ![Uygulamanın kayıt - izinlerini güncelleştirme API'si](./media/active-directory-integrating-applications/update-app-registration-settings-permissions-api.png)

  ![Uygulamanın kayıt - izinleri izinleri güncelleştir](./media/active-directory-integrating-applications/update-app-registration-settings-permissions-perms.png)
...
6. İşiniz bittiğinde tıklayın **Kaydet**. Artık web API'nizi dizininizdeki başka uygulamalar tarafından kullanılmak üzere yapılandırılmıştır. 
  ...
  ![Bir uygulamanın kaydı güncelleştir](./media/active-directory-integrating-applications/update-app-registration-manifest.png)
...
3. Üzerinde **erişimini etkinleştir** istemci izin istekleri için kullanılabilir yeni kapsam gördüğünüz sayfası.
  ...
  ![Yeni izinleri gösterilir](./media/active-directory-integrating-applications/update-app-registration-settings-permissions-perms-newscopes.png)
...
- Azure AD'de kimlik doğrulaması birlikte nasıl çalıştığı hakkında daha fazla bilgi için bkz. [Azure AD için kimlik doğrulama senaryoları](active-directory-authentication-scenarios.md).
