...
JavaScript tabanlı yaklaşımı, üst düzey için uygulamaları tek sayfalık uygulamalar veya Spa'lar çağrılır: söz konusu uygulamaların yalnızca bir ilk HTML sayfası ve ilişkili JavaScript ile Web API'sı tarafından yönetilen tüm etkileşiminde hizmet emin olur çağrıları JavaScript aracılığıyla gerçekleştirilir. Ancak, karma yaklaşımları burada uygulama çoğunlukla geri gönderme temelli ancak bazen JS çağrıları gerçekleştirir, seyrek olmayan – örtük akış kullanımıyla ilgili tartışma de olanlar için geçerlidir.
