...
> [!IMPORTANT]
> Kullanan ortak istemciler [OAuth 2.0 örtük grant](active-directory-dev-understanding-oauth2-implicit-grant.md) OBO akışı kullanamazsınız. Bu istemciler kendi erişim belirteci OBO akışları gerçekleştirmek için Orta katmanda gizli istemciye geçmesi gerekir. Hangi istemcilerin OBO çağrıları gerçekleştirebilir daha fazla bilgi için bkz: [istemci kısıtlamaları](#client-limitations).
...
Joker karakter yanıt URL'leri ortak istemcileriyle kullanamaz bir `id_token` OBO akışlar için. Ancak, gizli bir istemci erişim belirteçleri ortak istemci bir joker karakter yeniden yönlendirme URI'si kayıtlı olsa bile örtük grant aktığı edinilen hala kullanmak.
