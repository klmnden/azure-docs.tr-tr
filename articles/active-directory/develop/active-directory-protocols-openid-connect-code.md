...
Web uygulamanız kullanıcı kimlik doğrulaması gerektiğinde, kullanıcıya yönlendirmelisiniz `/authorize` uç noktası. Bu istek için ilk oluşturan benzer [OAuth 2.0 yetkilendirme kod akış](active-directory-protocols-oauth-code.md), bazı önemli farklılıklar ile:
...
Doğrulandıktan sonra `id_token`, kullanıcı bir oturumla başlar ve talepleri kullanan `id_token` uygulamanızda kullanıcı hakkında bilgi edinmek için. Bu bilgiler kullanılabilir görüntüleme, kayıtları, yetkilendirmeleri, vb. için. Talepler ve belirteç türleri hakkında daha fazla bilgi için okuma [desteklenen belirteç ve talep türleri](active-directory-token-and-claims.md).
