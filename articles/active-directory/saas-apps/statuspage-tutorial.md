---
title: 'Öğretici: Azure Active Directory Tümleştirmesi ile StatusPage | Microsoft Docs'
...
* Otomatik olarak (çoklu oturum açma) StatusPage için kendi Azure AD hesapları ile oturum açmış, kullanıcıların etkinleştirebilirsiniz.
...
* StatusPage destekler **IDP** tarafından başlatılan
...
4. Arama kutusuna **StatusPage**seçin **StatusPage** sonucu panelinden ardından **Ekle** uygulama eklemek için Ekle düğmesine.

    ![Sonuç listesinde StatusPage](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>Yapılandırma ve Azure AD çoklu oturum açmayı test etme

Bu bölümde, yapılandırma ve Azure AD çoklu oturum açma StatusPage adlı bir test kullanıcı tabanlı test **Britta Simon**.
Tek iş için oturum açma için bir Azure AD kullanıcısının StatusPage ilgili kullanıcı arasında bir bağlantı ilişkisi kurulması gerekir.
...
5. **[StatusPage test kullanıcısı oluşturma](#create-statuspage-test-user)**  - kullanıcı Azure AD gösterimini bağlı StatusPage Britta simon'un bir karşılığı vardır.
...
1. İçinde [Azure portalında](https://portal.azure.com/), **StatusPage** uygulama tümleştirme sayfasında **çoklu oturum açma**.
...
6. Üzerinde **StatusPage kümesi** bölümünde, ihtiyacınıza göre uygun URL'lerini kopyalayın.
...
### <a name="configure-statuspage-single-sign-on"></a>StatusPage tek oturum açmayı yapılandırın
...
1. SSO Kurulumu sayfasında, aşağıdaki adımları gerçekleştirin:
    ...
    a. İçinde **SSO hedef URL** metin değerini yapıştırın **oturum açma URL'si**, hangi Azure Portalı'ndan kopyaladığınız.
...
1. Azure portalında **kurumsal uygulamalar**seçin **tüm uygulamaları**, ardından **StatusPage**.

    ![Kurumsal uygulamalar dikey penceresi](common/enterprise-applications.png)

2. Uygulamalar listesinde **StatusPage**.

    ![Uygulamalar listesinde StatusPage bağlantı](common/all-applications.png)
...
### <a name="create-statuspage-test-user"></a>StatusPage test kullanıcısı oluşturma

Bu bölümün amacı StatusPage Britta Simon adlı bir kullanıcı oluşturmaktır.

Tam zamanında sağlama StatusPage destekler. İçinde zaten etkinleştirdiyseniz [yapılandırma Azure AD çoklu oturum açma](#configure-azure-ad-single-sign-on).
...
1. Tür **e-posta adresi**, **ad**, ve **Soyadı** istediğiniz ilgili metin kutularına zbilgisayarlar geçerli bir kullanıcı. 
...
Erişim paneli StatusPage kutucuğa tıkladığınızda, size otomatik olarak SSO'yu ayarlama StatusPage için oturum açmanız. Erişim paneli hakkında daha fazla bilgi için bkz: [erişim Paneli'ne giriş](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
