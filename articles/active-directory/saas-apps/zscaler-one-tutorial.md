---
title: 'Öğretici: Azure Active Directory tümleştirmesiyle Zscaler bir | Microsoft Docs'
...
ms.locfileid: "64694432"
---
# <a name="tutorial-azure-active-directory-integration-with-zscaler-one"></a>Öğretici: Zscaler bir ile Azure Active Directory Tümleştirme
...
* Zscaler bir erişimi olan Azure AD'de kontrol edebilirsiniz.
* Azure AD hesaplarına otomatik olarak Zscaler One (çoklu oturum açma) oturum açmış, kullanıcıların etkinleştirebilirsiniz.
...
* Zscaler bir destekler **SP** tarafından başlatılan

* Zscaler bir destekler **zamanında** kullanıcı sağlama
...
Bu bölümde, yapılandırma ve Azure AD çoklu oturum açma Zscaler adlı bir test kullanıcı tabanlı bir test **Britta Simon**.
Tek iş için oturum açma için bir Azure AD kullanıcısı ile ilgili kullanıcı Zscaler bir arasında bir bağlantı ilişki kurulması gerekir.
...
2. **[Zscaler bir çoklu oturum açmayı yapılandırma](#configure-zscaler-one-single-sign-on)**  - uygulama tarafında çoklu oturum açma ayarlarını yapılandırmak için.
3. **[Bir Azure AD test kullanıcısı oluşturma](#create-an-azure-ad-test-user)**  - Azure AD çoklu oturum açma Britta Simon ile test etmek için.
4. **[Azure AD test kullanıcı atama](#assign-the-azure-ad-test-user)**  - Azure AD çoklu oturum açmayı kullanmak Britta Simon etkinleştirmek için.
5. **[Zscaler bir test kullanıcısı oluşturma](#create-zscaler-one-test-user)**  - Zscaler kullanıcı Azure AD gösterimini bağlantılı bir Britta simon'un bir karşılığı vardır.
...
1. İçinde [Azure portalında](https://portal.azure.com/), **Zscaler bir** uygulama tümleştirme sayfasında **çoklu oturum açma**.
...
4. Üzerinde **temel SAML yapılandırma** bölümünde, aşağıdaki adımları gerçekleştirin:
    ...
    ![Zscaler bir etki alanı ve URL'ler tek oturum açma bilgileri](common/sp-signonurl.png)

    İçinde **oturum açma URL'si** metin kutusu, türü URL kullanıcılarınız oturum açmaya Zscaler bir uygulamanıza tarafından kullanılıyor.

    > [!NOTE]
    > Değerin gerçek oturum açma URL'si ile güncelleştirin. İlgili kişi [Zscaler bir istemci Destek ekibine](https://www.zscaler.com/company/contact) değeri alınamıyor. Gösterilen desenleri de başvurabilirsiniz **temel SAML yapılandırma** bölümünde Azure portalında.

5. Zscaler bir uygulamanız SAML onaylamalarını özel öznitelik eşlemelerini SAML belirteci öznitelikleri yapılandırmanıza ekleyin gerektiren belirli bir biçimde bekliyor. Aşağıdaki ekran görüntüsünde, varsayılan öznitelikler listesinde gösterilmiştir. Tıklayın **Düzenle** açmak için simgeyi **kullanıcı öznitelikleri** iletişim.

    ![image](common/edit-attribute.png)

6. Yukarıdaki için ayrıca Zscaler bir uygulama SAML yanıtta geçirilecek birkaç daha fazla öznitelik bekliyor. İçinde **kullanıcı taleplerini** bölümünde **kullanıcı öznitelikleri** iletişim kutusunda gösterildiği gibi SAML belirteci özniteliği eklemek için aşağıdaki adımları gerçekleştirin tablonun altındaki:
...
8. Üzerinde **Zscaler bir kümesi** bölümünde, ihtiyacınıza göre uygun URL'lerini kopyalayın.
...
### <a name="configure-zscaler-one-single-sign-on"></a>Zscaler bir çoklu oturum açmayı yapılandırın

1. Zscaler bir içinde yapılandırmasını otomatik hale getirmenizi yüklemeniz gerekir **My Apps güvenli oturum açma tarayıcı uzantısı** tıklayarak **uzantıyı yükleme**.

    ![Uygulamaları uzantım](common/install-myappssecure-extension.png)

2. Uzantı tarayıcıya ekledikten sonra tıklayarak **Kurulum Zscaler bir** Zscaler tek bir uygulama için yönlendirir. Burada, Zscaler bir oturum açmak için yönetici kimlik bilgilerini sağlayın. Tarayıcı uzantısı otomatik olarak sizin için uygulamayı yapılandırma ve 3-6 adımları otomatik hale getirin.

    ![Kurulum sso](common/setup-sso.png)

3. Zscaler bir el ile ayarlamak istiyorsanız, yeni bir web tarayıcı penceresi ve oturum şirket sitenizin Zscaler bir yönetici olarak oturum açın ve aşağıdaki adımları gerçekleştirin:
...
5. Üzerinde **Düzenle SAML** penceresinde aşağıdaki adımları gerçekleştirin: ve Kaydet'e tıklayın.  
    ...
    ![Kullanıcı ve kimlik doğrulaması yönetmek](./media/zscaler-one-tutorial/ic800208.png "kullanıcı ve kimlik doğrulaması'nı yönetme")
...
5. Proxy sunucusu bölümünde aşağıdaki adımları gerçekleştirin:   
    ...
    b. Adresi metin kutusuna yazın **ağ geçidi. Zscaler One.net**.
...
7. Tıklayın **Tamam** kapatmak için **Internet Seçenekleri** iletişim.
...
1. Azure portalında **kurumsal uygulamalar**seçin **tüm uygulamaları**, ardından **Zscaler bir**.
...
Bu bölümde, Britta Simon adlı bir kullanıcı bir Zscaler içinde oluşturulur. Zscaler bir tam zamanında kullanıcı hazırlama, varsayılan olarak etkin olduğu destekler. Bu bölümde, hiçbir eylem öğesini yoktur. Bir kullanıcı zaten Zscaler bir mevcut değilse yeni bir kimlik doğrulamasından sonra oluşturulur.

>[!Note]
>Bir kullanıcı el ile oluşturmanız gerekiyorsa, kişi [Zscaler bir Destek ekibine](https://www.zscaler.com/company/contact).

### <a name="test-single-sign-on"></a>Çoklu oturum açma testi 

Bu bölümde, erişim panelini kullanarak Azure AD çoklu oturum açma yapılandırmanızı test edin.

Erişim paneli Zscaler bir kutucuğa tıkladığınızda, size otomatik olarak Zscaler SSO'yu ayarlama bir oturum açmanız. Erişim paneli hakkında daha fazla bilgi için bkz: [erişim Paneli'ne giriş](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
