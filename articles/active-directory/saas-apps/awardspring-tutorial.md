...
3. **[Bir AwardSpring test kullanıcısı oluşturma](#create-an-awardspring-test-user)**  - kullanıcı Azure AD gösterimini bağlı AwardSpring Britta simon'un bir karşılığı vardır.
...
3. Üzerinde **AwardSpring etki alanı ve URL'ler** bölümünde, uygulamada yapılandırmak istiyorsanız aşağıdaki adımları gerçekleştirin **IDP** başlatılan modu:
...
5. AwardSpring uygulama belirli bir biçimde SAML onaylamalarını bekler. Bu uygulama için aşağıdaki talepleri yapılandırın. Bu öznitelikleri değerlerini yönetebilirsiniz **kullanıcı öznitelikleri** uygulama tümleştirme sayfasında bölümü. Aşağıdaki ekran görüntüsü bunun bir örneği gösterilmektedir.
...
9. Çoklu oturum açmayı yapılandırma **AwardSpring** tarafı, indirilen göndermek için ihtiyacınız **meta veri XML** için [AwardSpring Destek ekibine](mailto:support@awardspring.com). Bunlar, her iki kenarı da düzgün ayarlandığından SAML SSO bağlantı sağlamak için bu ayarı ayarlayın.
