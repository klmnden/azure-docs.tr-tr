...
* Otomatik olarak (çoklu oturum açma) PolicyStat için kendi Azure AD hesapları ile oturum açmış, kullanıcıların etkinleştirebilirsiniz.
...
* PolicyStat destekler **SP** tarafından başlatılan

* PolicyStat destekler **zamanında** kullanıcı sağlama
...
4. Arama kutusuna **PolicyStat**seçin **PolicyStat** sonucu panelinden ardından **Ekle** uygulama eklemek için Ekle düğmesine.

     ![Sonuç listesinde PolicyStat](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>Yapılandırma ve Azure AD çoklu oturum açmayı test etme

Bu bölümde, yapılandırma ve Azure AD çoklu oturum açma PolicyStat adlı bir test kullanıcı tabanlı test **Britta Simon**.
Tek iş için oturum açma için bir Azure AD kullanıcısının PolicyStat ilgili kullanıcı arasında bir bağlantı ilişkisi kurulması gerekir.
...
5. **[PolicyStat test kullanıcısı oluşturma](#create-policystat-test-user)**  - kullanıcı Azure AD gösterimini bağlı PolicyStat Britta simon'un bir karşılığı vardır.
...
1. İçinde [Azure portalında](https://portal.azure.com/), **PolicyStat** uygulama tümleştirme sayfasında **çoklu oturum açma**.
...
4. Üzerinde **temel SAML yapılandırma** bölümünde, aşağıdaki adımları gerçekleştirin:
    ...
    a. İçinde **oturum açma URL'si** metin kutusuna şu biçimi kullanarak bir URL yazın: `https://<companyname>.policystat.com`

    b. İçinde **tanımlayıcı (varlık kimliği)** metin kutusuna şu biçimi kullanarak bir URL yazın: `https://<companyname>.policystat.com/saml2/metadata/`

    > [!NOTE]
    > Bu değerler gerçek değildir. Bu değerler gerçek oturum açma URL ve tanımlayıcıdır ile güncelleştirin. İlgili kişi [PolicyStat istemci Destek ekibine](http://www.policystat.com/support/) bu değerleri almak için. Gösterilen desenleri de başvurabilirsiniz **temel SAML yapılandırma** bölümünde Azure portalında.
...
5. PolicyStat uygulamanız SAML onaylamalarını özel öznitelik eşlemelerini SAML belirteci öznitelikleri yapılandırmanıza ekleyin gerektiren belirli bir biçimde bekliyor. Aşağıdaki ekran görüntüsünde, varsayılan öznitelikler listesinde gösterilmiştir. Tıklayın **Düzenle** açmak için simgeyi **kullanıcı öznitelikleri** iletişim.

    ![image](common/edit-attribute.png)

6. Yukarıdaki için ayrıca PolicyStat uygulama SAML yanıtta geçirilecek birkaç daha fazla öznitelik bekliyor. İçinde **kullanıcı taleplerini** bölümünde **kullanıcı öznitelikleri** iletişim kutusunda gösterildiği gibi SAML belirteci özniteliği eklemek için aşağıdaki adımları gerçekleştirin tablonun altındaki:
    ...
    d. Kaynağı olarak **dönüştürme**.

    e. Gelen **dönüştürme** listesinde, ilgili satır için gösterilen öznitelik değeri yazın.
    
    f. Gelen **parametresi 1** listesinde, ilgili satır için gösterilen öznitelik değeri yazın.

    g. **Kaydet**’e tıklayın.

7. Üzerinde **PolicyStat kümesi** bölümünde, ihtiyacınıza göre uygun URL'lerini kopyalayın.
...
### <a name="configure-policystat-single-sign-on"></a>PolicyStat tek oturum açmayı yapılandırın

1. Farklı bir web tarayıcı penceresinde PolicyStat şirket sitenize yönetici olarak oturum.

2. Tıklayın **yönetici** sekmesine ve ardından **çoklu oturum açma yapılandırması** sol gezinti bölmesinde.
   
    ![Yönetici menü](./media/policystat-tutorial/ic808633.png "yönetici menüsü")

3. İçinde **Kurulum** bölümünden **etkinleştirme tek oturum açma tümleştirmesi**.
   
    ![Çoklu oturum açma yapılandırması](./media/policystat-tutorial/ic808634.png "çoklu oturum açma yapılandırması")

4. Tıklayın **öznitelikleri yapılandırma**ve ardından **öznitelikleri yapılandırma** bölümünde, aşağıdaki adımları gerçekleştirin:
...
5. Tıklayın **bilgisayarınızı IDP meta verileri**ve ardından **bilgisayarınızı IDP meta verileri** bölümünde, aşağıdaki adımları gerçekleştirin:
...
1. Azure portalında **kurumsal uygulamalar**seçin **tüm uygulamaları**, ardından **PolicyStat**.

    ![Kurumsal uygulamalar dikey penceresi](common/enterprise-applications.png)

2. Uygulamalar listesinde **PolicyStat**.

    ![Uygulamalar listesinde PolicyStat bağlantı](common/all-applications.png)
...
Bu bölümde, Britta Simon adlı bir kullanıcı PolicyStat oluşturulur. PolicyStat just-ın-time kullanıcı hazırlama, varsayılan olarak etkin olduğu destekler. Bu bölümde, hiçbir eylem öğesini yoktur. Bir kullanıcı PolicyStat içinde zaten mevcut değilse yeni bir kimlik doğrulamasından sonra oluşturulur.
...
Erişim paneli PolicyStat kutucuğa tıkladığınızda, size otomatik olarak SSO'yu ayarlama PolicyStat için oturum açmanız. Erişim paneli hakkında daha fazla bilgi için bkz: [erişim Paneli'ne giriş](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
