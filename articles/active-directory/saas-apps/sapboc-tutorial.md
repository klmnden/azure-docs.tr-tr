...
5. Sonuçlar panelinde seçin **SAP Business nesnesi bulut**ve ardından **Ekle**.
    ...
    ![SAP Business nesnesi bulut sonuçlar listesinde](./media/sapboc-tutorial/tutorial_sapboc_addfromgallery.png)
...
2. Üzerinde **çoklu oturum açma** sayfası için **modu**seçin **SAML tabanlı oturum açma**.
    ...
    ![SAML tabanlı oturum açma seçin](./media/sapboc-tutorial/tutorial_sapboc_samlbase.png)
...
3. Altında **SAP Business nesnesi bulut etki alanı ve URL'leri**, aşağıdaki adımları tamamlayın:
    ...
    ![SAP Business nesnesi bulut etki alanı ve URL'leri sayfa URL'leri](./media/sapboc-tutorial/tutorial_sapboc_url.png)
 
    > [!NOTE] 
    > Bu URL'leri yalnızca tanıtım değerler. Değerlerini tanımlayıcı URL'sini ve gerçek oturum açma URL'si ile güncelleştirin. Oturum açma URL'si almak için başvurun [SAP Business nesnesi bulut istemci destek ekibi](https://help.sap.com/viewer/product/SAP_BusinessObjects_Cloud/release/en-US). Yönetici konsolundan SAP Business nesnesi bulut meta verileri yükleyerek tanımlayıcı URL'sini alabilirsiniz. Bu öğreticinin ilerleyen bölümlerinde açıklanmıştır. 
...
7. Seçin **menü** > **sistem** > **Yönetim**.
    ...
    ![Menüsünü, sonra sistem ve yönetim seçin](./media/sapboc-tutorial/config1.png)

8. Üzerinde **güvenlik** sekmesine **Düzenle** (Kalem) simgesi.
    
    ![Güvenlik sekmesinde Düzenle simgesine seçin](./media/sapboc-tutorial/config2.png)  

9. İçin **kimlik doğrulama yöntemini**seçin **SAML çoklu oturum açma (SSO)**.

    ![SAML çoklu oturum açma kimlik doğrulama yöntemini seçin](./media/sapboc-tutorial/config3.png)  
...
11. Hizmet sağlayıcısı meta verileri (2. adım) altında Azure portalından indirdiğiniz dosyayı karşıya yükleme için **karşıya kimlik sağlayıcısı meta verileri**seçin **karşıya**.  
    ...
    ![Karşıya yükleme karşıya kimlik sağlayıcısı meta verileri altında seçin](./media/sapboc-tutorial/config5.png)
...
13. Kimlik sağlayıcısı (4. adım), hesabı doğrulamak için **oturum açma kimlik bilgileri (e-posta)** kutusuna, kullanıcının e-posta adresi girin. Ardından, seçin **hesabı doğrula**. Sistem kullanıcı hesabı ile oturum açma kimlik bilgilerini ekler.
    ...
    ![E-posta girin ve Doğrula hesabı seçin](./media/sapboc-tutorial/config7.png)
...
2. Seçin **menü** > **güvenlik** > **kullanıcılar**.
    ...
    ![Çalışanı ekleyin](./media/sapboc-tutorial/user1.png)

3. Üzerinde **kullanıcılar** seçin sayfasında, yeni kullanıcı ayrıntıları eklemek için **+**. 

    ![Kullanıcılar Sayfası Ekle](./media/sapboc-tutorial/user4.png)
