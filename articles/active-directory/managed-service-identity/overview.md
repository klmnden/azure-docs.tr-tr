---
title: Azure kaynakları için Yönetilen Hizmet Kimliği
description: Azure kaynakları için Yönetilen Hizmet Kimliği özelliğine genel bakış.
...
Kodunuzda Azure AD kimlik doğrulamasını destekleyen hizmetler için erişim belirteci istemek üzere yönetilen hizmet kimliği kullanılabilir. Hizmet örneği tarafından kullanılan kimlik bilgilerinin dağıtımıyla Azure ilgilenir.
...
### <a name="how-a-system-assigned-identity-works-with-an-azure-vm"></a>Sistem tarafından atanmış kimliğin Azure VM ile çalışma şekli
...
3. Azure Resource Manager sanal makinede kimliği yapılandırır:
    ...
    1. VM uzantısı sağlar ve hizmet sorumlusu istemci kimliğiyle sertifikasını ekler. (Bu adımın kullanımdan kaldırılması planlanmaktadır.)
...
5. Sanal makine üzerinde çalıştırılan kodunuz, yalnızca sanal makinenin içinden erişilebilen iki uç noktadan belirteç isteyebilir:
    ...
    - VM uzantısı uç noktası (kullanımdan kaldırılması planlanmaktadır): `http://localhost:50342/oauth2/token` 
        - Resource parametresi belirtecin gönderildiği hizmeti belirtir. Azure Resource Manager ile kimlik doğrulaması için `resource=https://management.azure.com/` kullanın.
...
### <a name="how-a-user-assigned-identity-works-with-an-azure-vm"></a>Kullanıcı tarafından atanmış kimliğin Azure VM ile çalışma şekli

1. Azure Resource Manager kullanıcı tarafından atanan bir kimlik oluşturma isteği alır.
2. Azure Resource Manager, Azure AD'de kullanıcı tarafından atanan kimlik için bir hizmet sorumlusu oluşturur. Hizmet sorumlusu, bu abonelik tarafından güvenilen Azure AD kiracısında oluşturulur.
3. Azure Resource Manager sanal makinede kullanıcı tarafından atanan kimliği yapılandırmak için bir istek alır:
    1. Azure Instance Metadata Service kimliği uç noktasını kullanıcı tarafından atanan kimlik hizmet sorumlusu istemci kimliği ve sertifikasıyla güncelleştirir.
    1. VM uzantısını sağlar ve kullanıcı tarafından atanan kimlik hizmet sorumlusu istemci kimliğiyle sertifikasını ekler. (Bu adımın kullanımdan kaldırılması planlanmaktadır.)
4. Kullanıcı tarafından atanan kimlik oluşturulduktan sonra hizmet sorumlusu bilgilerini kullanarak Azure kaynaklarına erişim verir. Azure Resource Manager'ı çağırmak için Azure AD'de RBAC özelliğini kullanarak kullanıcı tarafından atanan kimliğin hizmet sorumlusuna uygun rolü atayabilirsiniz. Key Vault'u çağırmak için kodunuza Key Vault'ta belirli bir gizli diziye veya anahtara erişim verebilirsiniz.
...
## <a name="how-can-i-use-managed-service-identities"></a>Yönetilen hizmet kimliklerini nasıl kullanabilirim?

Yönetilen hizmet kimliklerini farklı Azure kaynaklarına erişme amacıyla kullanmayı öğrenmek için aşağıdaki öğreticileri deneyin.

Yönetilen hizmet kimliklerini Windows VM ile kullanmayı öğrenin:
...
Yönetilen hizmet kimliklerini Linux VM ile kullanmayı öğrenin:
...
Yönetilen hizmet kimliklerini diğer Azure hizmetleri ile kullanmayı öğrenin:
...
## Bu özelliği hangi Azure hizmetleri destekliyor?<a name="which-azure-services-support-managed-service-identity"></a>

Yönetilen hizmet kimlikleri, Azure AD kimlik doğrulamasını destekleyen hizmetlerde kimlik doğrulaması yapmak için kullanılabilir. Yönetilen Hizmet Kimliği özelliğini destekleyen Azure hizmetlerinin listesi için bkz. [Yönetilen Hizmet Kimliği’ni destekleyen hizmetler](services-support-msi.md).

## <a name="next-steps"></a>Sonraki adımlar

Yönetilen Hizmet Kimliği özelliğini kullanmaya başlamak için aşağıdaki hızlı başlangıçlardan yararlanın:

* [Resource Manager'a erişmek için Windows VM yönetilen hizmet kimliğini kullanma](tutorial-windows-vm-access-arm.md)
* [Resource Manager'a erişmek için Linux VM yönetilen hizmet kimliğini kullanma](tutorial-linux-vm-access-arm.md)
