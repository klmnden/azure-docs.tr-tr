...
- **Etkin olmayan hazırda bekleme ile Aktif-Pasif**: oluncaya kadar yük devretme için bir gereksinim yük devretme bu çözümde, VM'ler ve diğer bekleme bölgede çalışan gereçlere giden etkin değil. Ancak, üretim ortamına biçiminde yedeklemeler, VM görüntüleri veya Resource Manager şablonları, farklı bir bölgeye çoğaltılır. Bu yük devretme mekanizması, uygun maliyetli ancak tam bir yük devretme gerçekleştirmek için çok uzun sürüyor.
...
- **Etkin bekleme ile Aktif/Pasif**: Bu yük devretme çözümü bekleme bölge önceden warmed ve temel yük almaya hazır, otomatik ölçeklendirme açık ve çalışır duruma tüm örnekleri şunlardır. Bu çözüm tam üretim yük gerçekleştirilecek ölçeklenmez ancak işlevsel ve tüm hizmetleri ve çalışıyor. Bu çözüm, pilot ışık yaklaşım genişletilmiş bir sürümüdür.
...
-   Her iki birincil ve ikincil site için bir Azure DNS bölgesi yok. Örneğin, birincil site için uç nokta prod.contoso.com ve yedekleme sitesi için dr.contoso.com şeklindedir. Ayrıca, www.contoso.com bilinen ana uygulama için bir DNS kaydı yok.   
-   TTL veya kuruluştaki ayarlamak RTO SLA altında ' dir. Örneğin bir kuruluş daha iyi RTO, 60 dakika, tercihen alt'den küçük olan uygulama olağanüstü durum yanıt TTL olmalıdır, 60 dakika olarak ayarlar. El ile yük devretme için Azure DNS'yi gibi ayarlayabilirsiniz:
1. DNS bölgesi oluşturma
2. DNS bölgesi kayıtları oluşturma
...
Bu bölge içinde üç kayıt (örneğin - www.contoso.com, prod.contoso.com ve dr.consoto.com) aşağıda gösterildiği gibi oluşturun.
...
### <a name="step-3-update-the-cname-record"></a>3. adım: CNAME kaydını güncelleştir
...
30 dakika içinde önbelleğe alınan bölge dosyasını yeniler Çözümleyicileri çoğu sırasında www.contoso.com için herhangi bir sorgu için dr.contoso.com yönlendirilirsiniz.
...
### <a name="step-2-create-endpoints-within-the-traffic-manager-profile"></a>2. adım: Traffic Manager profilini içindeki uç noktalar oluşturma
...
Bu adımda, DNS TTL'yi 10 saniyeye, çoğu internet'e yönelik özyinelemeli çözümleyiciler tarafından kabul ayarlayın. Bu yapılandırma hiçbir DNS Çözümleyicisi 10 saniyeden fazla bilgilerini önbelleğe alacağı anlamına gelir. Uç Nokta İzleyicisi ayarları için geçerli kümesinin yoludur / veya kök, ancak bir yolu, örneğin, prod.contoso.com/index değerlendirmek için uç nokta ayarlarını özelleştirebilirsiniz. Gösterir aşağıdaki örnekte **https** araştırma protokolü olarak. Ancak, seçebileceğiniz **http** veya **tcp** de. Protokol seçimi son uygulama gereksinimlerinize bağlıdır. Yoklama aralığı 10 saniye için hızlı yoklama sağlayan ayarlanır ve yeniden 3 olarak ayarlayın. Sonuç olarak, Traffic Manager yapmayacağınıza ikinci uç nokta yük devretme üç ardışık aralıkları hata kaydedin. Aşağıdaki formülü bir otomatik yük devretme için toplam süreyi tanımlar: yük devretme için zaman TTL = + yeniden deneyin * Probing aralığı ve bu durumda, değer 10 + 3 * 10 = 40 saniye (en fazla).
