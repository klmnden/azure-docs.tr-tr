---
title: Bir Azure SaaS uygulaması doğrulamak için istemci kendi kendine test | Microsoft Docs
description: Bir Azure SaaS uygulamasına önceden doğrulamak için bir kendi kendine test istemcisi oluşturma
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Bir Azure SaaS uygulaması doğrulamak için bir kendi kendine test istemcisi oluşturma
...
|          DNSName          |         DNS adı VM'nin test oluşturacağız.          |
|          Kullanıcı          |          Bağlanmak ve sanal makineye oturum açma için kullanıcı adı.         |
|           Parola         |         Bağlanmak ve VM'de oturum açmak için parola.          |
|          İşletim Sistemi          |         Sanal makinenin, işletim sistemi "Linux" veya "Windows".          |
|         PortNo           |         VM'ye bağlanmak için açık bağlantı noktası numarası. Genellikle "22" Linux ve Windows için "5986" için bağlantı noktası numarasıdır.          |
...
Aşağıdaki ekran görüntüsü yakalamayı çevrimiçi bir JSON Görüntüleyicisi'nde görüntülemesini JSON test sonuçlarının bir örneği gösterilmektedir.
...
CURL ile API çağrısı için şu adımları izleyin:
...
3. Altında **Oluştur**, aşağıdaki alanlar için gerekli bilgileri sağlayın:
   ...
   - **Oturum açma URL'si** – türü "https://isvapp.azurewebsites.net/selftest"
...
20. Anahtar'ı seçerek bir gizli anahtar oluşturma **açıklama** metin. Aşağıdaki alanları yapılandırın:
    ...
    - Altında **değer**, anahtarı kopyalayın.
     ...
     >[!Important]
     >Anahtarları formun çıktıktan sonra anahtar değeri görmek mümkün olmayacaktır.
...
Azure SaaS uygulamanız başarıyla test ettikten sonra [teklifi yayımlama](./cpp-publish-offer.md)