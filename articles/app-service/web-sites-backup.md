...
Aşağıdaki veritabanı çözümleri ile yedekleme özelliği desteklenir: 
   ...
   - [MySQL (Önizleme) için Azure veritabanı](https://azure.microsoft.com/services/mysql)
   - [(Önizleme) PostgreSQL için Azure veritabanı](https://azure.microsoft.com/services/postgresql)
...
* [Haftalık yedeklemeler ayarlayabilir](web-sites-backup.md#configure-automated-backups) uygulamanızın statik içeriği olan hiçbir zaman, eski blog gönderilerini veya görüntüleri gibi değiştirir.
...
Karşıya yükleme `_backup.filter` dosyasını `D:\home\site\wwwroot\` kullanarak site dizininde [ftp](app-service-deploy-ftp.md) veya başka bir yöntem. İsterseniz, doğrudan Kudu kullanarak bu dosyayı oluşturabilirsiniz `DebugConsole` ve orada içeriği ekleyin.
