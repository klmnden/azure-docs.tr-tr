---
title: "Yay önyükleme uygulamayı Azure'a dağıtmak için Azure Web uygulamaları için Maven eklentisi kullanma"
description: "Azure Web uygulamaları için Maven eklentisi yay önyükleme uygulamayı Azure'a dağıtmak için nasıl kullanılacağını öğrenin."
...
---
# <a name="how-to-use-the-maven-plugin-for-azure-web-apps-to-deploy-a-spring-boot-app-to-azure"></a>Yay önyükleme uygulamayı Azure'a dağıtmak için Azure Web uygulamaları için Maven eklentisi kullanma

[Azure Web uygulamaları için Maven eklentisi](https://github.com/Microsoft/azure-maven-plugins/tree/master/azure-webapp-maven-plugin) için [Apache Maven](http://maven.apache.org/) Maven projelerine Azure App Service'in sorunsuz tümleştirme sağlar ve geliştiricilerin web uygulamaları Azure App Service'e dağıtma işlemini kolaylaştırır.

Bu makalede, Azure uygulama hizmetleri için örnek bir yay önyükleme uygulama dağıtmak için Azure Web uygulamaları için Maven eklentisi kullanma gösterilmektedir.
...
## <a name="clone-the-sample-spring-boot-web-app"></a>Örnek yay önyükleme web uygulaması kopyalama
...
1. Kopya [yay önyükleme Başlarken] örnek proje dizine oluşturduğunuz; örneğin:
   ```shell
...
Bu bölümde, bir Azure web uygulamanızı Azure'a dağıtırken Maven eklentisi kullanan hizmet sorumlusu oluşturun.
...
## <a name="optional-customize-your-pomxml-before-deploying-your-web-app-to-azure"></a>İsteğe bağlı:, pom.xml web uygulamanızı Azure'a dağıtmadan önce özelleştirme
...
`<appName>` | Web uygulamanız için bir hedef adı belirtir. Bu örnekte, hedef addır `maven-web-app-${maven.build.timestamp}`, burada `${maven.build.timestamp}` soneki çakışmayı önlemek için bu örnekte eklenir. (Zaman damgasını isteğe bağlıdır; uygulama adı için benzersiz bir dize belirtin.)
...
`<javaVersion>` | Web uygulamanız için Java Çalışma zamanı sürümü belirler. (Tam bir liste olarak [Azure Web uygulamaları için Maven eklentisi] belgelerine.)
...
`<resources>` | Kaynakları ve web uygulamanızı Azure'a dağıtırken Maven kullanan hedef konumları belirtir. Bu örnekte, iki `<resource>` öğelerini belirtmek Maven web uygulamanızın JAR dosyasını dağıtacağınız ve *web.config* yay önyükleme proje dosyasından.

## <a name="build-and-deploy-your-web-app-to-azure"></a>Derleme ve web uygulamanızı Azure'a dağıtma

Tüm ayarlar, bu makalenin önceki bölümlerinde yapılandırdıktan sonra web uygulamanızı Azure'a dağıtmak hazır olursunuz. Bunu yapmak için aşağıdaki adımları kullanın:
...
* [Kapsayıcılı yay önyükleme uygulamayı Azure'a dağıtmak için Azure Web uygulamaları için Maven eklentisi kullanma](containers/app-service-web-deploy-containerized-spring-boot-app-with-maven-plugin.md)
