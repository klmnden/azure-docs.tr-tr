...
Bu makalede bir ZIP dosyası veya WAR dosyasını web uygulamanıza dağıtmak için nasıl kullanılacağını gösteren [Azure App Service](app-service-web-overview.md). 
...
HTTP temel kimlik doğrulaması için App Service dağıtım kimlik bilgileriniz gerekir. Dağıtım kimlik bilgilerinizi ayarlamak hakkında bilgi için bkz. [ayarlayın ve kullanıcı düzeyi kimlik bilgilerini sıfırlama](app-service-deployment-credentials.md#userscope).
