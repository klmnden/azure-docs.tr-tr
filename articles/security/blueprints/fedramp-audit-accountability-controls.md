...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı denetim özelliği Azure İzleyicisi'ni ve günlük analizi hizmeti tarafından sağlanır. Azure İzleyicisi dağıtılan kaynaklarla ilgili etkinliği hakkında ayrıntılı denetim günlüklerini sağlar. Bunlar ve işletim sistemi düzeyinde günlükleri günlük analizi tarafından toplanır ve günlük analizi çalışma alanında depolanır. Günlük analizi bu çözümü tarafından dağıtılmış kaynaklar genelinde karşılık gelen denetim verilerini ve müşteri tarafından dağıtılan web uygulamasına genişletilebilir. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Azure etkinlik günlüğü olaylarını denetim bilgilerini 20'den fazla türleri için alanları içeren ayrıntılı bir şema kullanın. Etkinlik günlüğü yanı sıra bu şeması Windows günlükleri, Linux günlükleri, Azure tanılama günlüklerini ve müşteri günlükleri de dahil olmak üzere veri kaynakları, farklı bir kümesini destekler günlük analizi çözümü dağıtır.  |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı denetim verilerini kuruluş genelinde situational tanıma destekleme dağıtılan kaynaklar arasında merkezileştirmek için günlük analizi çözüm uygular. Daha fazla günlük analizi diğer sistemlerle tümleştirmek için müşteriler may seçerseniz. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı denetim verilerini dağıtılan kaynaklar, Destek Merkezi gözden geçirme, çözümleme ve raporlama arasında merkezileştirmek için günlük analizi çözüm uygular. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu şeması güvenlik ve denetim çözümü dağıtır. Bu çözüm güvenlik tutumunu kapsamlı bir görünümünü sağlar. Güvenlik ve Denetim Panosu kullanılabilir veri taban çizgisi ve düzeltme eki değerlendirmesi güvenlik açığı verileri ve günlük verilerini tümleştirme dağıtılan günlük analizi çözümleri kullanılarak dağıtılan kaynakların güvenlik durumu üst düzey bir anlayış sağlar. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi çözüm uygular. Günlük analizi, merkezi bir depoya yönetilen kaynaklarından veri toplayarak Azure için izleme hizmetleri sağlar. Toplanan veriler uyarı, analiz ve dışarı aktarma için kullanılabilir hale gelir. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi çözüm uygular. Günlük analizi, merkezi bir depoya yönetilen kaynaklarından veri toplayarak Azure için izleme hizmetleri sağlar. İçerik ve Denetim kayıtlarını zaman sıralama günlük analizi tarafından toplanan zaman değiştirilmiş değil. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi çözüm uygular. Günlük analizi, merkezi bir depoya yönetilen kaynaklarından veri toplayarak Azure için izleme hizmetleri sağlar. Toplanan veriler uyarı, analiz ve dışarı aktarma için kullanılabilir hale gelir. Log Analytics, depoda saklanan verilerin ayıklanması için güçlü bir sorgu dili içerir. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi hizmeti uygular. Dağıtılan VM'ler ve Azure tanılama depolama hesapları günlük analizi için bağlı kaynaklar ve bunların kaynaktan ayrı olarak korunur. Verileri, günlük analizi tarafından yakın gerçek zamanlı olarak toplanır. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi hizmeti uygular. Günlük analizi gelen verilerin güvenilir bir kaynaktan sertifikalar ve Azure kimlik doğrulaması ile veri bütünlüğünü doğrulayarak olmasını sağlar. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı denetim özelliği Azure İzleyicisi'ni ve günlük analizi hizmeti tarafından sağlanır. Azure İzleyicisi dağıtılan kaynaklarla ilgili etkinliği hakkında ayrıntılı denetim günlüklerini sağlar. Bunlar ve işletim sistemi düzeyinde günlükleri günlük analizi tarafından toplanır ve günlük analizi çalışma alanında depolanır. Bu günlükler, bilgileri sistem olayları ayrıntılı kayıtlar içeriyordu ve inkar karşı korunmasına yardımcı olabilir. Ayrıca, verileri günlüğe kaydetmek için erişim unauthored değişiklik ya da günlük verilerinin silinmesini önlemek için rol tabanlı erişim denetimini kullanarak sınırlıdır. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi hizmeti uygular. Günlük analizi, merkezi bir depoya yönetilen kaynaklarından veri toplayarak Azure için izleme hizmetleri sağlar. Toplandığında, günlük analizi yapılandırması başına bir yıl için veriler korunur. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
...
|||
|---|---|
| **Müşteri** | Bu ayrıntılı günlük analizi hizmeti uygular. Günlük analizi, merkezi bir depoya yönetilen kaynaklarından veri toplayarak Azure için izleme hizmetleri sağlar. Denetim kaydı zaman damgaları değiştirilmediğini, bu nedenle denetim izi zaman bağıntılı. |
| **Sağlayıcı (Microsoft Azure)** | Uygulanamaz |
