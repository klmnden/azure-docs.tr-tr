---
title: Azure güvenlik ve uyumluluk planı - PCI DSS için veri analizi
description: Azure güvenlik ve uyumluluk planı - PCI DSS için veri analizi
services: security
author: meladie
...
Azure güvenlik ve uyumluluk planı, ödeme kartı sektörü veri güvenliği standartları (PCI DSS 3.2) gereksinimleri ile veri analizi mimarisi yönetmenize yardımcı olan Azure dağıtımı için yönergeler sağlar. Bu yaygın bir başvuru mimarisi gösteren ve güvenli, uyumlu, çok katmanlı bir ortamda (kartı numarası, geçerlilik süresi ve doğrulama verileri dahil) kredi kartı verileriyle doğru işlenmesi gösterir. Bu şema, müşterilerin belirli güvenlik ve uyumluluk gereksinimlerini karşılayabilecek bir yolunu gösterir ve müşterilerin oluşturmak ve Azure'da kendi veri analizi çözümleri yapılandırmak bir temel olarak görev yapar.
...
![PCI DSS başvurusu mimari şeması için Analytics](images/pcidss-analytics-architecture.png "analizi için PCI DSS başvuru mimarisi diyagramı")
...
Bu başvuru mimarisine yönelik veri akış diyagramı kullanılabilir [indirme](https://aka.ms/PCIAnalyticsTM) veya altında bulunabilir. Bu model, değişiklikler yaparken sistemi altyapısında potansiyel risk puanları anlamasına yardımcı olabilir.

![PCI DSS başvurusu mimari şeması için Analytics](images/pcidss-analytics-threat-model.png "analizi için PCI DSS tehdit modeli")

## <a name="compliance-documentation"></a>Uyumluluk belgeleri
[Azure güvenlik ve uyumluluk planı - PCI DSS müşteri sorumluluk matris](https://aka.ms/pcidss-analytics-tm) sorumlulukları PCI DSS 3.2 tüm gereksinimleri listeler.

[Azure güvenlik ve uyumluluk planı - PCI DSS veri analizi uygulaması matris](https://aka.ms/pcidss-analytics-cim) üzerinde hangi PCI DSS 3.2 gereksinimleri yönelik ayrıntılı açıklamaları da dahil olmak üzere veri analizi mimarisi tarafından bilgi sağlar Uygulama kapsanan her denetimin gereksinimleri nasıl karşıladığını biri.
