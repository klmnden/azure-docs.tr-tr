...
ms.locfileid: "46989694"
---
#<a name="load-data-with-the-azure-machine-learning-data-prep-sdk"></a>Azure Machine Learning veri hazırlığı SDK'sı ile veri yükleme

[Azure Machine Learning veri hazırlığı SDK'sı](https://docs.microsoft.com/python/api/overview/azure/dataprep?view=azure-dataprep-py) , farklı giriş veri türleri sağlar. Veri dosyası türünü ve parametrelerini belirtin veya otomatik olarak bir dosya türünü algılamak için SDK'sı akıllı okuma işlevini kullanabilirsiniz.

## <a name="read-lines"></a>Satırları oku
...
## <a name="read-excel"></a>Okuma Excel
Azure Machine Learning veri hazırlığı SDK'sını içeren bir `read_excel` Excel dosyalarını yüklemek için işlevi.
```
...
Excel dosyası birinci sayfasını yüklediniz. Açıkça yüklemek istediğiniz sayfanın adını belirterek aynı sonucu elde edebilirsiniz. Bunun yerine ikinci sayfa yüklemek istiyorsanız, bağımsız değişken olarak abonenin adını sağlayabilirsiniz.
```
...
Gördüğünüz gibi tablonun ikinci sayfasında üst bilgiler ve üç boş satır yoktu. İşlev bağımsız uygun şekilde değiştirmeniz gerekir.
```
...
## <a name="read-fixed-width-files"></a>Sabit genişlikte dosyaları okuma
Sabit genişlikli dosyalar için uzaklık listesi belirtebilirsiniz. İlk sütunda, her zaman 0 uzaklığında başlatmak için varsayılır.
...
Dosyaları bir üstbilgi varsa, ilk satır verisi olarak kabul isteyebilirsiniz. Geçirerek `PromoteHeadersMode.NONE` üstbilgi anahtar sözcük bağımsız değişkeni için üst bilgi algılama önlemek ve doğru verileri alın.
...
SQL Server'dan veri okumak için gerekli bağlantı bilgilerini içeren bir veri kaynağı nesnesi oluşturun.
...
Bir veri kaynağı nesnesi oluşturduktan sonra veri okumak için geçebilirsiniz.
```
...
## <a name="read-from-azure-data-lake-storage"></a>Azure Data Lake depolama alanından okuyun
...
Bir hizmet sorumlusu oluşturmak için Azure CLI'yı ve karşılık gelen sertifikayı kullanabilirsiniz. Bu belirli bir hizmet sorumlusu okuyucu Azure Data Lake Storage hesabını yalnızca 'dpreptestfiles' sınırlı kapsamı ile yapılandırılmış
```
...
##### <a name="configure-an-azure-data-lake-storage-account-for-the-service-principal"></a>Hizmet sorumlusunun bir Azure Data Lake Storage hesabını yapılandırın
Azure Data Lake Store dosya sistemine ACL yapılandırmak için kullanıcının veya bu örneğin, hizmet sorumlusu nesne kimliği'ni kullanın:
```
az ad sp show --id "8dd38f34-1fcb-4ff9-accd-7cd60b757174" --query objectId
```
##### <a name="configure-read-and-execute-access-for-the-azure-data-lake-storage-file-system"></a>Yapılandırma okuma ve yürütme erişimi Azure Data Lake Store dosya sistemi.
Temel alınan HDFS ACL modeli devralma desteklemediğinden, ACL klasörleri ve dosyaları için tek tek yapılandırmanız gerekir.
```
