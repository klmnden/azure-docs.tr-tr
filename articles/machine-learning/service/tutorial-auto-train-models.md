---
title: 'Öğretici: Sınıflandırma modelini otomatikleştirilmiş makine öğrenmesiyle eğitme - Azure Machine Learning hizmeti'
description: Otomatikleştirilmiş makine öğrenmesini kullanarak makine öğrenmesi modeli oluşturmayı öğrenin.  Azure Machine Learning, sizin için otomatikleştirilmiş bir yolla veri ön işlemini, algoritma seçimini ve hiper parametre seçimini gerçekleştirebilir. Bundan sonra son model Azure Machine Learning hizmetiyle dağıtılır.
...
> * Yerel bilgisayarınızda otomatikleştirilmiş sınıflandırıcı kullanarak eğitme
> * Sonuçları inceleme
...
Kolaylık olması için, bu öğretici bir [Jupyter notebook](https://aka.ms/aml-notebook-tut-03) olarak sağlanır. `03.auto-train-models.ipynb` notebook'unu Azure Notebooks üzerinde veya kendi Jupyter notebook sunucunuzda çalıştırın.
...
Geliştirme çalışmanızdaki tüm kurulum Python notebook’da gerçekleştirilebilir.  Kurulum şunları içerir:

* Python paketlerini içeri aktarma
* Yerel bilgisayarınızla uzak kaynaklar arasında iletişim kurulmasına olanak tanımak için çalışma alanını yapılandırma
* Eğitim betiklerini depolamak için dizin oluşturma
...
Mevcut çalışma alanından bir çalışma alanı nesnesi oluşturun. `Workspace.from_config()`, **aml_config/config.json** dosyasını okur ve ayrıntıları `ws` adlı nesneye yükler.  Bu öğreticideki kodun kalanında `ws` kullanılır.

Bir çalışma alanı nesneniz olduğunda, deneme için bir ad belirtin ve çalışma alanıyla yerel bir dizini kaydedin. Tüm çalıştırmaların geçmişi belirtilen denemenin altına kaydedilir.
...
İlk eğitim öğreticisi MNIST veri kümesinin yüksek çözünürlüklü bir sürümünü kullanmıştı (28x28 piksel).  Otomatikleştirilmiş ML eğitimi için çok fazla yineleme gerektiğinden, her yineleme için gereken süreyi kısaltmak ve kavramları göstermek için bu öğreticide daha düşük çözünürlükte görüntü sürümleri kullanılır (8x8).  
...
Verileri `numpy` dizilerine yükleyin. Ardından `matplotlib` kullanarak, üst kısımlarında etiketleriyle veri kümesinden 30 rastgele görüntü çizin.
...
![rakamlar](./media/tutorial-auto-train-models/digits.png)
...
Modeli otomatik olarak eğitmek için önce denemenin yapılandırma ayarlarını tanımlayın ve sonra da denemeyi çalıştırın.


### <a name="define-settings"></a>Ayarları tanımlama

Deneme ayarlarını ve model ayarlarını tanımlayın.

|Özellik| Bu öğreticideki değer |Açıklama|
|----|----|---|
|**primary_metric**|Ağırlıklı AUC | İyileştirmek istediğiniz ölçüm.|
|**max_time_sec**|12.000|Her yineleme için saniye cinsinden süre sınırı|
...
Denemeyi yerel olarak çalıştırmak için başlatın. İşlem hedefini yerel olarak ayarlayın ve denemedeki ilerleme durumunu görüntülemek için çıkışı true olarak ayarlayın.
...
Her yineleme devam ettikçe, çıkış aşağıdaki gibi bir kerede bir satır olmak üzere görüntülenir.  Her **10-15 saniyede bir** yeni bir satır görürsünüz.
...
Deneme sonuçlarını bur Jupyter pencere öğesiyle veya deneme geçmişini inceleyerek keşfedin.
...
Burada pencere öğesinin statik bir görüntüsü yer alır.  Notebook'ta, her yinelemede kullanılabilen her ölçümün grafiğini görüntülemek için, grafiğin üst kısmındaki açılan listeyi kullanabilirsiniz.

![pencere öğesi tablosu](./media/tutorial-auto-train-models/table.png)
![pencere öğesi çizimi](./media/tutorial-auto-train-models/graph.png)

### <a name="retrieve-all-iterations"></a>Tüm yinelemeleri alma

Deneme geçmişini görüntüleyin ve her yineleme çalıştırması için tek tek ölçümlere bakın.
...
Bu tabloda sonuçlar gösterilir:
...
## <a name="register-the-best-model"></a>En iyi modeli kaydetme 

En iyi modeli almak ve çalışma alanına kaydetmek için `local_run` nesnesini kullanın. 
...
## <a name="test-the-best-model"></a>En iyi modeli test etme

Birkaç rastgele rakamı tahmin etmek için modeli kullanın.  Tahmin edilen değeri ve görüntüyü gösterin.  Yanlış sınıflandırılmış örnekleri vurgulamak için kırmızı yazı tipi ve ters görüntü (siyah üzerine beyaz) kullanılır.
...
![sonuçlar](./media/tutorial-auto-train-models/results.png)
...
> * Geliştirme ortamınızı kurma
> * Verilere erişme ve onları inceleme
> * Özel parametrelerle yerel olarak otomatikleştirilmiş sınıflandırıcıyı kullanıp eğitme
> * Sonuçları inceleme
> * Eğitim sonuçlarını gözden geçirme
> * En iyi modeli kaydetme

[Otomatik eğitim ayarlarını yapılandırma](how-to-configure-auto-train.md) veya [uzak kaynakta otomatik eğitimi kullanma](how-to-auto-train-remote.md) hakkında daha fazla bilgi edinin.  
