---
title: Oluşturun ve metin analizi için Azure Machine Learning paketi kullanarak bir metin sınıflandırma modeli dağıtın.
description: Oluşturma, eğitme, sınama ve metin analizi için Azure Machine Learning paketi kullanarak bir metin sınıflandırma modeli dağıtma konusunda bilgi edinin.
...
ms.locfileid: "46994947"
---
# <a name="build-and-deploy-text-classification-models-with-azure-machine-learning"></a>Azure Machine Learning ile metin sınıflandırma modellerini Derleme ve dağıtma

[!INCLUDE [workbench-deprecated](../../../includes/aml-deprecating-preview-2017.md)]


Bu makalede, eğitmek ve bir metin sınıflandırma modeli dağıtma hakkında bilgi edinebilirsiniz **metin analizi için Azure Machine Learning paketi** (AMLPTA). Metin sınıflandırma amacı bir metin parçası için atamak için ya da daha fazla sınıflar veya kategorileri önceden tanımlanmış. Bu metin, olabilir belge, haber makaleleri, arama sorgusu, e-posta, tweet, destek biletlerini.

Metin sınıflandırma gibi geniş kapsamlı uygulamalar vardır: 
+ Kategorilere gazete makaleler ve kablo içeriğine haber konuları
+ Hiyerarşik kategoriler halinde Web sayfalarını düzenleme 
+ İstenmeyen e-posta filtreleme
+ Yaklaşım analizi
+ Arama sorgularından kullanıcının amacını tahmin etme
+ Yönlendirme destek biletlerini
+ Müşteri geri bildirimi analiz etme 

Metin sınıflandırma modeli yapı ve dağıtım iş akışı AMLPTA sahip bir model şu şekildedir:

1. Verileri yükleme
2. Modeli eğitme
3. Sınıflandırıcı Uygula 
4. Model performansını değerlendirme
5. İşlem hattı Kaydet
6. İşlem hattını test etme
8. Bir web hizmeti olarak modeli dağıtacağız

Başvurun [paketini başvuru belgeleri](https://aka.ms/aml-packages/text) her modülü ve sınıf için ayrıntılı başvuru.

Bu makaledeki örnek kod kullanan bir scikit-işlem hattı öğrenin.
...
1. Aşağıdaki hesapları ve uygulama kurmalı ve yüklü:
   - Azure Machine Learning denemesi hesabı 
...
1. Metin analizi için Azure Machine Learning paketi yüklü olmalıdır. Bilgi edinmek için nasıl [burada bu paketi yüklemek](https://aka.ms/aml-packages/text).
...
Kendiniz deneyin. Not defterini indirin ve kendiniz çalıştırın.

> [!div class="nextstepaction"]
> [Jupyter not defterini alma](https://aka.ms/aml-packages/text/notebooks/text_classification_sentiment_data)

### <a name="download-and-explore-the-sample-data"></a>İndirin ve örnek verileri keşfedin
Aşağıdaki örnekte [20 haber dataset](http://qwone.com/~jason/20Newsgroups/) scikit kullanılabilen-metin Sınıflandırıcısı, metin analizi için Azure Machine Learning paketiyle oluşturma işlemini göstermek için kitaplık öğrenin. 

İki alt kümede bölünmüş 20 farklı konularda 18, 000'etrafında haber gönderileri 20 haber veri kümesi vardır: biri eğitim ve diğer bir performans değerlendirmesi için. Eğitme ve test Böl her ileti gönderi tarihi alan önce ya da belirli bir tarihten sonra.
...
### <a name="set-the-location-of-the-data"></a>Veri konumunu ayarlama
Veri dizini parametre verilerinde indirdiğiniz konumunu ayarlayın. Kendi verilerinizi kullanmak, giriş veri kümesi *.tsv dosya biçimi olmalıdır.
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      <th>1</th>
      <td>5</td>
      <td>Kimden: Rick Miller &lt; rick@ee.uwm.edu &gt;\nSubject:...</td>
    </tr>
    <tr>
      <th>2</th>
      <td>0</td>
...
Kategori adlarıyla arasındaki correspondance alın.
...
Şimdi, eğitim sınıf sıklık Ön araştırma çizim histogram oluşturmak ve veri kümelerini test. 
...
Önceki kod bloğu çalıştırdıktan sonra Jupypter not defteri çalıştırırken, çizimleri görüntülenir.


## <a name="train-the-model"></a>Modeli eğitme

### <a name="specify-scikit-learn-algorithm-and-define-the-text-classifier"></a>Scikit belirtin-algoritması öğrenin ve metin Sınıflandırıcısı tanımlayın

Bu adım bir scikit eğitimi içerir-Rest karşı birini kullanarak metin sınıflandırma modeli öğrenme Lojistik regresyon öğrenme algoritmasını.

Dersleri tam listesi için başvurmak [Scikit Öğrencileriyle](http://scikit-learn.org/stable/supervised_learning) belgeleri.
...
Paket varsayılan parametreleri kullanır. Varsayılan olarak, metin Sınıflandırıcısı ayıklar:
...
Eğitim sırasında hem metin hem de etiket sütun başlıkları olmalıdır. Ancak yalnızca metin sütununu tahminler elde etmek için gereklidir. 

### <a name="examine-and-set-the-parameters-of-the-different-pipeline-steps"></a>İncelemek ve farklı bir işlem hattı adımların parametrelerini ayarlayın
    
Genellikle, bir model uygun önce parametrelerini ayarlayın. 

***Text_word_ngrams ile gösterilen örnek*** 

Aşağıdaki kod örnekleri varsayılan işlem hattı ve model parametreleri kullanarak modeli eğitmek nasıl gösterir. 

Parametreleri "text_word_ngrams için" dahil olduğunu görmek için kullanmak [get_step_param_names_by_name](https://docs.microsoft.com/python/api/tatk.core.base_text_model.basetextmodel). Bu işlev, vb. gibi küçük input_col, output_col parametreleri döndürür. 
...
Gerekirse varsayılan parametreleri değiştirebilirsiniz.  Aşağıdaki kod ile ayıklanan karakter n-karakter üç-gram hem de 4 gram ayıklanacak gram'lar (4,4) öğesinden (3,4) için aralığını değiştirebilirsiniz:
...
### <a name="export-the-parameters-to-a-file"></a>Parametreleri bir dosyaya aktarın.
Gerekirse, düzeltilmiş parametrelerle adım sığdırma modeli çalıştırarak model performansını en iyi duruma getirebilirsiniz:
...
Eğitilen metin Sınıflandırıcısı sınıfı tahminler üretmek için test veri kümesinde geçerlidir:
...
[Değerlendirme Modülü](https://docs.microsoft.com/python/api/tatk.evaluation) test veri kümesinde eğitilen metin Sınıflandırıcısı doğruluğunu değerlendirir. Evaluate işlevi, bir karışıklık matrisi oluşturur ve bir makro F1 puanı sağlar.
...
Karışıklık olmadan görselleştirme normalleştirme matrisi çizebilirsiniz.
...
Normalleştirme olmadan karışıklık Matrisi
    
Not defterini çalışırken karışıklık matrisi görüntülenir



Görselleştirme için normalleştirilmiş karışıklık matrisi çizebilirsiniz.
...
## <a name="save-the-pipeline"></a>İşlem hattı Kaydet
Sınıflandırma işlem hattı, bir zip dosyası olarak kaydedin. Ayrıca, word ngrams ve karakter n-gram'lar, metin dosyaları olarak kaydedin.
...
## <a name="load-the-pipeline"></a>İşlem hattını yükleyin
Sınıflandırma işlem hattı ve sözcük ngrams ve karakter n-gram için çıkarım yükle:
...
## <a name="test-the-pipeline"></a>İşlem hattını test etme

Test veri kümesini değerlendirmek için yüklenen metin sınıflandırma işlem hattı geçerlidir:
...
Bu bölümde, bir Azure Machine Learning web hizmetini kullanarak olarak metin sınıflandırma işlem hattını dağıtmak [Azure Machine Learning CLI](https://docs.microsoft.com/azure/machine-learning/desktop-workbench/cli-for-azure-machine-learning). Ardından, eğitim ve puanlama web hizmeti kullanır.

**Azure CLI ile Azure aboneliğinizde oturum açın**
...
1. Azure Machine Learning ortamınızı emin olmak için model Yönetimi hesabı ve kaynak grubunun aynı bölgede yer alır.

1. Blob depolama alanından dağıtım yapılandırma dosyası indirin ve yerel olarak kaydedin:
...
1. Kaynaklarınızı yansıtmak için indirmiş olduğunuz dağıtım yapılandırma dosyasını güncelleştirin:
...
1. Eğitilen model başarıyla dağıtıldı düşünüldüğünde, yeni bir veri kümesini Puanlama web hizmeti çağırırsınız:
...
1. Web hizmeti adını kullanarak herhangi bir zamanda yükle:
...
1. Web hizmeti, 20 newsgrpoups veri kümesinden alınan iki e-posta gövdesi ile test edin:
...
Azure Machine Learning paketi hakkında daha fazla metin analizi için aşağıdaki makalelerde öğrenin:

+ Okuma [genel bakış paketini](https://aka.ms/aml-packages/text).

+ Keşfedin [başvuru belgeleri](https://aka.ms/aml-packages/text) bu paket için.
