...
Bu makalede, sizi tüm aracılara--başvurmak olup olmadığını Linux veya Windows, System Center Operations Manager yönetim grubu veya doğrudan günlük analizi--olarak bağlı olup olmadığını *OMS Aracısı*. Aracının dağıtım adını ancak bağlam için gerekli olduğunda kullanacağız.
...
![Hizmet eşlemesi aracıları](media/monitoring-service-map/agents.png)
...
OMS doğrudan Aracısı'nı kullanıyorsanız, OMS günlük analizi veya OMS ağ geçidine bağlanmak için aracının kendisi yapılandırmanız gerekir. OMS ağ geçidi yüklenebilir [Microsoft Download Center](https://www.microsoft.com/download/details.aspx?id=52666). OMS ağ geçidi yapılandırmak ve dağıtmak hakkında daha fazla bilgi için bkz: [OMS ağ geçidi'ni kullanarak Internet erişimi bilgisayarları bağlamak](../log-analytics/log-analytics-oms-gateway.md).  
...
1.  Açıklanan yöntemlerden birini aşağıdaki OMS Aracısı [günlük analizi ile ortamınızdaki bilgisayarlardan verileri toplama](../log-analytics/log-analytics-concept-hybrid.md).
...
#### <a name="installer-prompts-for-a-reboot"></a>Yeniden başlatma için yükleyici ister
...
- Bilgi nasıl [hizmet eşlemesi kullanmak]( monitoring-service-map.md) sonra dağıtılan ve yapılandırılmış.
