...
Bağlantı sağlayıcınız listelenip listelenmediğini denetleyin. Aşağıdaki bilgileri not edin. Bir bağlantı hattı oluşturduğunuzda, daha sonra gerekecektir.
...
Aşağıdaki örnek 200 MB/sn expressroute bağlantı hattı üzerinden Equinix Silikon Vadisi'nde oluşturulacağını gösterir. Farklı bir sağlayıcı ve farklı ayarlar kullanıyorsanız, bu bilgileri isteğiniz yaptığınızda değiştirin. Yeni bir hizmet anahtarı için bir örnek isteği verilmiştir:
...
Yanıt aşağıdaki örneğe benzer görünür:
...
Bağlantı sağlayıcı onu sizin için etkinleştirme sürecinde olduğunda bağlantı hattı aşağıdaki durumuna değiştirin:
...
Yanıt aşağıdaki örneğe benzer olacaktır:
...
Sınırlar ve sınırlamalar hakkında daha fazla bilgi için başvurmak [ExpressRoute SSS](expressroute-faqs.md).
...
Bağlantı hattı şimdi etkin ExpressRoute premium eklentisi özellikleri sahip olur. Komut başarıyla çalıştırıldı hemen premium eklenti özellik faturalama işlemini başlatacak.
...
* Standart Premium'dan düşürmek önce devresine bağlı sanal ağlar sayısı 10'dan az olduğundan emin olmalısınız. Bunu yapmazsanız, güncelleştirme isteği başarısız olur ve biz premium oranlarda faturalanacağı.
