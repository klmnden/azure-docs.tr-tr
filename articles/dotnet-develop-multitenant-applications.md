...
Azure SQL veritabanı veya Azure depolama hizmetleri gibi hizmetleri için büyük miktarda yapılandırılmamış verileri depolama ve büyük miktarda yapılandırılmamış metin ve ikili depolamak için hizmetleri sağlayan Blob hizmeti sağlayan tablo hizmeti gibi veri yönetimi video, ses ve görüntü gibi verileri.

* Çok Kiracılı verileri SQL veritabanında uygun güvenlik altına alma SQL Server oturumları Kiracı başına.
* Uygulama kaynakları belirten bir kapsayıcı düzeyinde erişim ilkesi tarafından için Azure tabloları kullanarak yeni URL'leri paylaşılan erişim imzaları ile korumalı kaynaklar için vermek zorunda kalmadan izinleri ayarlamanızı yapabilirsiniz.
...
* Azure Active Directory (Azure AD), bulut uygulamalarınız için kimlik yönetimi ve erişim kontrolü özellikleri sağlayan modern, REST tabanlı bir hizmettir. Azure AD ile kimlik doğrulaması ve yetkilendirme, kodunuzu faktörlenebilen için özelliklerinin verirken web uygulamaları ve Hizmetleri erişim kazanmak için kimlik doğrulama ve kullanıcıları yetkilendirme kolay bir yol sağlar, uygulama kaynakları için Azure AD kullanarak.
* Azure Service Bus, güvenli Mesajlaşma sağlar ve veri akış özelliği için dağıtılmış ve karmaşık güvenlik duvarı ve güvenlik gerek kalmadan karma uygulamaları, Azure arasındaki iletişimi gibi uygulamalar ve şirket içi uygulamalar ve hizmetler, barındırılan altyapıları. Uç noktaları olarak kullanıma sunulan hizmetler için uygulama kaynakları için Service Bus geçişi kullanarak (örneğin, şirket içi gibi sistemin dışında barındırılan) kiracısına ait olabilir ya da (çünkü özellikle Kiracı için sağlanan hizmetler olabilir duyarlı, kiracıya özgü verileri bunların arasında dolaşır).

**Kaynak sağlama**

Azure, uygulama için yeni Kiracı sağlama çeşitli şekillerde sağlar. Çok sayıda kiracılar ile çok kiracılı uygulamaları için Self Servis sağlama etkinleştirilerek bu işlemi otomatikleştirmek genellikle gereklidir.
