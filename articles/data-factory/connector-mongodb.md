...
| password |Kullanıcının parolası. Bu alan SecureString işaretleyin. |Evet (Temel kimlik doğrulaması kullanılıyorsa). |
...
> [!TIP]
> Ne zaman dikkat DateTime biçimine SQL sorgusunu belirtin. Örneğin, `$$Text.Format('SELECT * FROM Account WHERE LastModifiedDate >= {{ts\\'{0:yyyy-MM-dd HH:mm:ss}\\'}} AND LastModifiedDate < {{ts\\'{1:yyyy-MM-dd HH:mm:ss}\\'}}', <datetime parameter>, <datetime parameter>)`
