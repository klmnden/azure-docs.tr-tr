...
3. **JSON** değerini aşağıdaki JSON ile değiştirin:
    ...
    Özellik | Açıklama
    -------- | ----------- 
    Sürüm | Oluşturulacak HDInsight Hadoop kümesinin sürümünü belirtir.
...
3. Düzenleyicide **JSON** değerini aşağıdaki JSON kod parçacığıyla değiştirin:
    ...
    external | Bu özellik, giriş verileri işlem hattı tarafından oluşturulmadıysa true olarak ayarlanır. Bu özellik yalnızca giriş veri kümelerinde belirtilir. Birinci etkinliğin giriş veri kümesi için her zaman true olarak ayarlayın.
...
3. Düzenleyicide **JSON**’u aşağıdaki JSON ile değiştirin:
    ...
    Burada, **availability** bölümü çıktı veri kümesinin aylık tabanda oluşturulduğunu belirtiyor. Çıkış veri kümesi, işlem hattının zamanlamasını belirler. İşlem hattı, başlangıç ve bitiş saatleri arasında ayda bir kez çalışır. 
...
3. **JSON**’u aşağıdaki kod parçacığıyla değiştirin:
    ...
    **partitionweblogs.hql** Hive betik dosyası Azure depolama hesabında (scriptLinkedService tarafından belirtilir) ve `adfgetstarted` kapsayıcısındaki `script` klasöründe depolanır.
    ...
    İşlem hattının **start** ve **end** özellikleri işlem hattının etkin dönemini belirtir. Aylık olarak üretilecek veri kümesini yapılandırdınız; bu nedenle, işlem hattı tarafından yalnızca bir dilim üretilir (ay aynı başlangıç ve bitiş tarihleri arasında olduğu için).
...
Sonraki adımda çözümü yayımladığınızda, **partitionweblogs.hql** dosyası `adfgetstarted` blob kapsayıcısındaki **betik** klasörüne yüklenir.   
...
1. [Azure portalında](https://portal.azure.com/) oturum açıp aşağıdaki adımları uygulayın:
   ...
   2. Data factory listesinden data factory’nizin adını seçin (örneğin: **DataFactoryUsingVS09152016**).
...
Dağıtımı yaptığınızda, yapılandırma dosyasına ait değerler, varlıklar Azure Data Factory hizmetine dağıtılmadan önce JSON dosyalarında özelliklere değer ayarlamak için kullanılır.   
...
| Konu | Açıklama |
|:--- |:--- |
| [İşlem hatları](data-factory-create-pipelines.md) |Bu makale, Azure Data Factory’de işlem hatlarının ve etkinliklerini anlamanıza ve senaryonuz ya da işletmeniz için veri odaklı iş akışları oluşturmak amacıyla bunları nasıl kullanacağınızı anlamanıza yardımcı olur. |
