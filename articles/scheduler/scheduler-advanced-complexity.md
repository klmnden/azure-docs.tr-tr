...
| ***aralığı*** |Sayı |Hayır |1 |1 ile 1000. |<code>"interval":10</code> |
| ***endTime*** |Dize |Hayır |None |Gelecekteki bir zamanı temsil eden bir tarih saat değeri |<code>"endTime" : "2013-02-09T09:30:00-08:00"</code> |
| ***sayısı*** |Sayı |Hayır |None |>= 1 |<code>"count": 5</code> |
| ***zamanlama*** |Nesne |Hayır |None |Zamanlamaya nesnesi |<code>"schedule" : { "minute" : [30], "hour" : [8,17] }</code> |
...
Bu koşullar altında *ilk yürütme* 2015-04-09 14: 00'dan olacaktır\. Zamanlayıcı altyapısı, başlangıç zamanından itibaren yürütme oluşumu hesaplar.  Geçmişte herhangi örnekleri atılır. Altyapısı gelecekte oluşur sonraki örneği kullanır.  Bu durumda, *startTime* 2: 00'dan 2015-04-09 olan bu süre, 2 gün sonraki örneğidir 2: 00'da, 2015-04-07 olduğundan.
...
|:--- |:--- |:--- |
| **dakika** |İşin çalışacağı saat, dakika |<ul><li>Tamsayı veya</li><li>Dizisi</li></ul> |
| **saatleri** |İş çalışacağı günün saatleri |<ul><li>Tamsayı veya</li><li>Dizisi</li></ul> |
| **Haftanın günü** |İşin çalışacağı günleri. Yalnızca haftalık sıklığı ile belirtilebilir. |<ul><li>"Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi" ve "Pazar"</li><li>Yukarıdaki değerleri (en büyük dizi boyutu 7) dizisi</li></ul>*Değil* büyük küçük harfe duyarlı |
| **monthlyOccurrences** |Ayın hangi günleri iş çalışacağını belirler. Yalnızca bir aylık sıklığı ile belirtilebilir. |<ul><li>MonthlyOccurrence nesneler dizisi:</li></ul> <pre>{ "day": *day*,<br />  "occurrence": *occurrence*<br />}</pre><p> *gün* olan iş haftanın günü çalıştırılır, örneğin her Pazar ayın {Pazar}. Gerekli.</p><p>Oluşum *oluşumu* gün ay sırasında örneğin {Pazar, -1} ayın son Pazar ise. İsteğe bağlı.</p> |
| **monthDays** |Ayın iş çalıştırılır. Yalnızca bir aylık sıklığı ile belirtilebilir. |<ul><li>Herhangi bir değer < = -1 ve >-31 =.</li><li>Herhangi bir değer > = 1 ve < = 31 açın.</li><li>Yukarıdaki değerleri dizisi</li></ul> |
...
| <code>{"minutes":[15]}</code> |15 dakika olarak çalıştır saati aşan her saat. Çalıştıran her saat 00: 15'de, başlatma, 1:15 AM, 2: 15'da, vb. ve 10: 15'te ve 11: 15'te bitiş. |
...
| <code>{"minutes":[0,15,30,45], "weekDays":["monday", "tuesday", "wednesday", "thursday", "friday"]}</code> |Her 15 dakikada günlerinde Çalıştır |
