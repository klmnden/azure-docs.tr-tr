<properties
    pageTitle="App Service’de CORS desteği | Microsoft Azure"
...
8. Bir tarayıcıda [Azure portalına](https://portal.azure.com/) gidin.
...
10. **API uygulaması** dikey penceresinin sağ tarafında açılan **Ayarlar** dikey penceresinde, **API** bölümünü bulun ve ardından **CORS**’ye tıklayın.

    ![Ayarlar dikey penceresinden CORS’yi seçin](./media/app-service-api-cors-consume-javascript/clicksettings.png)

11. Metin kutusunda JavaScript çağrılarının alınmasına izin vermek istediğiniz URL’yi veya URL'leri girin.
...
3. **App Service Oluştur** iletişim kutusunun **Barındırma** sekmesinde, *azurewebsites.net* etki alanında benzersiz bir **Web Uygulaması Adı** girin. 
...
4. **App Service Planı** açılır listesinde, önceden oluşturduğunuz planı seçin. 
...
2. Tarayıcınızda Geliştirici Araçları’nda Konsol penceresini açın.

3. AngularJS kullanıcı arabirimini gösteren tarayıcı penceresinde **Yapılacaklar Listesi** bağlantısına tıklayın.
...
12. Metin kutusunda, ToDoListAngular (ön uç) web uygulamasının URL'sini girin. Örneğin, ToDoListAngular projesini todolistangular0121 adlı bir web uygulamasına dağıttıysanız, `https://todolistangular0121.azurewebsites.net` URL’sinden gelen çağrılara izin verin.
...
1. **WebApiConfig** sınıfının **Register** yöntemine aşağıdaki örnekteki gibi bir `config.EnableCors()` kod satırı ekleyin. 
...
1. Web API denetleyicinizde, `System.Web.Http.Cors` ad alanı için bir `using` bildirimi ekleyin ve denetleyici sınıfına veya tek tek eylem yöntemlerine `EnableCors` özniteliğini ekleyin. Aşağıdaki örnekte, CORS desteği tüm denetleyiciye uygulanır.
