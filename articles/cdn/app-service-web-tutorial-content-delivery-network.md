...
Bu öğretici, [Azure Uygulama Hizmeti'ndeki bir web uygulamasına](../app-service/app-service-web-overview.md) nasıl Azure CDN ekleyeceğinizi gösterir. [Azure Content Delivery Network (CDN)](cdn-overview.md), kullanıcılara içerik teslim etmek için en yüksek aktarım hızını sağlamak üzere stratejik olarak yerleştirilmiş konumlardaki statik web içeriğini önbelleğe alır. Azure CDN, ayrıca web uygulamanızdaki sunucu yükünü de azaltır. 
...
Henüz bir etki alanınız yoksa, Azure portalını kullanarak bir etki alanı satın almak için [App Service etki alanı öğreticisini](../app-service/custom-dns-web-site-buydomains-web-app.md) izlemeniz yararlı olabilir. 
...
Bir CNAME kaydı oluşturarak özel etki alanınızı CDN uç noktanıza eşleyeceksiniz. CNAME kaydı, bir kaynak etki alanını hedef etki alanına eşleyen bir DNS özelliğidir. Örneğin, cdn.contoso.com veya static.contoso.com'u contoso.azureedge.net'e eşleyebilirsiniz.

Özel bir etki alanınız yoksa, Azure portalını kullanarak bir etki alanı satın almak için [App Service etki alanı öğreticisini](../app-service/custom-dns-web-site-buydomains-web-app.md) izlemeniz yararlı olabilir. 

### <a name="find-the-hostname-to-use-with-the-cname"></a>CNAME ile kullanılacak ana bilgisayar adını bulma

Azure portalının **Uç Nokta** sayfasında, sol gezintide **Genel Bakış**'ın seçildiğinden emin olun ve ardından sayfanın üst kısmındaki **+ Özel Etki Alanı** düğmesini seçin.

![Özel Etki Alanı Ekle'yi seçin](media/app-service-web-tutorial-content-delivery-network/portal-select-add-domain.png)

**Özel etki alanı ekle** sayfasında, CNAME kaydı oluşturulurken kullanılacak uç nokta ana bilgisayar adını görürsünüz. Ana bilgisayar adı, CDN uç nokta URL'nizden türetilir: **&lt;UçnoktaAdı>.azureedge.net** . 
...
Etki alanı kayıt şirketinizin web sitesine gidin ve DNS kayıtları oluşturma bölümünü bulun. Örneğin, bu bölümün adı **Etki Alanı Adı**, **DNS** veya **Ad Sunucusu Yönetimi** olabilir.
...
**Özel etki alanı ekle** sayfasına geri dönün ve iletişim kutusuna alt etki alanıyla birlikte özel etki alanınızı girin. Örneğin *cdn.contoso.com* gidin.   
...
Tarayıcıda, özel etki alanınızı kullanarak *index.html* dosyasına gidin (örneğin, cdn.contoso.com/index.html) ve doğrudan &lt;uçnoktaadı&gt;azureedge.net/index.html konumuna gitmenizle aynı sonucu verdiğini doğrulayın.
