...
| Sorgu dizesi parametresi bulunamadı | % {is_amp} | Bu değişken için değeri istek en az bir sorgu dizesi parametresi içeriyor göre değişir.<br /><br />-Parametresi bulunamadı: &<br />-Herhangi bir parametre: NULL | & |
...
| Sorgu dizesi değeri | % {QUERY_STRING} | İstek URL'SİNDE tanımlanan tüm sorgu dizesi değerini gösterir. |key1 = val1 & key2 = val2 & anahtar3 val3 = |
| Başvuran etki alanı | % {referring_domain} | Referer istek üstbilgisinde tanımlanan etki alanını gösterir. | www.Google.com |
...
| İstek ana bilgisayarı | % {konak} | İstek URL'SİNDE tanımlanan konak gösterir. | www.mydomain.com |
| İstek Protokolü | % {request_protocol} | İstek Protokolü belirtir. | HTTP/1.1 |
| İstek düzeni | % {scheme} | İstek düzenini gösterir. |http |
| İstek URI'si (göreli) | % {request_urı} | Tanımlanan istek URI'SİNDE sorgu dizesi dahil olmak üzere, göreli yol gösterir. | /Marketing/foo.js?loggedin=true |
...
| ------ | -------- | ---------- |
| %{&lt;HTTPVariable&gt;} | % {konak} | Belirtilen karşılık gelen tüm değerini almak için şu söz dizimini kullanın &lt;HTTPVariable&gt;. |
...
- Değişkenle ilişkili değer dönüştürün.
     ...
     Örnek: Tüm küçük harfe Dönüştür.
...
| Sondaki karakterler | Bir değişken izine karakter değişmez değerler kabul edilir. <br />Örnek sağındaki değer değişmez değer kabul edilir izleyen bir küme ayracı içerir. | % {konak}} |
...
| Eksik olduğunda, bir üst bilgi varsayılan bir değere ayarlayın. | % {Değişken = Value} | % {http_referer = belirtilmeyen} | Referer üst bilgisi yalnızca ayarlanacak *belirtilmeyen* eksik olduğunda. Bunu ayarlarsanız, hiçbir eylem gerçekleşir. |
...
- Alt dizenin başlangıç karakteri uzaklığı terimi için atanan değer belirler:
     ...
     - : Sıfırdır başlangıç karakteri alt dizenin ilk karakteri bir dize.
     - Negatif: Dizenin başlangıç karakteri dizedeki son karakter hesaplanır.

- Alt dizenin uzunluğunu tarafından belirlenen *uzunluğu* dönemi:

     - Atlanmış: uzunluğu terimi atlama alt dizeyi başlangıç karakteri dize sonu arasındaki tüm karakterler sağlar.
...
| % {request_urı % html} htm | /800001/myorigin/Marketing/Product.HTML?Language=en-us | Değişken desen ile sonlanmıyor olduğundan, hiçbir değişiklik yoktu.|
...
- Bir işleç deseni önce belirtilmelidir. Belirtilen işleç deseni Yakalama davranışını belirler:
     ...
     - `$`: Belirtilen desen ile biten metin yakalama olacağını belirtir.
