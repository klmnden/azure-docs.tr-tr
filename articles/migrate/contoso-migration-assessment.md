...
Bu makalede, uygulamayı Azure'a geçiş için hazırlama, şirket içi SmartHotel uygulamada Contoso değerlendirir.

Bu makale, kendi şirket içi kaynaklara Contoso adlı kurgusal şirketin Microsoft Azure bulutuna nasıl geçirdiğini belgeleri makaleler serisinin üçüncü yöneliktir. Seri arka plan bilgileri içerir ve bir geçiş altyapısını kurma nasıl çalışılacağını dağıtım senaryolarında bir dizi geçiş için şirket içi kaynaklara uygunluğunu değerlendirmek ve farklı türde geçiş çalıştırın. Senaryoları, karmaşık hale gelmesi. Makaleler, zaman içinde serinin eklenir.
...
3. makale: şirket içi kaynaklarınızı Azure'a geçiş için değerlendirme | Contoso, Wmware'de çalışan kendi şirket içi iki katmanlı SmartHotel uygulamasının bir değerlendirme çalışır. Contoso uygulaması Vm'leri kullanarak değerlendirir [Azure geçişi](migrate-overview.md) hizmeti. Contoso kullanarak uygulama SQL Server veritabanı değerlendirir [Data Migration Yardımcısı](https://docs.microsoft.com/sql/dma/dma-overview?view=sql-server-2017). | Bu makalede
...
- **Verimliliği artırmak**: Contoso gereken gereksiz yordamları kaldırıp, geliştiriciler ve kullanıcılar için süreçlerini kolaylaştırabilirsiniz. Hızlı ve değil atık zaman veya para şirket kadar olacak şekilde iş gereksinimlerini BT müşteri gereksinimlerine daha hızlı sunabilirsiniz.
...
> [!div class="checklist"]
> * **1. adım: İndirme ve veri geçiş Yardımcısı'nı yüklemek**: Contoso hazırlar Data Migration Yardımcısı için şirket içi SQL Server veritabanının değerlendirmesi.
...
> * **5. adım: Azure Geçişi'ni kullanarak bağımlılık Analizine hazırlanma**: Contoso şirketi VM'ler arasındaki bağımlılık eşlemesini görebilmeniz için bu Azure geçişi aracılarını Vm'lerde yükler.
> * **6. adım: Azure Geçişi'ni kullanarak sanal makineleri değerlendirme**: Contoso bağımlılıkları denetler, Vm'leri gruplar ve değerlendirme çalışır. Değerlendirme hazır olduğunda, Contoso geçiş hazırlığında değerlendirmeyi analiz eder.
...
- **İzinleri**: seçin veri merkezi nesnesi için **alt nesneleri yay** onay kutusu. İçin **rol**seçin **salt okunur**.
...
3. Contoso MMA'yı yükler:
    ...
    - 64-bit komutlardır.
    - Çalışma alanı kimliği ve birincil anahtar Microsoft Operations Management Suite (OMS) Portalı'nda yer alır. Seçin **ayarları**ve ardından **bağlı kaynaklar** sekmesi.
    - OMS Aracısı'nı indirme, sağlama toplamını doğrulama ve yüklemek için aşağıdaki komutları çalıştırın ve ekleme aracısı:
