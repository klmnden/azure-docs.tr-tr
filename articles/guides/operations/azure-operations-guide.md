...
Azure kaynaklarını tek tek işlem, ağ, veri veya uygulama barındırma, Azure aboneliğinin dağıtılan Hizmetleri ' dir. Bazı ortak sanal makineler, depolama hesabı veya SQL veritabanları kaynaklardır. Azure Hizmetleri, genellikle birkaç ilgili Azure kaynaklarını oluşur. Örneğin, bir Azure sanal makinesi bir VM, depolama hesabı, ağ bağdaştırıcısı ve genel IP adresi içerebilir. Bu yok oluşturulmuş, yönetilen ve ayrı ayrı veya bir grup olarak silindi. Azure kaynakları, bu kılavuzun devamında daha ayrıntılı ele alınmıştır.
