...
Uygulama ağ geçidi ile arasındaki trafiği gönderen yeniden yönlendirme yapılandırması eklemek *aratıp* kullanarak [az ağ uygulama ağ geçidi yeniden yönlendirme-config oluşturma](/cli/azure/network/application-gateway/redirect-config#az_network_application_gateway_redirect_config_create).
